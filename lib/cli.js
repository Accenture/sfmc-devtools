...
// File's import-time notifier must not perform network/config work for local migrations.
if (hideBin(process.argv).includes('migrate')) {
    process.env.NO_UPDATE_NOTIFIER = '1';
}
const { Util } = await import('./util/util.js');
const { default: Mcdev } = await import('./index.js');
...
yargs(hideBin(process.argv))
    ...
    .command(
        'migrate <BU>',
        'Migrate local retrieve assets for one configured credential/BU (explicit confirmation required)',
        (yargs) =>
            yargs.positional('BU', {
                type: 'string',
                describe: 'exact configured credential/BU',
            }),
        async (argv) => {
            try {
                await Mcdev.migrate(argv.BU);
            } catch (ex) {
