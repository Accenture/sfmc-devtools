...
yargs
    ...
    .command({
        command: 'buildTemplate <BU> <TYPE> <NAME> <MARKET>',
        aliases: ['bt'],
        desc: 'Retrieves a specific metadata file by name for templating from a local folder',
        builder: (yargs) => {
            yargs
                .positional('BU', {
                    type: 'string',
        ...
        handler: (argv) => {
            Mcdev.setSkipInteraction(!!argv.skipInteraction);
            Util.setLoggingLevel(argv);
            Mcdev.buildTemplate(argv.BU, argv.TYPE, argv.NAME, argv.MARKET);
        },
    })
    .command({
