...
yargs(hideBin(process.argv))
        ...
        (yargs) =>
            yargs
                ...
                .option('refresh', {
                    ...
                    group: 'Options for deploy:',
                    describe:
                        'for asset-email: runs refresh command for related triggeredSends after deploy',
                })
                .option('execute', {
                    alias: 'e',
                    type: 'boolean',
        ...
        (yargs) =>
            yargs
                ...
                .option('shareAsset', {
                    type: 'boolean',
                    group: 'Options for build:',
                    describe: 'share assets with corresponding business unit',
                }),
        (argv) => {
            Mcdev.setOptions(argv);
            const typeKeyCombo = Mcdev.metadataToTypeKey(argv.metadata);
    ...
    .command(
        ['migrate [BU]'],
        'migrates historical asset groupings in one selected BU retrieve tree',
        (yargs) =>
            yargs.positional('BU', {
                type: 'string',
                describe:
                    'the business unit to migrate (in format "credential name/BU name"); omit to select interactively; use "credential name/*" to migrate all BUs of that credential',
            }),
        (argv) => {
            Mcdev.setOptions(argv);
            Mcdev.migrate(argv.BU);
        }
    )
    .command(
        ['upgrade', 'up'],
    ...
    .version(Util.packageJsonMcdev.version)
    .demandCommand(1, 'Please enter a valid command')
    .strict()
    .recommendCommands()
    .wrap(yargs(hideBin(process.argv)).terminalWidth())
    .epilog(
        'Copyright 2026. Accenture. Get support at https://github.com/Accenture/sfmc-devtools/issues'
    )
    .help().argv;
