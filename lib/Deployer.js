    ...
    static async deploy(businessUnit, selectedTypesArr, keyArr, fromRetrieve) {
        ...
        if (businessUnit === '*') {
            ...
            for (const buPath of deployFolders.filter((r) => r.includes(path.sep))) {
                ...
                await this._deployBU(cred, bu, properties, selectedTypesArr, keyArr);
                counter_credBu++;
                Util.logger.info('');
                Util.restartLogger();
            }
            ...
            if (bu === '*' && properties.credentials && properties.credentials[cred]) {
                ...
                for (const buPath of deployFolders) {
                    await this._deployBU(cred, buPath, properties, selectedTypesArr, keyArr);
                    counter_credBu++;
                    Util.logger.info('');
                    Util.restartLogger();
                }
    ...
    static async _deployBU(cred, bu, properties, typeArr, keyArr) {
        ...
        if (buObject !== null) {
            ...
            try {
                ...
                await deployer._deploy(typeArr, keyArr);
            } catch (ex) {
