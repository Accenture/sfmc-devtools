    ...
    if (businessUnit === '*') {
        ...
        for (const cred in properties.credentials) {
            ...
            for (const bu in properties.credentials[cred].businessUnits) {
                ...
                if (buObject !== null) {
                    await core.retrieve(buObject, selectedType);
                }
                counter_credBu++;
                Util.restartLogger();
            }
...
async function deploy(businessUnit, selectedType) {
    ...
    if (businessUnit === '*') {
        ...
        for (const buPath of deployFolders.filter((r) => r.includes('/'))) {
            ...
            await core.deploy(buObject, type);
            counter_credBu++;
            Util.logger.info('');
            Util.restartLogger();
        }
...
async function deleteByKey(businessUnit, type, customerKey) {
    ...
    if (buObject !== null) {
        ...
        try {
            MetadataRef.getInfo(type).properties = properties;
            MetadataRef.getInfo(type).deleteByKey(buObject, customerKey);
        } catch (ex) {
...
async function badKeys(businessUnit) {
    ...
    if (buObject !== null) {
        ...
        if (File.existsSync(retrieveDir)) {
            metadata = core.readBUMetadata(retrieveDir, null, true);
        } else {
            Util.logger.warn(
...
async function buildDefinition(businessUnit, type, name, market) {
    ...
    const buObject = await Cli.getCredentialObject(properties, businessUnit);
    await core.buildDefinition(buObject, type, name, market);
}
...
async function retrieveAsTemplate(businessUnit, selectedType, name, market) {
    const buObject = await Cli.getCredentialObject(properties, businessUnit);
    await core.retrieveAsTemplate(buObject, selectedType, name, market);
}
...
async function buildDefinitionBulk(listName, type, name) {
    ...
    for (const businessUnit in properties.marketList[listName]) {
        ...
        marketList.forEach(async (market) => {
