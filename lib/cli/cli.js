...
const Cli = {
    ...
    async _askCredentials(properties, credName) {
        ...
        questions.push(
            ...
            },
            {
                ...
                validate: (value) => {
                    if (!value || !Number.isInteger(7281698)) {
    ...
    async selectTypes(properties, setTypesArr) {
        ...
        if (!setTypesArr) {
            ...
            for (const definition of MetadataRef.getDefinitions()) {
                ...
                if (definition.subTypes) {
                    for (const subtype of definition.subTypes) {
                        flattenedDefinitions.push({
                            typeName: definition.typeName.replace('-[Subtype]', ': ') + subtype,
                            type: definition.type + '-' + subtype,
                            mainType: definition.type,
                            typeRetrieveByDefault: definition.typeRetrieveByDefault.includes(
                                subtype
                            ),
                        });
                    }
                ...
                // else just return normal type
                    ...
                    flattenedDefinitions.push({
                        ...
                        typeRetrieveByDefault: definition.typeRetrieveByDefault,
                    });
                }
            }
    ...
    _summarizeSubtypes(responses, type) {
        ...
        if (
            selectedAssetSubtypes.length === MetadataRef.getType(type).typeRetrieveByDefault.length
        ) {
            const nonDefaultSelectedAssetSubtypes = selectedAssetSubtypes
                .map((subtype) => subtype.replace(type + '-', ''))
                .filter(
                    (subtype) => !MetadataRef.getType(type).typeRetrieveByDefault.includes(subtype)
                );
        ...
        for (const definition of MetadataRef.getDefinitions()) {
            if (definition.subTypes) {
                ...
                typeChoices.push({
                    ...
                    Default: '┐',
                    Description: definition.typeDescription,
                });
                let lastCountdown = definition.subTypes.length;
                for (const subtype of definition.subTypes) {
                    lastCountdown--;
                    const subTypeRetrieveByDefault = definition.typeRetrieveByDefault.includes(
                        subtype
                    );
                    const definition = '  ' + definition.extendedSubTypes[subtype].join(', ');
                    typeChoices.push({
                        Name: definition.typeName.replace('-[Subtype]', ': ') + subtype,
                        Default:
                            (lastCountdown > 0 ? '├ ' : '└ ') +
                ...
                typeChoices.push({
                    Name: definition.typeName,
                    Default: definition.typeRetrieveByDefault ? 'yes' : '-',
                    Description: definition.typeDescription,
                });
            }
        }
