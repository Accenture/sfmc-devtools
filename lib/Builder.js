...
saved
     ...
     */
    async _buildDefinition(metadataType, nameArr, templateVariables, bindings) {
            ...
            const result = (
                await Promise.all(
                    nameArr.map((name) => {
                        ...
                        if (type === 'asset' && bindings) {
                            const binding = bindings.get(name);
                            if (!binding?.ownerPath) {
                                throw new Error(`Asset ${name} has no verified definition binding`);
                            }
                            return MetadataTypeInfo.asset.buildBoundAsset(
                                this.templateDir,
                                this.targetDir,
                                name,
                                templateVariables,
     ...
     */
    static async buildTemplate(businessUnit, selectedType, keyArr, marketArr, verifiedAssetInputs) {
     ...
     * @returns {Promise.<MultiMetadataTypeList>} Promise
     */
    async _buildTemplate(metadataType, keyArr, templateVariables, verifiedAssetInputs) {
        const type = metadataType;
        try {
            const removeKeys = [];
                    ...
                    keyArr.map(async (key) => {
                        ...
                        try {
                            if (
                                type === 'asset' &&
                                verifiedAssetInputs &&
                                !Array.isArray(verifiedAssetInputs.get(key))
                            ) {
                                throw new Error(`Asset ${key} has no verified template inputs`);
                            }
                            const response =
                                type === 'asset'
                                    ? await MetadataTypeInfo.asset.buildTemplate(
                                          this.retrieveDir,
                                          this.templateDir,
                                          key,
                                          templateVariables,
                                          verifiedAssetInputs?.get(key)
                                      )
                                    : await MetadataTypeInfo[type].buildTemplate(
                                          this.retrieveDir,
                                          this.templateDir,
                                          key,
                                          templateVariables
                                      );
                            if (response?.binding && verifiedAssetInputs) {
                                verifiedAssetInputs.set(key, response.binding);
                            }
                            if (!response) {
                                removeKeys.push(key);
                            }
     ...
     * @returns {Promise.<MultiMetadataTypeList>} -
     */
    static async buildDefinition(businessUnit, selectedType, nameArr, marketArr, bindings) {
        const properties = await config.getProperties();
        if (!properties) {
            return;
        }
        ...
        if (buObject !== null) {
            ...
            return builder._buildDefinition(selectedType, nameArr, templateVariables, bindings);
        }
    }
     ...
     */
    static async buildDefinitionBulk(listName, type, nameArr, bindings) {
        const properties = await config.getProperties();
        if (!properties) {
            return;
        }
