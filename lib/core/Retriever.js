...
class Retriever {
    ...
    async retrieve(metadataTypes, name, templateVariables) {
        for (const metadataType of Util.getMetadataHierachy(metadataTypes)) {
            ...
            MetadataRef.getType(type).cache = this.metadata;
            MetadataRef.getType(type).properties = this.properties;
            ...
            try {
                if (!metadataTypes.includes(type)) {
                    ...
                    await Util.retryOnError(
                        ...
                        async () => {
                            result = await MetadataRef.getType(type).retrieveForCache(
                                this.buObject,
                                subType
                            );
                            if (type == 'list') {
                    ...
                    await Util.retryOnError(`Retrying ${metadataType}`, async () => {
                        result = await Promise.all(
                            nameArr.map((name) => {
                                ...
                                return MetadataRef.getType(type).retrieveAsTemplate(
                                    this.templateDir,
                                    name,
                                    templateVariables,
                                    subType
                    ...
                    await Util.retryOnError(`Retrying ${metadataType}`, async () => {
                        result = await MetadataRef.getType(type).retrieve(
                            this.savePath,
                            null,
                            this.buObject,
                            subType
