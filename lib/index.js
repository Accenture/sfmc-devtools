...
class Mcdev {
    ...
    static async fixKeys(businessUnit, type, keys) {
        ...
        try {
            ...
            Util.OPTIONS.fromRetrieve = true;
            deployed = await Deployer.deploy(businessUnit, [type], keysForDeploy);
            const dependentTypes = await this.#getDependentMetadata(type);
            Util.logger.info(`Successfully updated keys of type ${type}`);
            Util.logger.warn(
     ...
     */
    static async #getDependentMetadata(fixedType) {
        const dependencies = new Set();

        for (const dependentType of Object.keys(MetadataTypeDefinitions)) {
            if (MetadataTypeDefinitions[dependentType].dependencies.includes(fixedType)) {
                // fixedType was found as a dependency of dependentType
                dependencies.add(dependentType);
            } else if (
                MetadataTypeDefinitions[dependentType].dependencies.some((dependency) =>
