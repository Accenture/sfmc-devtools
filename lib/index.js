...
const RetrieverLocal = require('./RetrieverLocal');
...
class Mcdev {
    ...
    */
     ...
     * @returns {Promise<Util.MultiMetadataTypeList>} -
     */
    static async buildTemplate(businessUnit, selectedType, name, market) {
        Util.logger.info('mcdev:: Build Template from local folder');
        properties = properties || File.loadConfigFile();
        // errors
        const [type, subType] = selectedType ? selectedType.split('-') : [];
        if (type && !MetadataTypeInfo[type]) {
        ...
        const buObject = await Cli.getCredentialObject(properties, businessUnit);
        if (buObject !== null) {
            const retrieverLocal = new RetrieverLocal(properties, buObject, null);
            if (this._checkMarket(market)) {
                return retrieverLocal.retrieve(retrieveTypesArr, name, properties.markets[market]);
            }
        }
    }
    /**
