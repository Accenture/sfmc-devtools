...
class Mcdev {
    ...
    static async retrieve(businessUnit, selectedTypesArr, keys, changelogOnly) {
        ...
        if (!(await config.checkProperties(properties))) {
            ...
            Util.stopTimer(startTime, 'retrieve');
            return null;
        }
    ...
    static async deploy(businessUnit, selectedTypesArr, keyArr, fromRetrieve = false) {
        const startTime = Util.startTimer();
        const response = Deployer.deploy(businessUnit, selectedTypesArr, keyArr, fromRetrieve);
        Util.stopTimer(startTime, 'deploy');
        return response;
    }
    ...
    static async retrieveAsTemplate(businessUnit, selectedType, name, market) {
        ...
        const properties = await config.getProperties();
        if (!(await config.checkProperties(properties))) {
            Util.stopTimer(startTime, 'retrieveAsTemplate');
            return null;
        }
        if (!Util._isValidType(selectedType)) {
            Util.stopTimer(startTime, 'retrieveAsTemplate');
        ...
        if (buObject !== null) {
            ...
            if (Util.checkMarket(market, properties)) {
                const response = retriever.retrieve(
                    retrieveTypesArr,
                    name,
                    properties.markets[market]
                );
                Util.stopTimer(startTime, 'retrieveAsTemplate');
                return response;
            }
    ...
    static async buildTemplate(businessUnit, selectedType, keyArr, market) {
        ...
        const response = Builder.buildTemplate(businessUnit, selectedType, keyArr, market);
        Util.stopTimer(startTime, 'buildTemplate');
        return response;
    }
    /**
    ...
    static async buildDefinition(businessUnit, selectedType, name, market) {
        ...
        const response = Builder.buildDefinition(businessUnit, selectedType, name, market);
        Util.stopTimer(startTime, 'buildDefinition');
        return response;
    }
    ...
    static async buildDefinitionBulk(listName, type, name) {
        ...
        const startTime = Util.startTimer();
        const response = Builder.buildDefinitionBulk(listName, type, name);
        Util.stopTimer(startTime, 'buildDefinitionBulk');
        return response;
    }
    /**
     *
