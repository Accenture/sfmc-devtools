...
class Mcdev {
     ...
     * @param {string} cred name of Credential
     * @param {string} bu name of BU
     * @param {TYPE.SupportedMetadataTypes[]|TYPE.TypeKeyCombo} [selectedTypesArr] limit to given metadata types
     * @param {string[]} keyArr customerkey of the metadata
     */
    static async start(businessUnit, selectedTypesArr, keys) {
        Util.logger.info('mcdev:: Start');
        const properties = await config.getProperties();
        let counter_credBu = 0;
        if (!(await config.checkProperties(properties))) {
            // return null here to avoid seeing 2 error messages for the same issue
        ...
        if (businessUnit === '*') {
            Util.logger.info(
                '\n               :: Starting the entity on all BUs for all credentials'
            );
            let counter_credTotal = 0;
            for (const cred in properties.credentials) {
                Util.logger.info(`\n               :: Starting the entity on all BUs for ${cred}`);

                for (const bu in properties.credentials[cred].businessUnits) {
                    console.log('*** ', bu);
                    await this._startBU(cred, bu, selectedTypesArr, keys);
                    counter_credBu++;
                    Util.restartLogger();
                }
                counter_credTotal += counter_credBu;
            ...
            if (bu === '*' && properties.credentials && properties.credentials[cred]) {
                Util.logger.info(`\n               :: Starting the entity on all BUs for ${cred}`);
                console.log('*** 2 Starting one entity on all bus');
                let counter_credBu = 0;
                for (const bu in properties.credentials[cred].businessUnits) {
                    console.log('*** not *, deploying per BU', bu);
                    console.log('*** selectedTypesArr', selectedTypesArr);
                    await this._startBU(cred, bu, selectedTypesArr, keys);
                    counter_credBu++;
                ...
                // start the entity on one BU only
                console.log('*** 1 starting one entity');
                console.log('*** selectedTypesArr', selectedTypesArr);
                await this._startBU(cred, bu, selectedTypesArr, keys);
                counter_credBu++;
                Util.logger.info(`:: Done\n`);
     ...
     * @param {string[]} keyArr customerkey of the metadata
     */
    static async _startBU(cred, bu, selectedTypesArr, keyArr) {
        const properties = await config.getProperties();
        const buObject = await Cli.getCredentialObject(
            properties,
            cred === null ? null : cred + '/' + bu,
        ...
        if (!keyArr || (Array.isArray(keyArr) && !keyArr.length)) {
            // no keys were provided, ensure we start all
            keyArr = [null];
        }
        if (!selectedTypesArr || (Array.isArray(selectedTypesArr) && !selectedTypesArr.length)) {
            // no types were provided, start queries only
            Util.logger.info(`\n               :: No type was provided, starting queries\n`);
            selectedTypesArr = ['query'];
        }
        ...
        Util.logger.info(`\n               :: Starting ${selectedTypesArr} on ${cred}/${bu}\n`);
        const startTypes = [];
        /* if (selectedTypesArr && Array.isArray(selectedTypesArr)) {
            console.log('*** selectedType: ', selectedTypesArr);
            for (const selectedType of selectedTypesArr) {
                console.log('*** selectedType: ', selectedType);
                //const type = selectedType.split('-')[0]; // what is this for?
                //console.log('*** type: ', type);
                this.metadata[type] = Util.filterObjByKeys(this.metadata[type], keyArr);
            }
        }*/
        try {
            console.log('**********Before start iterate types and set sdk');
            //more than one type was provided, iterate types and start items
            for (const type of selectedTypesArr) {
                try {
                    MetadataTypeInfo[type].client = auth.getSDK(buObject);
                } catch (ex) {
                ...
                await MetadataTypeInfo[type].start(keyArr);
            }
        } catch (ex) {
            Util.logger.errorStack(ex, 'mcdev.start ' + ex.message);
        }
    }
}
