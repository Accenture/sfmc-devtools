...
class Mcdev {
    ...
    static async _startBU(cred, bu, selectedTypesArr, keyArr) {
        ...
        console.log('bu: ', bu);
        console.log('buObject: ', buObject);
        if (buObject !== null) {
            cache.initCache(buObject);
            cred = buObject.credential;
            bu = buObject.businessUnit;
        }
        Util.logger.info(`\n               :: ${selectedTypesArr} on ${cred}/${bu}\n`);
        const startTypes = [];
        ...
        try {
            ...
            //for (const type of selectedTypesArr) {
            try {
                MetadataTypeInfo[selectedTypesArr].client = auth.getSDK(buObject);
            } catch (ex) {
                Util.logger.error(ex.message);
                return;
            }
            ...
            console.log('*** selectedTypesArr', selectedTypesArr);
            await MetadataTypeInfo[selectedTypesArr].start(keyArr);
        } catch (ex) {
