...
export default class Mcdev {
    ...
    static async migrate(businessUnit) {
        ...
        const assetRoot = File.normalizePath([
        ...
        Util.logger.info(
            `Asset v10 migration: ${report.moved.length} moved, ${report.conflicts.length} conflicts, ${report.skipped.length} skipped.`
        );
        for (const conflict of report.conflicts) {
            Util.logger.warn(`Asset v10 migration conflict: ${conflict}`);
        }
        if (report.unremovedEmptyDirs.length) {
            Util.logger.warn(
                `Asset v10 migration: the operating system did not allow deleting these now-empty folders; please remove them manually: ${report.unremovedEmptyDirs.join(', ')}`
            );
        }
        if (report.unremovedNonEmptyDirs.length) {
            ...
            Util.logger.warn(
                `Asset v10 migration: these folders still contain files and must NOT be deleted: ${remaining.join('; ')}`
            );
        }
        ...
        if (unmappedSummary) {
            Util.logger.warn(
                `Asset v10 migration: unmapped asset types still inside: ${unmappedSummary}`
            );
        }
        return report;
    }
