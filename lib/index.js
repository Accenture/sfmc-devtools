...
class Mcdev {
    ...
    static async fixKeys(businessUnit, type, keys) {
        ...
        if (type) {
            for (const selectedType of Array.isArray(type) ? type : Object.keys(type)) {
                if (!Util._isValidType(selectedType)) {
                    return;
                }
            }
    ...
    static async _fixKeysBU(cred, bu, type, keyArr) {
        ...
        try {
            ...
            await MetadataTypeInfo[type].fixKeys(properties);
            ...
            await this.#retrieveDependentMetadata(cred, bu, type[0]); //type is an array with one type only
        } catch (ex) {
    ...
    /**
     *helper for {@link _fixKeysBU}. Retrieve dependent metadata
     *
     * @private
     * @param {string} cred name of Credential
     * @param {string} bu name of BU
     ...
     */
    static async #retrieveDependentMetadata(cred, bu, type) {
        const dependencies = [];
        ...
        await this._retrieveBU(cred, bu, dependencies);
    }
}
