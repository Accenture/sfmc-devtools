...
export default class Mcdev {
    ...
    static async migrate(businessUnit) {
        const { migrate } = await import('./util/migrations/migrate.js');
        return migrate(businessUnit);
    }
    ...
    static async build(
        ...
        try {
            ...
            return bulk
                ? await this.buildDefinitionBulk(
                      marketDefinition[0],
                      typeKeyCombo,
                      null,
                      verifiedAssetInputs
            ...
            try {
                // Retry transient removal failures, including after a failed build.
                try {
                ...
                // Preserve the primary build error, but surface cleanup-only failures.
                if (!buildFailed) {
                    // eslint-disable-next-line no-unsafe-finally -- Only replace a successful result; the primary build error always wins.
                    throw ex;
                }
            } finally {
    ...
    static async buildTemplate(
        ...
        if (!verifiedAssetInputs && !Util.OPTIONS.dependencies) {
        ...
        for (const type of Object.keys(typeKeyList)) {
            ...
            if (
                !(verifiedAssetInputs && type === 'asset') &&
                keyArr.some((key) => key.startsWith('name:'))
            ) {
        ...
        // Verified selections cannot expand to inputs that were not preflighted.
        if (!verifiedAssetInputs) {
     ...
     * @returns {Promise.<MultiMetadataTypeList>} -
     */
    static async buildDefinition(businessUnit, selectedTypes, nameArr, marketArr, bindings) {
        this.#welcomeMessage();
     ...
     * @returns {Promise.<object>} -
     */
    static async buildDefinitionBulk(listName, selectedTypes, nameArr, bindings) {
        this.#welcomeMessage();
