...
class Mcdev {
    ...
    static async execute(businessUnit, selectedTypesArr, keys) {
        ...
        if (businessUnit === '*') {
            ...
            for (const cred in properties.credentials) {
                ...
                for (const bu in properties.credentials[cred].businessUnits) {
                    await this._executeBU(cred, bu, selectedTypesArr, keys);
                    counter_credBu++;
                    Util.restartLogger();
                }
                counter_credTotal += counter_credBu;
    ...
    static async _executeBU(cred, bu, selectedTypesArr, keyArr) {
        ...
        if (!keyArr || (Array.isArray(keyArr) && !keyArr.length)) {
            // no keys were provided, ensure we execute all
            keyArr = [null];
        }
        if (!selectedTypesArr || (Array.isArray(selectedTypesArr) && !selectedTypesArr.length)) {
            // no types were provided, execute queries only
            Util.logger.info(`\n               :: No type was provided, executing queries\n`);
        ...
        Util.logger.info(`\n               :: Executing ${selectedTypesArr} on ${cred}/${bu}\n`);
        try {
            ...
            for (const type of selectedTypesArr) {
                ...
                await MetadataTypeInfo[type].execute(keyArr);
            }
        } catch (ex) {
            Util.logger.errorStack(ex, 'mcdev.execute ' + ex.message);
        }
    }
}
