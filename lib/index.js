...
class Mcdev {
    ...
    static async addDependencies(businessUnit, selectedTypes) {
        ...
        if (
            Object.keys(notFoundList).reduce(
                (previousValue, type) => previousValue + notFoundList[type].length,
                0
            )
        ) {
            ...
            Util.logger.warn(
                `   mcdev retrieve ${businessUnit} ${Object.keys(notFoundList)
                    .reduce((previousValue, type) => {
                        previousValue.push(
                            ...notFoundList[type].map((key) => `-m "${type}:${key}"`)
                        );
        ...
        if (Object.keys(dependencies).length) {
            Util.logger.info(
                `Found ${Object.keys(dependencies).reduce((previousValue, type) => previousValue + dependencies[type].length, 0)} items across ${Object.keys(dependencies).length} types.`
            );
            for (const type in dependencies) {
    ...
    static async buildTemplate(businessUnit, selectedTypes, keyArr, market) {
        ...
        if (Util.OPTIONS.dependencies) {
            ...
            Util.logger.info(
                `   mcdev bd ${businessUnit} ${Object.keys(selectedTypes)
                    .reduce((previousValue, type) => {
                        previousValue.push(
                            ...selectedTypes[type].map((key) => `-m "${type}:${key}"`)
                        );
                        return previousValue;
                    }, [])
                    .join(' ')} --market ${market}`
