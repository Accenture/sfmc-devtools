...
class Mcdev {
    ...
    */
    ...
    static async retrieve(businessUnit, selectedType, changelogOnly) {
        ...
        if (businessUnit === '*') {
            ...
            if (
                ...
                const buObject = await Cli.getBusinessUnitObject(
                    properties,
    ...
    static async _deployBU(cred, bu, type) {
        ...
        const properties = File.loadConfigFile();
        const buObject = await Cli.getBusinessUnitObject(properties, buPath, null, true);
        if (buObject !== null) {
            cache.initCache(buObject);
    ...
    static async initProject(credentialsName, skipInteraction) {
        ...
        await Init.initProject(
            File.loadConfigFile(!!credentialsName),
            credentialsName,
            skipInteraction
        );
    }
    ...
    static async findBUs(credentialsName) {
        ...
        const properties = File.loadConfigFile();
        const buObject = await Cli.getBusinessUnitObject(properties, credentialsName, true);
        if (buObject !== null) {
    ...
    static async deleteByKey(businessUnit, type, customerKey) {
        ...
        const properties = File.loadConfigFile();
        const buObject = await Cli.getBusinessUnitObject(properties, businessUnit);
        if (buObject !== null) {
            ...
            try {
                MetadataTypeInfo[type].client = await auth.getSDK(buObject);
            } catch (ex) {
    ...
    static async retrieveAsTemplate(businessUnit, selectedType, name, market) {
        ...
        if (buObject !== null) {
            ...
            let client;
            try {
                client = await auth.getSDK(buObject);
            } catch (ex) {
                Util.logger.error(ex.message);
                return;
            }
