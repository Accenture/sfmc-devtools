...
class Mcdev {
     ...
     *
     * @param {string} businessUnit references credentials from properties.json
     * @param {TYPE.SupportedMetadataTypes} selectedType supported metadata type
     ...
     * @returns {Promise.<void>} -
     */
    static async convertClassic(businessUnit) {
        properties = properties || File.loadConfigFile();
        const buObject = await Cli.getCredentialObject(properties, businessUnit);

        const cm = new ClassicMigration(properties, buObject);
        cm.convert();
    }
}
