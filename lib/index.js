...
class Mcdev {
    ...
    static async fixKeys(businessUnit, type, keys) {
        ...
        // update keys on a BU
        if (await this._fixKeysBU(cred, bu, type, keys)) {
            counter_credBu++;
        } else {
            counter_failed++;
        }
    ...
    static async _fixKeysBU(cred, bu, type, keyArr) {
        ...
        try {
            ...
            Object.keys(retrieved).forEach((key) => {
                //console.log(Object.values(retrieved[key][0])[0]);
                //console.log(Object.values(retrieved[key][0]));
                for (const item of Object.values(retrieved[key][0])) {
                    ...
                    if (item != null) {
                        ...
                        Util.logger.error(`${item.key} not found on server.`);
                    }
                }
            });

            //console.log(Object.values(Object.values(retrieved)[0]));
            // console.log(Object.values(retrieved)[0]);
            // console.log(Array.isArray(Object.values(retrieved)[0]));
            // console.log(Object.values(retrieved));
            // console.log(Object.values(retrieved));
            // let i = 0;
            // for (const item of Object.keys(retrieved)) {
            //     console.log(Object.values(retrieved[key][0])[0])

            //     if (Object.values(item)[0] != null) {
            //         console.log('name: ', Object.values(item)[0].name);
            //         console.log('key: ', Object.values(item)[0].key);
            //         if (Object.values(item)[0].name != Object.values(item)[0].key) {
            //             console.log(Object.values(item)[0].key);
            //             // toDeploy.push(Object.values(item)[0].key);
            //             console.log('Object.values(item)[0].key', Object.values(item)[0].key);
            //             Util.logger.info(`Updating key for query ${Object.values(item)[0].key}`);
            //         } else {
            //             Util.logger.info(
            //                 ` ☇ skipping query ${
            ...
            //         }
            //     } else {
            //         Util.logger.error(`${Object.values(item)[0].key} not found on server.`);
            //     }
            // }
            ...
            console.log(MetadataTypeDefinitions[type].dependencies);
