...
class Mcdev {
    ...
    static async addDependencies(businessUnit, selectedTypes) {
        ...
        Util.logger.info('Searching for Dependencies');
        /** @type {TypeKeyCombo} */
        const dependencies = {};
        ...
        for (const type of initiallySelectedTypesArr) {
            MetadataTypeInfo[type].properties = properties;
            MetadataTypeInfo[type].buObject = buObject;
            await MetadataTypeInfo[type].getDependentFiles(selectedTypes[type], dependencies);
        }

        // add dependencies to selectedTypes
        if (Object.keys(dependencies).length) {
            Util.logger.info(
                `Found ${Object.keys(dependencies).reduce((previousValue, type) => previousValue + dependencies[type].length, 0)} dependencies across ${Object.keys(dependencies).length} types.`
            );
