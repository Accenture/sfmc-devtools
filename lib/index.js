...
class Mcdev {
     ...
     * @param {string} [key] limit retrieval to given metadata key
     * @param {boolean} [changelogOnly] skip saving, only create json in memory
     * @returns {Promise.<object>} -
     */
    static async retrieve(businessUnit, selectedTypesArr, key, changelogOnly) {
        ...
        if (businessUnit === '*') {
            ...
            for (const cred in properties.credentials) {
                ...
                for (const bu in properties.credentials[cred].businessUnits) {
                    await this._retrieveBU(cred, bu, selectedTypesArr, key);
                    counter_credBu++;
                    Util.restartLogger();
                }
     ...
     */
    static async _retrieveBU(cred, bu, selectedTypesArr, key, changelogOnly) {
        properties = properties || File.loadConfigFile();
        const buObject = await Cli.getCredentialObject(
            properties,
            cred !== null ? cred + '/' + bu : null,
            ...
            if (selectedTypesArr) {
                for (const selectedType of selectedTypesArr) {
                    ...
                    if (!key) {
                        ...
                        File.removeSync(File.normalizePath(removePathArr));
                    }
                }
            }
    ...
    static async deploy(businessUnit, selectedTypesArr, keyArr, fromRetrieve = false) {
        ...
        if (businessUnit === '*') {
            ...
            for (const buPath of deployFolders.filter((r) => r.includes(path.sep))) {
                ...
                await this._deployBU(cred, bu, selectedTypesArr, fromRetrieve);
                counter_credBu++;
                Util.logger.info('');
                Util.restartLogger();
            }
