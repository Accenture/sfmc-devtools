...
class Mcdev {
    ...
    static async addDependentCbReferences(businessUnit, selectedTypes) {
        ...
        if (assetDependencies.size) {
            const depType = 'asset';
            Util.logger.info(
                `Found ${assetDependencies.size} items linked via ContentBlockByX across ${Object.keys(selectedTypes).length} type${Object.keys(selectedTypes).length === 1 ? '' : 's'}: ` +
                    Util.getGrayMsg([...assetDependencies].join(','))
            );
        ...
        if (selectedTypes.asset?.length) {
            ...
            if (additionalAssetDependencies.length) {
                Util.logger.info(
                    `Found ${additionalAssetDependencies.length} items linked via ContentBlockByX in assets.`
                );
            }
