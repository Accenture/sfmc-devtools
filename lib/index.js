...
class Mcdev {
     ...
     * @returns {Promise.<boolean>} true if all items were executed, false otherwise
     */
    static async fixKeys(bu, selectedTypesArr, keyArr) {
        Util.startLogger();
        ...
        if (businessUnit === '*') {
            Util.logger.info('\n               :: Updating keys on all BUs for all credentials');
            let counter_credTotal = 0;
            for (const cred in properties.credentials) {
                Util.logger.info(`\n               :: Updating keys on all BUs for ${cred}`);

                for (const bu in properties.credentials[cred].businessUnits) {
                    if (await this._fixKeysBU(cred, bu, selectedTypesArr, keys)) {
                        counter_credBu++;
                    } else {
                        counter_failed++;
                    }
     ...
     * @returns {Promise.<boolean>} true if all items were executed, false otherwise
     */
    static async _fixKeysBU(cred, bu, selectedTypesArr, keyArr) {
        const properties = await config.getProperties();
        let counter_failed = 0;
        const buObject = await Cli.getCredentialObject(
            properties,
        ...
        Util.logger.info(
            `\n               :: Updating keys for ${selectedTypesArr.join(
                ', '
            )} on ${cred}/${bu}\n`
        );
        try {
            ...
            for (const type of selectedTypesArr) {
                ...
                // result will be undefined (false) if execute is not supported for the type
                if (!(await MetadataTypeInfo[type].changeKeyField(keyArr))) {
                    counter_failed++;
                }
            }
        } catch (ex) {
