...
import { migrateAssetV10Tree } from './util/migrations/assetV10.js';
...
export default class Mcdev {
     ...
     */
    static async migrate(businessUnit) {
        Util.startLogger();
        const properties = await config.getProperties();
        if (!properties) {
            return;
        ...
        const target = businessUnit ? businessUnit.split('/') : [null, null];
        const [cred, bu] = target;
        if (businessUnit && (target.length > 2 || target.some((part) => !part))) {
            Util.logger.error(
                `Invalid migration target '${businessUnit}': expected credential/BU or credential/*.`
            );
            return;
        }
        if (bu === '*' && properties.credentials?.[cred]) {
            // every BU of one credential; the BU list comes from the config, never from disk
            return await this.#migrateAllBus(properties, cred);
        }
        ...
        const assetRoot = this.#getMigrationRoot(properties, buObject);
        this.#logMigrationHeader();
        const report = await migrateAssetV10Tree(assetRoot);
        await this.#logMigrationReport(report);
        return report;
    }
     ...
     */
    static async #migrateAllBus(properties, cred) {
        this.#logMigrationHeader();
        /** @type {Object.<string, AssetV10MigrationReport>} */
        const reports = {};
        let conflictedBus = 0;
        for (const buName in properties.credentials[cred].businessUnits) {
            // resolve every BU through the same logic the single-BU command uses
            const buObject = await Cli.getCredentialObject(properties, `${cred}/${buName}`);
            if (!buObject) {
                continue;
            }
            const buPath = `${buObject.credential}/${buObject.businessUnit}`;
            try {
                Util.logger.info(`Migrating assets on ${buPath}:`);
                const report = await migrateAssetV10Tree(
                    this.#getMigrationRoot(properties, buObject)
                );
                reports[buPath] = report;
                if (report.conflicts.length) {
                    conflictedBus++;
                }
                await this.#logMigrationReport(report, buPath);
            } catch (ex) {
                // a broken BU must not stop the remaining BUs from being migrated
                Util.logger.error(`${buPath}: Asset v10 migration failed: ${ex.message}`);
            }
        }
        Util.logger.info(
            `Asset v10 migration: processed ${Object.keys(reports).length} BUs of ${cred}, ${conflictedBus} of them had conflicts.`
        );
        return reports;
    }
     ...
     */
    static #getMigrationRoot(properties, buObject) {
     ...
     * @returns {void} -
     */
    static #logMigrationHeader() {
        Util.logger.info('Migrating assets to their new folders ...');
        Util.logger.info(
            'This only reorganizes the local retrieve folder and makes no changes on the server.'
        );
        Util.logger.info('This may take a while ...');
    }
     ...
     */
    static async #logMigrationReport(report, buPath) {
        const prefix = buPath ? `${buPath}: ` : '';
        Util.logger.info(
            `${prefix}Asset v10 migration: ${report.moved.length} moved, ${report.conflicts.length} conflicts, ${report.skipped.length} skipped.`
        );
        for (const conflict of report.conflicts) {
            Util.logger.warn(`${prefix}Asset v10 migration conflict: ${conflict}`);
        }
        if (report.unremovedEmptyDirs.length) {
            Util.logger.warn(
                `${prefix}Asset v10 migration: the operating system did not allow deleting these now-empty folders; please remove them manually: ${report.unremovedEmptyDirs.join(', ')}`
            );
        }
        if (report.unremovedNonEmptyDirs.length) {
            const remaining = [];
            for (const dir of report.unremovedNonEmptyDirs) {
                let entries = [];
                try {
                    entries = await fs.readdir(dir);
                } catch {
                    // the folder vanished after the migration report was built
                }
                remaining.push(entries.length ? `${dir} (contains: ${entries.join(', ')})` : dir);
            }
            Util.logger.warn(
                `${prefix}Asset v10 migration: these folders still contain files and must NOT be deleted: ${remaining.join('; ')}`
            );
        }
        const unmappedSummary = Object.entries(report.unmapped)
            .map(([assetTypeName, owners]) => `${assetTypeName} (${owners.length})`)
            .join(', ');
        if (unmappedSummary) {
            Util.logger.warn(
                `${prefix}Asset v10 migration: unmapped asset types still inside: ${unmappedSummary}`
            );
        }
    }
        ...
        if (buObject !== null) {
            ...
            if (!retrieveTypesArr.length) {
                ...
                retrieveTypesArr.push(
                    ...new Set(
                        properties.metaDataTypes.retrieve.map((type) => type.split('-', 1)[0])
                    )
                );
    ...
    static async document(businessUnit, type) {
        ...
        try {
            ...
            const buObject = await Cli.getCredentialObject(
                properties,
                parentBUOnlyTypes.includes(type) ? businessUnit.split('/', 1)[0] : businessUnit,
    ...
    static async retrieveAsTemplate(businessUnit, selectedType, name, market) {
        ...
        const properties = await config.getProperties();
        if (!properties || !Util._isValidType(selectedType)) {
            return;
        }
        const { type, subType } = Util.getTypeAndSubType(selectedType);
    ...
    static async buildTemplate(businessUnit, selectedTypes, keyArr, marketArr, filter) {
        ...
        for (const type of Object.keys(typeKeyList)) {
            ...
            if (keyArr.every((key) => !key.startsWith('name:'))) {
                continue;
            }
                ...
                .map((key) => {
                    if (key.startsWith('name:')) {
                        ...
                        const foundKeysByName = builTemplateCache
                            .filter(
                                (item) => name == item[MetadataTypeInfo[type].definition.nameField]
                            )
                            .map((item) => item[MetadataTypeInfo[type].definition.keyField]);
                        if (foundKeysByName.length === 1) {
                            key = foundKeysByName[0];
                            Util.logger.debug(`- found ${type} key '${key}' for name '${name}'`);
                            return key;
    ...
    static async buildDefinition(businessUnit, selectedTypes, nameArr, marketArr) {
        ...
        const properties = await config.getProperties();
        if (!properties || !Util.checkMarketList(marketArr, properties)) {
            return;
        }
        const typeKeyList = Util.checkAndPrepareTypeKeyCombo(
            selectedTypes,
    ...
    static async schedule(businessUnit, selectedTypes, keys) {
        return this.#runMethod('schedule', businessUnit, selectedTypes, keys);
    }
    ...
    static async replaceCbReference(businessUnit, selectedTypesObj, to, fromList) {
        ...
        const properties = await config.getProperties();
        if (!properties || !Util.isValidBU(properties, businessUnit)) {
            return;
        }
