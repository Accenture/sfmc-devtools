            ...
            for (const sourceMlName of sourceMarketListArr) {
                ...
                for (const bu in buTypeDelta) {
                    for (const type in buTypeDelta[bu]) {
                        ...
                        console.log('keys1', Util.getMetadataMapToList(result[type]));

                        // ensure we have the right key for bd/bdb that matches the name used for rt
                        const keys = Util.getMetadataMapToList(result[type])
                            .map((item) => {
                                console.log(
                                    'MetadataTypeInfo[type].definition.keyField',
                                    MetadataTypeInfo[type].definition.keyField
                                );
                                console.log(
                                    'item[MetadataTypeInfo[type].definition.keyField]',
                                    item[MetadataTypeInfo[type].definition.keyField]
                                );
                                return item[MetadataTypeInfo[type].definition.keyField];
                            })
                            .filter((item) => !!item);
                ...
                if (deltaCounter) {
                    Util.logger.info(`- ✔️  ${deltaCounter} Templates created`);
                    console.log('typeDelta', typeDelta);
                } else {
                    Util.logger.error(
