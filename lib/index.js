...
class Mcdev {
    ...
    */
    static async createDeltaPkg(argv) {
        ...
        if (!(await Util.checkProperties(properties, argv.skipInteraction))) {
            return null;
        }
        ...
        if (argv.filter) {
            ...
            for (const sourceMlName of sourceMarketListArr) {
                ...
                if (!Object.keys(typeDelta).length) {
                    Util.logger.error(
    ...
    /**
     * @param {boolean} [skipInteraction] optionally dont show questions
     * @returns {Promise} .
     */
    static async selectTypes(skipInteraction) {
        properties = properties || File.loadConfigFile();
        if (!(await Util.checkProperties(properties, skipInteraction))) {
            return null;
        }
     ...
     */
    static async retrieve(businessUnit, selectedType, changelogOnly, skipInteraction) {
        ...
        if (businessUnit === '*') {
            ...
            for (const cred in properties.credentials) {
                ...
                for (const bu in properties.credentials[cred].businessUnits) {
                    await this._retrieveBU(cred, bu, selectedType, false, skipInteraction);
                    counter_credBu++;
                    Util.restartLogger();
                }
     ...
     */
    static async _retrieveBU(cred, bu, selectedType, changelogOnly, skipInteraction) {
        properties = properties || File.loadConfigFile();
        const buObject = await Cli.getCredentialObject(
            properties,
            cred !== null ? cred + '/' + bu : null,
        ...
        if (buObject !== null) {
            ...
            try {
                ...
                Util.logger.error('Error during auth: ' + ex.message);
                return;
            }
