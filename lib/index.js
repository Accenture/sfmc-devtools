...
class Mcdev {
    ...
    static async fixKeys(businessUnit, type, keys) {
        ...
        // update keys on a BU
        // fixKeyResult[]
        // if (fixKeyResult.push(await this._fixKeysBU(cred, bu, type, keys))) {
        //     counter_credBu++;
        // } else {
        //     counter_failed++;
        // }
        ...
        try {
            ...
            // TODO find types that are dependent on this type by iterating over THEIR dependencies
            const dependentTypes = await this.#getDependentMetadata(type);
            await retriever.retrieve(dependentTypes, null, null, false);
            Util.OPTIONS.changeKeyField = false;
            await Deployer._deployBU(cred, bu, properties, dependentTypes, null, true);
            // TODO update other types that are dependent on this type.
        } catch (ex) {
    ...
    /**
     * helper for {@link Mcdev.fixKeys}
     *
     * @param {string} cred name of Credential
     * @param {string} bu name of BU
     * @param {TYPE.SupportedMetadataTypes} type limit execution to given metadata type
     * @param {string[]} [keyArr] customerkey of the metadata
     * @returns {Array} fixedKeys of re-deployed items
     */
    ...
    // }
    /**
     *helper for {@link Mcdev._fixKeysBU}. Retrieve dependent metadata
     *
     * @private
     * @param {string} cred name of Credential
     * @param {string} bu name of BU
