...
class Mcdev {
     ...
     *
     * @param {'execute'|'pause'|'fixKeys'|'updateNotifications'|'clearNotifications'} methodName what to run
     * @param {string} businessUnit name of BU
     * @param {TYPE.SupportedMetadataTypes} [selectedType] limit to given metadata types
     * @param {string[]} [keys] customerkey of the metadata
    ...
    static async #runMethod(methodName, businessUnit, selectedType, keys) {
        ...
        switch (methodName) {
            ...
            case 'clearNotifications': {
                lang_past = 'cleared notifications for';
                lang_present = 'clearing notifications for';
                requireKeyOrLike = true;
                checkMetadataSupport = false;
                break;
            }
    ...
    static async updateNotifications(businessUnit, selectedType, keys) {
        ...
        if (Util.OPTIONS.clear) {
            ...
            for (const cred of Object.keys(result)) {
                if (result[cred].length > 0) {
                    Util.logger.info(`Retrieving ${selectedType} to have most recent changes`);
                    const retriever = new Retriever(this.properties, this.buObject);
                    await retriever.retrieve([selectedType], result[cred], null, false);
                }
            }
        } else if (
