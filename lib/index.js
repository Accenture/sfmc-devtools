...
class Mcdev {
    ...
    static async #replaceCbReference(cred, bu, type, keyArr) {
        ...
        try {
            ...
            const metadataMap = await MetadataTypeInfo[type].getJsonFromFS(
                File.normalizePath([deployer.retrieveDir, type])
            );
            ...
            const keysForDeploy = await MetadataTypeInfo[type].replaceCbReference(
                metadataMap,
                savePath
            );
