...
const ClassicMigration = require('./ClassicMigration');
...
yargs
    ...
    .command({
        command: 'convertClassic [BU]',
        desc:
            'Converts classic E-Mails & classic Content Areas into the new Content Builder format',
        builder: (yargs) => {
            yargs.positional('BU', {
                type: 'string',
                describe: 'the business unit to deploy to',
 ...
 * @param {String} businessUnit references credentials from .mcdevrc.json
 * @returns {Promise<void>} -
 */
async function convertClassic(businessUnit) {
    ...
    const cm = new ClassicMigration(properties, buObject, client);
    cm.convert();
}
/**
