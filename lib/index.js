...
class Mcdev {
    ...
    */
     ...
     * @param {boolean} [listOnly] skip saving, only create json in memory
     * @returns {Promise<Object>} -
     */
    static async retrieve(businessUnit, selectedType, listOnly) {
        ...
        if (businessUnit === '*') {
            ...
            if (bu === '*' && properties.credentials && properties.credentials[cred]) {
                ...
                const retrieveList = await this._retrieveBU(cred, bu, selectedType, listOnly);
                if (listOnly) {
                    return retrieveList;
                }
     ...
     */
    static async _retrieveBU(cred, bu, selectedType, listOnly) {
        properties = properties || File.loadConfigFile();
        const buObject = await Cli.getCredentialObject(
            properties,
            cred !== null ? cred + '/' + bu : null,
        ...
        if (buObject !== null) {
            ...
            try {
                ...
                const retrieveList = await retriever.retrieve(
                    retrieveTypesArr,
                    null,
                    null,
                    listOnly
