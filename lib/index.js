...
class Mcdev {
    ...
    static async buildTemplate(businessUnit, selectedTypes, keyArr, marketArr, filter) {
        ...
        if (includeKey?.length) {
            Util.logger.debug('include-key-filters: ' + includeKey.join(', '));
            for (const type of Object.keys(typeKeyList)) {
                typeKeyList[type] = typeKeyList[type].filter((key) => {
                    let included = false;
                    for (const element of includeKey) {
                        if (Util.stringLike(key, element)) {
                            included = true;
                            break;
                        }
                    ...
                    if (!included) {
                        Util.logger.info(
                            `🚫 ☇ filtered ${type} ${key}: missing include-filter-match`
                        );
                        return false;
                    }
                    return true;
