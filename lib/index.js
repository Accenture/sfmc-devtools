...
class Mcdev {
    ...
    static async replaceCbReference(businessUnit, selectedTypesObj, to, fromList) {
        ...
        const response = {};
        for (const selectedType of Array.isArray(fallbackTypesArr)
            ? fallbackTypesArr
            : Object.keys(selectedTypesObj)) {
            const temp = await this.#runMethod(
                ...
                businessUnit,
                selectedType,
                Array.isArray(fallbackTypesArr) ? null : selectedTypesObj[selectedType]
            );
