...
class Retriever {
    ...
    async retrieve(metadataTypes, namesOrKeys, templateVariables, changelogOnly) {
        ...
        if (Array.isArray(namesOrKeys)) {
            // assuming not TypeKeyCombo - transform it into one!
        ...
        for (const metadataType of Util.getMetadataHierachy(metadataTypes)) {
            ...
            // in case types were added that werent initially requested, make sure the key-list is set to [null]
                    ...
                    Util.logger.info(
                        'Retrieving: ' +
                            ...
                            (typeKeyMap[type][0] !== null
                                ? ` ${color.dim}(Keys: ${typeKeyMap[type].join(', ')})${
                                      color.reset
                                  }`
                                : '')
                    );
