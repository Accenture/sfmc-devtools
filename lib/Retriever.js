        ...
        for (const metadataType of Util.getMetadataHierachy(metadataTypes)) {
            ...
            try {
                ...
                if (!metadataTypes.includes(type) && !metadataTypes.includes(metadataType)) {
                    ...
                    result = await metadataInstance.retrieveForCache(subType);
                    ...
                    result = await Promise.all(
                        nameArr.map((name) => {
                            ...
                            return metadataInstance.retrieveAsTemplate(
                                this.templateDir,
                                name,
                                templateVariables,
                                subType
                    ...
                    if (changelogOnly) {
                        result = await metadataInstance.retrieveChangelog(null, subType);
                    } else {
                        result = await metadataInstance.retrieve(this.savePath, null, subType);
                    }
                }
                if (result) {
