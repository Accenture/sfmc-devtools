            ...
            try {
                ...
                if (
                    ...
                    if (
                        ...
                        // note: retrieveSharedForCache() returns a plain metadata map, not a {metadata: ...} wrapper
                        const cacheSharedResult =
                            await MetadataTypeInfo[type].retrieveSharedForCache();
                        cache.mergeMetadata(type, cacheSharedResult);
                    }
                    Util.logger.info(
                        `Retrieving: ${type}` +
