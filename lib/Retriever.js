...
class Retriever {
     ...
     * @param {string[]} [nameOrKey] name of Metadata to retrieveAsTemplate or list of keys for normal retrieval
     ...
     */
    async retrieve(metadataTypes, nameOrKey, templateVariables, changelogOnly) {
        /**
         * @type {TYPE.MultiMetadataTypeList}
         */
        const retrieveChangelog = {};
        console.log('typeof nameOrKey', typeof nameOrKey, nameOrKey);
        if (typeof nameOrKey === 'string') {
            Util.logger.verbose(
                'Deprecated way of retrieving metadata, please pass in nameOrKey as string[]'
            );
            nameOrKey = [nameOrKey];
        } else if (!nameOrKey || !nameOrKey.length) {
            // make iterating over nameOrKey easier
            nameOrKey = [null];
        }
