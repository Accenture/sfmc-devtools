    ...
    async installDependencies(repoName) {
        ...
        if (loadDependencies.length) {
            ...
            const args = ['install', '--save-dev'].concat(
                loadDependencies.map((name) =>
                    // check mcdev.dependencies first
                    Object.keys(Util.packageJsonMcdev.dependencies).includes(name)
                        ? `${name}@${Util.packageJsonMcdev.dependencies[name]}`
                        : // then check mcdev.devDependencies
                        Object.keys(Util.packageJsonMcdev.devDependencies).includes(name)
                        ? `${name}@${Util.packageJsonMcdev.devDependencies[name]}`
                        : // finally, fallback to using latest version if not found
                          `${name}@latest`
                )
            );
