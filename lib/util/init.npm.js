...
const Init = {
    ...
    async installDependencies(repoName, versionBeforeUpgrade) {
        ...
        // remove formatter dependencies retired by the 10.0.0 project boilerplate
        if (
            semver.lt(semver.valid(versionBeforeUpgrade) || '0.0.0', '10.0.0') &&
            projectPackageJson?.devDependencies?.['prettier-plugin-sql']
        ) {
            Util.logger.info('Removing obsolete formatter dependency: prettier-plugin-sql');
            const uninstallOutput = Util.execSync(
                'npm',
                ['uninstall', '--save-dev', 'prettier-plugin-sql'],
                true
            );
            if (uninstallOutput === null) {
                Util.logger.error('Could not remove obsolete formatter dependency.');
                return false;
            }
            projectPackageJson = await File.readJSON('package.json');
            Util.logger.info('✔️  Obsolete formatter dependency removed.');
        }
        ...
        if (loadDependencies.length) {
            ...
            const installOutput = Util.execSync('npm', args, true);
            if (installOutput === null) {
                Util.logger.error('Could not install/update dependencies.');
                return false;
            }
