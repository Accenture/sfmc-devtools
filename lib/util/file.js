...
const File = {
    ...
    writePrettyToFile: async function (directory, filename, filetype, content, templateVariables) {
        let formatted =
            filetype === 'amp'
                ? await this.beautify_beautyAmp(content)
     ...
     */
    beautify_beautyAmp: async function (content) {
        const ampscript = {
        ...
        try {
            ...
            return (await beautyAmp.beautify(content)).trim() + '\n';
        } catch (ex) {
    ...
    _beautify_prettier: async function (directory, filename, filetype, content) {
        ...
        try {
            if (!FileFs.prettierConfig) {
                ...
                return await this.beautify_beautyAmp(content);
            }
