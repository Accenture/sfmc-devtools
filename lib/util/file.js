...
class File {
     ...
     */
    static filterIllegalPathChars(path) {
        return (
     ...
     * @returns {string} - corrected string
     */
    static filterIllegalFilenames(filename) {
        return (
     ...
     * @returns {string} - corrected string
     */
    static reverseFilterIllegalFilenames(filename) {
     ...
     */
    static async writeJSONToFile(directory, filename, content) {
     ...
     */
    static async writePrettyToFile(directory, filename, filetype, content, templateVariables) {
     ...
     */
    static _beautify_prettier(directory, filename, filetype, content) {
     ...
     * @returns {Promise.<boolean>} Promise
     */
    static async writeToFile(directory, filename, filetype, content, encoding) {
        directory = this.filterIllegalPathChars(this.normalizePath(directory));
        await fs.ensureDir(directory);
     ...
     */
    static readJSONFile(directory, filename, sync, cleanPath) {
        try {
     ...
     */
    static readFilteredFilename(directory, filename, filetype, encoding) {
        try {
            directory = this.filterIllegalPathChars(this.normalizePath(directory));
            filename = this.filterIllegalFilenames(filename);
     ...
     */
    static async readDirectories(directory, depth, includeStem, _stemLength) {
        try {
            if (!_stemLength) {
     ...
     */
    static readDirectoriesSync(directory, depth, includeStem, _stemLength) {
        try {
            const children = [];
     ...
     */
    static async saveConfigFile(properties) {
     ...
     */
    static async initPrettier(filetype) {
