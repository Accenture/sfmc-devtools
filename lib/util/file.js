                ...
                for (const cred in properties.credentials) {
                    if (auth[cred]) {
                        if (properties.credentials[cred].eid !== auth[cred].account_id && !silent) {
                            const err = `'${cred}' found in ${Util.configFileName} (${properties.credentials[cred].eid}) and ${Util.authFileName} (${auth[cred].account_id}) have a Enterprise ID mismatch. Please check.`;
                            Util.logger.error(err);
                            throw new Error(err);
                        }
