...
import { isAssetRegroupingPair } from './migrations/assetV10.js';
 ...
 */
export async function filterAssetRegroupingChanges(files, range, gitClient = git) {
    const [baseRef, targetRef] = range.split(/\.\.\.?/, 2);
    const suppressed = new Set();
    const ownerPairs = [];
    for (const oldFile of files) {
        const oldPath = oldFile.moved ? oldFile.fromPath : oldFile.file;
        const newFile = oldFile.moved
            ? oldFile
            : files.find(
                  (candidate) =>
                      candidate !== oldFile && isAssetRegroupingPair(oldPath, candidate.file)
              );
        if (!newFile || !isAssetRegroupingPair(oldPath, newFile.file)) {
            continue;
        }

        if (oldPath.endsWith('.json')) {
            ownerPairs.push({ oldPath, newFile });
        }
        let oldBlob;
        let newBlob;
        try {
            [oldBlob, newBlob] = await Promise.all([
                gitClient.revparse([`${baseRef}:${oldPath}`]),
                gitClient.revparse([`${targetRef}:${newFile.file}`]),
            ]);
        } catch {
            // keep the current-layout change when committed blob equality cannot be established
        }
        if (oldBlob && oldBlob === newBlob) {
            suppressed.add(oldFile);
            suppressed.add(newFile);
        } else if (!oldFile.moved) {
            // avoid a false logical deletion while retaining the edited current-layout file
            suppressed.add(oldFile);
        }
    }
    // A removed extracted companion is a logical asset edit even if its owner moved unchanged.
    // Keep that current owner selectable by copy/template consumers, which ignore deletions.
    const remaining = files.filter((file) => !suppressed.has(file));
    for (const { oldPath, newFile } of ownerPairs) {
        const ownerDirectory = path.posix.dirname(oldPath);
        const ownerKey = path.posix.basename(oldPath).replace(/\.asset-[^/]+-meta\.json$/, '');
        const oldGroup = oldPath.match(/\.asset-([^/]+)-meta\.json$/)[1];
        const groupMarker = `/asset/${oldGroup}/`;
        const relativeOwner = oldPath.slice(oldPath.indexOf(groupMarker) + groupMarker.length);
        const nested = relativeOwner.startsWith(ownerKey + '/');
        if (
            remaining.some(
                (file) =>
                    file.file !== oldPath &&
                    (nested
                        ? file.file.startsWith(ownerDirectory + '/')
                        : file.file.startsWith(oldPath.slice(0, -4)))
            )
        ) {
            suppressed.delete(newFile);
        }
    }
    return files.filter((file) => !suppressed.has(file));
}
...
const DevOps = {
    ...
    async getDeltaList(properties, range, saveToDeployDir, filterPathsCSV, commitHistory) {
        ...
        const deltaSync = diffSummary.files
            ...
            .map((/** @type {DeltaPkgItem} */ file) => {
                ...
                if (file.moved) {
                    ...
                    file.fromPath = '-';
                    file.file = path.normalize(file.file).replaceAll('\\', '/');
                }

                // get metadata type from file name
                file.type = file.file.split('/', 4)[3];
        ...
        // Pair both layouts before path filtering, which may select only the current group.
        const deltaFiltered = (await filterAssetRegroupingChanges(deltaSync, range)).filter(
            (file) => filterPaths.some((filterPath) => file.file.startsWith(filterPath))
        );

        Util.logger.info(
            `Identified ${deltaFiltered.length} relevant file changes after filtering file types and folders. Processing related metadata now.`
        );
        ...
        extendedBar.start(deltaFiltered.length, 0);
        ...
        const delta = await Promise.all(
            deltaFiltered.map((/** @type {DeltaPkgItem} */ file) =>
                rateLimit(async () => {
                    ...
                    if (file.type === 'folder') {
                        ...
                        file.externalKey = file.file
                            ...
                            .slice(0, -2)
                            .join('.');
                        if (file.type === 'asset' && !file.externalKey) {
                            // retrieve/cred/bu/asset/message/emailkey/blocks/views.html.slots.[8maw2k7jnlb-5llcajmxaqa].asset-email-meta.html
                            file.externalKey = file.file.split('/', 6)[5];
                        }
                        file.name = null;
                    }
                    ...
                    file._credential = file.file.split('/', 2)[1];
                    file._businessUnit = file.file.split('/', 3)[2];
                ...
                for (const type in typeKeysMap) {
                    ...
                    if (additionalFiles?.length) {
                        delta.push(
                            ...additionalFiles
                                .map((filePath) => ({
                                    file: path.normalize(filePath).replaceAll('\\', '/'),
                                    type,
