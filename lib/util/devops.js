...
const DevOps = {
                ...
                rateLimit(async () => {
                    ...
                    if (file.type === 'folder') {
                        ...
                        if (file.type === 'asset' && !file.externalKey) {
                            ...
                            file.externalKey = file.file.split('/')[5];
                        }
                        file.name = null;
                    }
    ...
    async buildDeltaDefinitions(properties, range, diffArr, commitHistory) {
        ...
        for (const sourceMlName of sourceMarketListArr) {
            ...
            const filter = sourceMarketLists.filter;
            delete sourceMarketLists.filter;
            /** @type {string} */
            const sourceBU = Object.keys(sourceMarketLists)[0];
            /** @type {string} */
            const sourceMarket = Object.values(sourceMarketLists)[0];
