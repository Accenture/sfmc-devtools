...
const config = {
    ...
    async getProperties(silent, isInit) {
        if (config.properties) {
            return isInit || this.checkOperationalSelectors(config.properties)
                ? config.properties
                : undefined;
        }
            ...
            try {
                ...
                if (
                    ...
                    (!(await this.checkProperties(config.properties, silent)) ||
                        !this.checkOperationalSelectors(config.properties))
                ) {
                    return;
                }
            } catch (ex) {
     ...
     */
    checkOperationalSelectors(properties) {
        /** @type {[string, string[]][]} */
        const lists = Array.from(['retrieve', 'createDeltaPkg', 'documentOnRetrieve'], (field) => [
            `metaDataTypes.${field}`,
            properties.metaDataTypes?.[field] || [],
        ]);
        for (const field of ['include', 'exclude']) {
            lists.push([`options.${field}`, Object.keys(properties.options?.[field] || {})]);
            for (const [name, marketList] of Object.entries(properties.marketList || {})) {
                lists.push([
                    `marketList.${name}.filter.${field}.key`,
                    Object.keys(marketList.filter?.[field]?.key || {}).filter(
                        (type) => type !== '*'
                    ),
                ]);
            }
        }
        for (const [field, selectors] of lists) {
            for (const selector of selectors) {
                if (!Util._isValidType(selector)) {
                    Util.logger.error(
                        `Invalid selector '${selector}' in ${Util.configFileName}: ${field}`
                    );
                    return false;
                }
            }
    ...
    async getDefaultProperties() {
        ...
        defaultProperties.metaDataTypes.retrieve = Util.getTypeChoices('typeRetrieveByDefault');
        // set default retrieve values
        defaultProperties.metaDataTypes.createDeltaPkg = Util.getTypeChoices('typeCdpByDefault');
