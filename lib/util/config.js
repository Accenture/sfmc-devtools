...
class config {
     ...
     */
    static async getProperties(silent) {
        if (config.properties) {
            return config.properties;
        }
        if (await fs.pathExists(Util.configFileName)) {
            try {
                config.properties = await fs.readJson(Util.configFileName);
            } catch (ex) {
            ...
            if (await fs.pathExists(Util.authFileName)) {
                let auth;
                try {
                    auth = await fs.readJson(Util.authFileName);
                } catch (ex) {
                    Util.logger.error(`${ex.code}: ${ex.message}`);
                    return;
                }
     ...
     */
    static async checkProperties(properties, silent) {
        if (!(await fs.pathExists(Util.configFileName))) {
     ...
     */
    static async getDefaultProperties() {
        ...
        const defaultProperties = await fs.readJson(configFileName);
