...
class Init {
     ...
     */
    static async fixMcdevConfig(properties) {
        if (!properties) {
    ...
    static async createIdeConfigFiles(versionBeforeUpgrade) {
        ...
        if (!(await fs.pathExists(gitignoreFileName))) {
            Util.logger.debug(`Dependency file not found in ${gitignoreFileName}`);
            return false;
        } else {
            const fileContent = await fs.readFile(gitignoreFileName, 'utf8');
            creationLog.push(
        ...
        for (const subdir of directories) {
            ...
            for (const file of await fs.readdir(curDir)) {
                // read all files in these directories
                if (!(await fs.lstat(path.join(curDir, file))).isDirectory()) {
     ...
     * @returns {void}
     */
    static _updateLeaf(propertiersCur, defaultPropsCur, fieldName) {
     ...
     */
    static async _getForcedUpdateList(projectVersion) {
        ...
        if (await fs.pathExists(Util.configFileName)) {
            forceIdeConfigUpdate = fs.readJsonSync(
     ...
     */
    static async _createIdeConfigFile(fileNameArr, relevantForcedUpdates, boilerplateFileContent) {
        let update = false;
        ...
        boilerplateFileContent =
            boilerplateFileContent || (await fs.readFile(boilerplateFileName, 'utf8'));

        if (await fs.pathExists(fileName)) {
            const existingFileContent = await fs.readFile(fileName, 'utf8');
     ...
     * @returns {Promise.<boolean>} returns true if worked without errors
     */
    static async upgradeAuthFile() {
        if (await fs.pathExists(Util.authFileName)) {
            const existingAuth = await fs.readJSON(Util.authFileName);
