...
const Init = {
    ...
    async createIdeConfigFiles(versionBeforeUpgrade) {
        ...
        creationLog.push(await this._removeIdeConfigFiles(relevantForced));
    ...
    async _createIdeConfigFile(fileNameArr, relevantForced, boilerplateFileContent) {
        ...
        if (await File.pathExists(fileName)) {
            ...
            if (await File.pathExists(fileName + '.BAK')) {
                Util.logger.warn(
                    `- ❌  ${fileName} cannot be updated because ${fileName + '.BAK'} already exists`
                );
                return false;
            }
