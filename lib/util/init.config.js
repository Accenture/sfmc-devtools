...
const Init = {
    ...
    _getForcedUpdateList(projectVersion) {
        ...
        // !newest entry on top, sorted by version number!
        let forceIdeConfigUpdate;
        if (File.existsSync(Util.configFileName)) {
            try {
                forceIdeConfigUpdate = File.readJsonSync(Util.configFileName);
            } catch (ex) {
        ...
        for (let i = 0; i < forceIdeConfigUpdate.length; i++) {
            const element = forceIdeConfigUpdate[i];
            if (!projectVersion || semver.gt(element.version, projectVersion)) {
                relevantForcedUpdates.push(
                    // adapt it for local file systems
                    ...element.files.map((item) => '.' + path.sep + path.normalize(item))
                );
            } else {
                continue;
            }
        ...
        console.log('relevantForcedUpdates', relevantForcedUpdates);
    ...
    async _createIdeConfigFile(fileNameArr, relevantForcedUpdates, boilerplateFileContent) {
        ...
        if (File.existsSync(fileName)) {
            ...
            if (relevantForcedUpdates.includes(fileName)) {
                Util.logger.info(`- ⚠  ${fileName} found but an update is required.`);
            } else {
                Util.logger.info(
        ...
        if (saveStatus) {
            Util.logger.info(`- ✔️  ${fileName} ${update ? 'updated' : 'created'}`);
            return true;
        } else {
