...
const Init = {
    ...
    async fixMcdevConfig(properties) {
        ...
        for (const cred in properties.credentials) {
            ...
            updateConfigNeeded = true;
            upgradeMsgs.push(`- ✔️ updated Business Unit format`);
        }
     ...
     *
     * @param {string} properties config file's json
     * @returns {Promise<boolean>} status of config file creation
     */
    async createIdeConfigFiles(properties) {
        ...
        const relevantForcedUpdates =
            properties && properties.version ? this._getForcedUpdateList(properties.version) : [];
    ...
    async upgradeAuthFile() {
        if (await File.pathExists(Util.authFileName)) {
            ...
            if (existingAuth.credentials) {
                ...
                await File.writeJSON(Util.authFileName, newAuth, { spaces: '\t' });
