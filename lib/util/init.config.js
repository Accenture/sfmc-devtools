...
const Init = {
    ...
    async fixMcdevConfig(properties) {
        ...
        try {
            await config.checkProperties(properties, true);
        } catch (ex) {
            if (ex.missingFields?.length) {
                ex.missingFields.forEach((fieldName) => {
                    switch (fieldName) {
                        case 'marketList':
                        ...
                        case 'options.exclude':
                            if (properties.options.filter) {
                                upgradeMsgs.push(
                                    `- ✔️  converted 'options.filter' to '${fieldName}'`
                                );
    ...
    async createIdeConfigFiles(versionBeforeUpgrade) {
        ...
        const directories = await File.readDirectories(boilerPlateFilesPath, 10, false);
