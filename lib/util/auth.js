...
const Auth = {
    ...
    async saveCredential(authObject, credential) {
        ...
        if (test.error) {
            ...
            await File.writeJSONToFile('./', Util.authFileName.split('.json', 1)[0], existingAuth);
...
function setupSDK(sessionKey, authObject) {
    // sfmc-sdk authenticates this payload before access_token exists, despite its stricter type.
    const sdkAuthObject = /** @type {ConstructorParameters<typeof SDK>[0]} */ (
        /** @type {unknown} */ (authObject)
    );
    return new SDK(sdkAuthObject, {
        eventHandlers: {
