...
module.exports = {
    ...
    async saveCredential(authObject, credential) {
        ...
        try {
            ...
            if (test.error) {
                ...
                await File.writeJSONToFile('./', Util.authFileName, existingAuth);
    ...
    getSDK(buObject) {
        ...
        if (initializedSDKs[credentialKey]) {
            ...
            const credentialKey = newAuthObj.client_id + '|' + buObject.mid;
            const existingAuthObj = credentialStore.get(credentialKey);
            if (!existingAuthObj) {
