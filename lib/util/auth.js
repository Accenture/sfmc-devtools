...
class Auth {
     ...
     * @returns {void}
     */
    static async saveCredential(authObject, credential) {
        ...
        try {
            ...
            if (test.error) {
                ...
                const existingAuth = (await fs.pathExists(Util.authFileName))
                    ? await fs.readJson(Util.authFileName)
                    : {};
    ...
    static getSDK(buObject) {
        ...
        if (initializedSDKs[credentialKey]) {
            ...
            authfile = authfile || fs.readJsonSync(Util.authFileName);
