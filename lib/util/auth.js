...
exports.initSDK = async (account_id) => {
    ...
    if (Object.keys(businessUnits).length == 0) {
        // todo preparing ot move loadConfig to async
        const temp = await loadConfigFile();
        for (const cred in temp.credentials) {
            for (const mid in temp.credentials[cred].businessUnits) {
                businessUnits[temp.credentials[cred].businessUnits[mid]] =
                    temp.credentials[cred].eid;
            }
        }
    }
    const existing = sessionStore.get(account_id.toString());
    const credential = getCredential(businessUnits[account_id]);
    let config;
