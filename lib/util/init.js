...
const Init = {
    ...
    async upgradeProject(properties, initial, repoName) {
        ...
        // Complete every coupled approval before touching configuration, files, or npm.
        const filePlan = await InitConfig.preflightIdeConfigFiles(versionBeforeUpgrade);
        if (!filePlan) {
            return false;
        }
        const packagePlan = await InitNpm.preflightDependencies(repoName, versionBeforeUpgrade);
        if (!packagePlan) {
            return false;
        }
        if (!initial) {
            ...
            // ensure an existing config is up to current specs without completing the version gate yet
            status = await InitConfig.fixMcdevConfig(properties, versionBeforeUpgrade);
            if (!status) {
                return false;
            }
        ...
        status = await InitConfig.createIdeConfigFiles(versionBeforeUpgrade, filePlan);
        if (!status) {
            return false;
        }

        // install node dependencies
        status = await InitNpm.installDependencies(repoName, versionBeforeUpgrade, packagePlan);
        if (!status) {
            return false;
        }

        if (!initial && properties) {
            status = await File.saveConfigFile(properties);
            if (!status) {
                return false;
            }
        }
