...
class Init {
     ...
     */
    static async initProject(properties, credentialName, skipInteraction) {
        if (!properties) {
     ...
     * @returns {Promise.<void>} -
     */
    static async _downloadAllBUs(bu, gitStatus, skipInteraction) {
        let responses;
        if (!skipInteraction) {
            responses = await inquirer.prompt([
                {
     ...
     */
    static async upgradeProject(properties, initial, repoName) {
        let status;
     ...
     */
    static _getMissingCredentials(properties) {
