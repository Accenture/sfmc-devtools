            ...
            const projectFolder = await inquirer.prompt([
                {
                    type: 'confirm',
                    name: 'isCreate',
                    message: `Do you want to clone the repository into the subfolder ${repoName}? If you choose "no", the repository will be cloned into the current folder.`,
                    default: true,
                },
            ]);
            if (!projectFolder.isCreate) {
                // we likely created a logs folder by now which would block git clone
                await File.rm('./logs', { recursive: true, force: true });
            }
