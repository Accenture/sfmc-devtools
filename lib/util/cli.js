...
const Cli = {
     ...
     * @param {String} skipInteraction.tenant auth_url of installed package
        ...
        do {
            ...
            try {
                ...
                properties.credentials[inputData.credentialName] = {
                    eid: parseInt(inputData.account_id),
                    businessUnits: {},
                };
            } catch (ex) {
                Util.logger.error(ex.message);
    ...
    async _askCredentials(properties, credName) {
        ...
        responses.account_id = responses.account_id.trim();

        // extract the tenant sub-domain from the URI; the format was already validated above
        responses.auth_url = responses.tenant.match(tenantRegex)[1];
