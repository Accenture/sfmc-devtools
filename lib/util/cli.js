...
class Cli {
     ...
     */
    static async initMcdevConfig(skipInteraction) {
     ...
     */
    static async addExtraCredential(properties, skipInteraction) {
     ...
     */
    static async updateCredential(properties, credName, skipInteraction) {
        if (credName) {
            if (!skipInteraction) {
     ...
     */
    static async getCredentialObject(properties, target, isCredentialOnly, allowAll) {
        try {
            if (!(await config.checkProperties(properties))) {
                // return null here to avoid seeing 2 error messages for the same issue
                return null;
     ...
     */
    static async _selectBU(properties, credential, isCredentialOnly, allowAll) {
        const credList = [];
        const buList = [];
        const questions = [];
     ...
     */
    static async _setCredential(properties, credName, skipInteraction) {
        // Get user input
     ...
     */
    static async _askCredentials(properties, credName) {
        const questions = [];
        if (!credName) {
            questions.push({
                type: 'input',
     ...
     * @returns {Promise.<void>} -
     */
    static async selectTypes(properties, setTypesArr) {
        let responses;
        if (!setTypesArr) {
     ...
     * @returns {void}
     */
    static _summarizeSubtypes(responses, type) {
