...
const Cli = {
     ...
     * @param {String} skipInteraction.auth_url auth_url of installed package
     ...
     */
    async getBusinessUnitObject(properties, target, isCredentialOnly, allowAll) {
        try {
            if (!(await Util.checkProperties(properties))) {
                // return null here to avoid seeing 2 error messages for the same issue
                return null;
            ...
            return {
                ...
                isParent:
                    properties.credentials[credential].eid ==
                    properties.credentials[credential].businessUnits[businessUnit],
            };
        } catch (ex) {
     ...
     * @param {Object} [skipInteraction] signals what to insert automatically for things usually asked via wizard
     * @param {String} skipInteraction.client_id client id of installed package
     * @param {String} skipInteraction.client_secret client id of installed package
     * @param {String} skipInteraction.auth_url client id of installed package
     * @param {String} skipInteraction.credentialsName how you would like the credential to be named
     *
        ...
        do {
            if (skipInteraction) {
                if (
                    ...
                    // assume skipInteraction=={client_id,client_secret,auth_url,credentialsName}
                    inputData = skipInteraction;
                } else {
                    throw new Error(
                        '--skipInteraction flag found but missing required input for client_id,client_secret,auth_url,eid,credentialsName'
                    );
                }
            } else {
            ...
            try {
                console.log(inputData.account_id, parseInt(inputData.account_id));
                await auth.saveCredential(
                    {
                        client_id: inputData.client_id,
                        client_secret: inputData.client_secret,
                        auth_url: inputData.auth_url,
                        account_id: parseInt(inputData.account_id),
                    },
                    inputData.credentialsName
                );
                credentialsGood = true;

                // update central config now that the credentials are verified
                properties.credentials[inputData.credentialsName] = {
    ...
    async _askCredentials(properties, credName) {
        ...
        questions.push(
            ...
            },
            {
                type: 'number',
                name: 'account_id',
                message: 'MID of Parent Business Unit (EID)',
                validate: (value) => {
