...
export const Util = {
     ...
     *
     * @param {SupportedMetadataTypes} selectedType type or type-subtype
     ...
     *
     * @returns {SupportedMetadataTypes[]} type choices
     */
    ...
    getMetadataHierachy(metadataTypes) {
        ...
        for (const type of flatList) {
            if (type.includes('-')) {
                ...
                if (subTypeDeps[key]) {
                    ...
                    setList[key].add(...temp); // TODO - this might have only added the first item?
                }
            } else {
                setList[type] = null;
            }
        ...
        for (const type of Object.keys(setList)) {
            if (setList[type] instanceof Set) {
                finalList[type] = [...setList[type]];
            }
        }
