...
const Util = {
     ...
     *
     * @param {string} absolutePath path from root to the project
     * @returns {void} throws errors if problems were found
     */
    checkPathForCloud(absolutePath) {
        // popular cloud services and their respective default name for the absolute path
        const cloudServices = ['Dropbox', 'OneDrive', 'CloudDocs', 'Google Drive', 'Cloud'];
        for (const variable in cloudServices) {
            if (absolutePath.includes(cloudServices[variable])) {
                Util.logger.warn(
                    `the git repo is being saved in '${cloudServices[variable]}' and can reduce performance together with git`
                );
            }
        }
    },
