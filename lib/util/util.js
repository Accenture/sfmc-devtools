    ...
    checkProperties: async function (properties, silent) {
        ...
        if (silent) {
            ...
            if (errorMsgs.length) {
                ...
                Util.logger.errorFatal(errorMsgOutput.join('\n'));
                if (Util.skipInteraction) {
                    return false;
                }
                Util.logger.info(
...
function startLogger() {
    ...
    const temp = {
        ...
        errorStack: function (ex, message) {
            if (message) {
                Util.logger.error(message + ': ' + ex.message);
            }
            let stack;
            if (
        ...
        /**
         * errors that should cause surrounding applications to take notice of this errors should be written with this method
         * @param {string} msg - the message to log
         * @returns {void}
         */
        errorFatal: function (msg) {
            Util.signalFatalError();
            Util.logger.error(msg);
        },
    };
    Util.logger = Object.assign(Util.logger, temp);
