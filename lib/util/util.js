...
const Util = {
     ...
     * @param {MetadataTypeMap|MetadataTypeItem[]} metadata map or array of metadata
     * @returns {MetadataTypeItem[]} Array of metadata items
     */
    getMetadataMapToList(metadata) {
        if (metadata && Array.isArray(metadata)) {
            return metadata;
        } else {
            return Object.keys(metadata).map((item) => metadata[item]);
        }
    },
    /**
     * helper to turn an array or object into a single item
     * @param {Object.<{metadata:MetadataTypeMap,type:string}>} obj map or array of metadata
     * @returns {Object.<{metadata:MetadataTypeItem,type:string}>} item list reduced to single item
     */
    reduceMetadataMap(obj) {
        const keys = Object.keys(obj.metadata);
        if (keys.length > 1) {
            throw new Error('expected only one entry. found ' + keys.length);
        }
        obj.metadata = obj.metadata[keys[0]];
        return obj;
    },
    /**
     ...
     * @param {Array<MetadataTypeItem>} results array of metadata
