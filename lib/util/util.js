    ...
    async getETClient(buObject) {
        ...
        const sdk = new SDK(config, {
            eventHandlers: {
                ...
                onConnectionError: (ex, remainingAttempts) =>
                    Util.logger.info(
                        `Connection Error (Code: ${ex.code}). Retrying ${remainingAttempts} times`
                    ),
            },
            requestAttempts: 3,
        ...
        try {
            ...
            if (test.error) {
                ...
                // find missing rights
                const missingAccess = sdk.auth.supportedScope.filter(
                    (element) => !test.scope.includes(element)
                );
                const excessAccess = test.scope.filter(
