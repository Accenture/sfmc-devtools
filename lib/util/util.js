...
const Util = {
    ...
    checkProperties: async function (properties, silent) {
        ...
        if (
            properties.version &&
            semver.gt(properties.version, packageJsonMcdev.version)
        ) {
            Util.logger.error(
        ...
        if (
            !properties.version ||
            (![null, 'patch'].includes(semver.diff(packageJsonMcdev.version, properties.version)) && semver.gt(packageJsonMcdev.version, properties.version) )
        ) {
            errorMsgs.push(
