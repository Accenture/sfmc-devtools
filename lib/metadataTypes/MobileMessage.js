...
class MobileMessage extends MetadataType {
    ...
    static postRetrieveTasks(metadata) {
        // mobileCode
        try {
            metadata.r__mobileCode_code = cache.searchForField(
                'mobileCode',
                metadata.code.code,
                'code',
                'code'
        ...
        try {
            for (const attr of ['keyword', 'subscriptionKeyword', 'nextKeyword']) {
                if (metadata[attr]?.id) {
                    metadata[attr] = {
                        r__mobileKeyword_c__codeKeyword: cache.searchForField(
                            'mobileKeyword',
                            metadata[attr].id,
                            'id',
                            'c__codeKeyword'
            ...
            if (Array.isArray(metadata.campaigns)) {
                ...
                for (const campaign of metadata.campaigns) {
                    try {
                        ...
                        metadata.r__campaign_name.push(campaign.name);
                    } catch (ex) {
                        Util.logger.warn(
                            ` - ${this.definition.type} ${metadata[this.definition.nameField]}: ${
                                ex.message
    ...
    static async preDeployTasks(metadata, deployDir) {
        // mobileCode
        if (metadata.r__mobileCode_code) {
            metadata.code ||= {};
            const code = cache.getByKey('mobileCode', metadata.r__mobileCode_code);
            if (!code) {
                throw new Error(`mobileCode ${metadata.r__mobileCode_code} not found in cache`);
            }
            metadata.code = code;
            delete metadata.r__mobileCode_code;
        }

        // mobileKeyword
        for (const attr of ['keyword', 'subscriptionKeyword', 'nextKeyword']) {
            if (metadata[attr]?.r__mobileKeyword_c__codeKeyword) {
                const keywordObj = cache.getByKey(
                    'mobileKeyword',
                    metadata[attr].r__mobileKeyword_c__codeKeyword
                );
                if (!keywordObj) {
                    throw new Error(
                        `mobileKeyword ${metadata[attr].r__mobileKeyword_c__codeKeyword} not found in cache`
                    );
                }
        ...
        // campaign
        if (Array.isArray(metadata.r__campaign_name)) {
            metadata.campaigns = [];

            for (const campaignName of metadata.r__campaign_name) {
            ...
            delete metadata.r__campaign_name;
        }
