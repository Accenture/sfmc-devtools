...
class MobileMessage extends MetadataType {
    ...
    static async retrieve(retrieveDir, _, __, key) {
        ...
        try {
            ...
            // duplicate-name early-detection (kept faithful — no items are dropped).
            // names must be unique within a BU for cross-BU name-fallback deploys to be unambiguous.
    ...
    /**
     * helper for {@link MetadataType.createOrUpdate}: resolves a to-be-deployed mobileMessage to an
     * existing target-BU item by NAME when the id/key does not match (cross-BU deploy).
     * mobileMessage uses the server-generated id as its key, so the key differs per BU; matching by
        ...
        for (const name in byName) {
            if (byName[name].length > 1) {
                const details = byName[name]
                    .map(
                        (item) =>
                            `${item[this.definition.keyField]} (createdDate: ${item.createdDate || 'n/a'})`
                    )
                    .join(', ');
                Util.logger.warn(
