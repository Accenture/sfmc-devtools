...
class ImportFile extends MetadataType {
    ...
    static async retrieve(retrieveDir, _, __, key) {
        ...
        if (smsImportResults.totalResults > 0 && smsImportResults.entry.length > 0) {
            // we are only interested in imports from data extensions, not those from files on FTP (which have the filename in item.sourceFile)
            const smsImportEntries = smsImportResults.entry.filter((item) => !!item.sourceObjectId);
            if (smsImportEntries.length > 0) {
                Util.logger.info(
                    Util.getGrayMsg(
            ...
            const sourceObject = {};
            for (const item of smsImportEntries) {
