    ...
    static async retrieve(retrieveDir, _, __, key) {
        ...
        if (smsImportResults.totalResults > 0 && smsImportResults.entry.length > 0) {
            for (const item of smsImportResults.entry) {
                ...
                try {
                    const sourceObject = await this.client.rest.get(
                        '/legacy/v1/beta/object/' + item.sourceObjectId
                    );
                    item.sourceObjectKey = sourceObject.key;
                } catch {
                    Util.logger.error(
        ...
        try {
            await this.client.rest.get('/legacy/v1/beta/audience/');
        } catch {
            Util.logger.error(`endpoint /legacy/v1/beta/audience does not exist`);
        }
        ...
        switch (metadata.c__destinationType) {
            ...
            case 'SMS': {
                if (this.smsImports[metadata.name]) {
                    ...
                    Util.logger.debug(
                        ` - ImportFile ${metadata.customerKey}: Could not find code and keyword for this SMS import activity.`
                    );
                }
                delete metadata.destinationObjectId;
