...
class DomainVerification extends MetadataType {
    ...
    static postRetrieveTasks(metadataItem) {
        if (metadataItem.status !== 'Verified') {
            Util.logger.warn(
                ` - ${this.definition.type} ${metadataItem.domain} is not verified. Current status: ${metadataItem.status}`
            );
        }
        if (!metadataItem.isSendable) {
            Util.logger.warn(` - ${this.definition.type} ${metadataItem.domain} is not sendable.`);
        }
        return metadataItem;
    }
