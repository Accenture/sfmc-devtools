...
class TriggeredSendSummary extends MetadataType {
    ...
    static postRetrieveTasks(metadata) {
        try {
            ...
            if (!triggeredSendCategoryID) {
                Util.logger.verbose(
                    ` ☇ skipping ${this.definition.typeName} '${metadata.Name}'/'${metadata.CustomerKey}': Could not find related TriggeredSend.`
                );
                // do not save this TSD Summary because it would not be visible in the user interface
                return;
            }
            ...
            throw new Error(
                `Could not find TriggeredSend with CustomerKey '${metadata.CustomerKey}`
            );
        }
        return metadata;
    }
