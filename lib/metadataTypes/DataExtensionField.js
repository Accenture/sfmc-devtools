...
class DataExtensionField extends MetadataType {
     ...
     */
    static isSharedDe(customerKey) {
        if (this.buObject.eid === this.buObject.mid) {
            // already on parent BU, no shared DEs possible
            return false;
        }
        const categoryId = cache.getCache().dataExtension?.[customerKey]?.CategoryID;
        if (!categoryId) {
            return false;
        }
        try {
            const contentType = cache.searchForField(
                'folder',
                categoryId,
                'ID',
                'ContentType',
            ...
            return MetadataTypeDefinitions.folder.folderTypesFromParent.includes(contentType);
        } catch {
            // folder not found in parent BU cache → local DE
            return false;
        }
    }
     ...
     * @returns {Promise.<object>} Promise of items
     */
    static async retrieveFieldsForSingleSharedDe(customerKey) {
        if (this.buObject.eid === this.buObject.mid) {
            // already on parent BU, shared retrieval is not needed - all DEs are local here
            Util.logger.debug(
                `DataExtensionField.retrieveFieldsForSingleSharedDe: already on parent BU, skipping for ${customerKey}`
            );
            return {};
        }
        if (!this.properties.credentials[this.buObject.credential]) {
            Util.logger.error(
                `DataExtensionField.retrieveFieldsForSingleSharedDe: credential '${this.buObject.credential}' not found in properties`
            );
            return {};
        }
        ...
        const fields = await this.retrieveFieldsForSingleDe(customerKey);
     ...
     * @returns {Promise.<object>} Promise of items
     */
    static async retrieveFieldsForSingleDeAuto(customerKey) {
        return this.isSharedDe(customerKey)
            ? this.retrieveFieldsForSingleSharedDe(customerKey)
            : this.retrieveFieldsForSingleDe(customerKey);
    }
    ...
    static async prepareDeployColumnsOnUpdate(deployColumns, deKey) {
        ...
        // capture field names from deploy payload before the loop modifies them (for renamed fields, the original name is used)
        const deployColumnNames = new Set(
            deployColumns.map((item) => (item.Name + '').toLowerCase())
        );
        ...
        // warn about fields that exist on the server but are not in the deploy payload
        // these fields are NOT auto-deleted to prevent unintended data loss
        const removedFieldNames = Object.values(existingFieldByName)
            .filter((field) => !deployColumnNames.has((field.Name + '').toLowerCase()))
            .map((field) => field.Name)
            .toSorted();
        if (removedFieldNames.length > 0) {
            Util.logger.warn(
                ` - ${removedFieldNames.length} field(s) of ${deKey} found on the server but not in the deploy payload. Fields cannot be auto-deleted due to potential data loss. Please run the following command to delete them:\nmcdev delete ${this.buObject.credential}/${this.buObject.businessUnit} -m ` +
                    removedFieldNames
                        .map((name) => `dataExtensionField:"${deKey}.${name}"`)
                        .join(' ')
            );
        }
