...
class DataExtensionField extends MetadataType {
    ...
    static async deleteByKeySOAP(customerKey, fieldId) {
        ...
        for (const item of deletionQueue) {
            ...
            try {
                ...
                const errorMsg = ex.results?.length
                    ...
                    : ex.message;
                Util.logger.error(
                    `- error deleting ${this.definition.type} ${item.deKey}.${item.fieldObjectID}: ${errorMsg}`
                );
        ...
        if (uniqueDEKeys.length > 0) {
            Util.logger.info(
                Util.getGrayMsg(
                    `  To refresh your local files, run mcdev r ${this.buObject.credential}/${this.buObject.businessUnit} -m ${uniqueDEKeys.map((key) => 'dataExtension:' + key).join(' ')}`
                )
            );
        }
        return success;
