...
class DataExtensionField extends MetadataType {
    ...
    constructor(properties, buObject) {
        ...
        this.definition = require('../MetadataTypeDefinitions').dataExtensionField;
    }
    /**
     ...
     * @returns {Promise<{metadata:DataExtensionFieldMap,type:string}>} Promise of items
     */
    async retrieveForCache(requestParams, additionalFields) {
     ...
     */
    convertToSortedArray(fieldsObj) {
        return (
     ...
     */
    postRetrieveTasks(metadata, forDataExtension) {
     ...
     */
    _parseMetadata(metadata, forDataExtension) {
        if (forDataExtension) {
     ...
     */
    async prepareDeployColumnsOnUpdate(deployColumns, deKey) {
