...
class MetadataType {
        ...
        for (const metadataKey in metadataMap) {
            ...
            try {
                ...
                try {
                    ...
                    deployableMetadata = await this.validation_deploy(
                        deployableMetadata,
                        deployDir
                    );
                } catch (ex) {
    ...
    static async saveResults(results, retrieveDir, overrideType, templateVariables) {
        ...
        for (const originalKey in results) {
            ...
            try {
                ...
                results[originalKey] = await this.validation_retrieve(
                    results[originalKey],
                    retrieveDir
                );
    ...
    static async buildDefinition(templateDir, targetDir, templateName, variables) {
        ...
        try {
            await this.validation_buildDefinition(
                metadata,
            ...
            if (
                ...
                (metadata.r__folder_Path.startsWith('Shared Content/') ||
     ...
     * @returns {Promise.<MetadataTypeItem>} Promise of a single metadata item
     */
    static async validation_retrieve(item, retrieveDir) {
        if (!this.properties.options?.validation?.retrieve) {
            return item;
        }
        const rules = this.properties.options?.validation?.retrieve;

        // TODO allow specifiying ValidationLevel per type

        return this.validation(rules, item, retrieveDir);
    }
     ...
     * @returns {Promise.<MetadataTypeItem>} Promise of a single metadata item
     */
    static async validation_buildDefinition(item, deployDir) {
        if (!this.properties.options?.validation?.buildDefinition) {
            return item;
        }
        const rules = this.properties.options?.validation?.buildDefinition;

        // TODO allow specifiying ValidationLevel per type

        return this.validation(rules, item, deployDir);
    }
     ...
     * @returns {Promise.<MetadataTypeItem>} Promise of a single metadata item
     */
    static async validation_deploy(item, deployDir) {
        if (!this.properties.options?.validation?.deploy) {
            return item;
        }
        const rules = this.properties.options?.validation?.deploy;
     ...
     * @returns {Promise.<MetadataTypeItem>} Promise of a single metadata item
     */
    static async validation(validationConfig, item, deployDir) {
        const definition = this.definition;
        const validations = await validationsRules(this.definition, item, deployDir);
