...
class MetadataType {
    ...
    static async createOrUpdate(
        ...
        const keyMatchedByKey = cache.getByKey(this.definition.type, normalizedKey);
        let keyMatchedByName;
        if (Util.OPTIONS.matchName && this.definition.allowMatchingByName) {
            ...
            try {
                keyMatchedByName = cache.searchForField(
                    this.definition.type,
                ...
                // do nothing - searchForField will throw an error if it cannot find the name but here we will just test for a value in the corresponding field
            }
        }
        ...
        } else if (keyMatchedByKey || keyMatchedByName) {
            // normal way of processing update files
            const cachedVersion = keyMatchedByKey || keyMatchedByName;
