...
class MetadataType {
    ...
    static async buildTemplate(retrieveDir, templateDir, key, templateVariables) {
        ...
        metadataStr = this.stringifyFieldsBeforeTemplate(metadataStr);
     ...
     *
     * @param {string} metadataStr single metadata item as string
     * @returns {string} metadataStr with specified fields converted that need to be stringified before templating
     */
    static stringifyFieldsBeforeTemplate(metadataStr) {
        if (this.definition.stringifyFieldsBeforeTemplate) {
            ...
            for (let field of this.definition.stringifyFieldsBeforeTemplate) {
                if (field.startsWith('%.')) {
                    field = field.slice(2);
                    if (field.endsWith('[]')) {
                        // expect an array of values or objects that needs to be checked for numeric values & updated
                    } else {
                        // iterate ALL fields, trying to find one named
                    }
                } else {
                    // 'field' contains the full field path
                }
