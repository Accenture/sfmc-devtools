...
class MetadataType {
     ...
     */
    constructor(businessUnit, cache, properties, subType) {
    ...
    static async createREST(metadataEntry, uri) {
        ...
        try {
            ...
            await Util.retryOnError(
                ...
                async () => (response = await sdk().rest.post(uri, metadataEntry))
            );
    ...
    static async createSOAP(metadataEntry) {
        ...
        try {
            ...
            await Util.retryOnError(
                `Retrying ${this.definition.type}: ${metadataEntry[this.definition.nameField]}`,
                async () =>
                    (response = await sdk().soap.create(
    ...
    static async updateREST(metadataEntry, uri) {
        ...
        try {
            ...
            await Util.retryOnError(
                `Retrying ${this.definition.type}: ${metadataEntry[this.definition.nameField]}`,
                async () => (response = await sdk().rest.patch(uri, metadataEntry))
            );
            Util.logger.info(
    ...
    static async updateSOAP(metadataEntry) {
        ...
        try {
            ...
            await Util.retryOnError(
                `Retrying ${this.definition.type}: ${metadataEntry[this.definition.nameField]}`,
                async () =>
                    (response = await sdk().soap.update(
                        this.definition.type.charAt(0).toUpperCase() +
                            this.definition.type.slice(1),
                        metadataEntry,
                        null
