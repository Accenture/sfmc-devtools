...
class MetadataType {
     ...
     */
    static async getDependentFiles(keyArr, multiTypeKeyList = {}) {
        Util.logger.info(
            `Searching for ${this.definition.type} dependencies: ${keyArr.join(', ')}`
        );
        ...
        for (const filePath of filePaths) {
            ...
            multiTypeKeyList[this.definition.type].push(metadataItem[this.definition.keyField]);
            ...
            if (this.definition.dependencyGraph) {
                for (const depType of this.definition.dependencyGraph) {
                    ...
                    for (const fieldReference of this.definition.dependencyGraph[depType]) {
                        // ! this assumes we always have keys in this
                        dependentKeyArr.push(this.getNestedValue(metadataItem, fieldReference));
                    }
                    if (dependentKeyArr.length) {
                        // try to read the jsons for those dependant keys
                        await MetadataTypeInfo[depType].getDependentFiles(
                            dependentKeyArr,
                            multiTypeKeyList
                        );
                    }
