...
class MetadataType {
    ...
    static stringifyValue(value) {
        if (Array.isArray(value)) {
            for (let i = 0; i < value.length; i++) {
                value[i] = MetadataType.stringifyValue(value[i]);
            }
            return value;
        } else if ('object' === typeof value) {
            for (const key in value) {
                value[key] = MetadataType.stringifyValue(value[key]);
            }
            return value;
        }
     ...
     * @returns {void} .
     */
    static stringifyFieldByPath(target, fieldPathParts) {
        if (!target) {
            return;
        }
        ...
        if (currentPart === '%') {
            if (Array.isArray(target)) {
                for (const item of target) {
                    MetadataType.stringifyFieldByPath(item, rest);
                }
            } else if ('object' === typeof target) {
                for (const key in target) {
                    MetadataType.stringifyFieldByPath(target[key], rest);
                }
            }
            return;
        }
        ...
        if (rest.length) {
            MetadataType.stringifyFieldByPath(target[currentPart], rest);
        } else {
            if (target[currentPart]) {
                target[currentPart] = MetadataType.stringifyValue(target[currentPart]);
            }
        }
    }
    ...
    static async buildTemplate(retrieveDir, templateDir, key, templateVariables) {
        ...
        if (this.definition.stringifyFieldsBeforeTemplate) {
            ...
            for (const field of this.definition.stringifyFieldsBeforeTemplate) {
                MetadataType.stringifyFieldByPath(metadataTemp, field.split('.'));
            }
    ...
    static async retrieveSOAP(retrieveDir, requestParams, singleRetrieve, additionalFields) {
        ...
        if (retrieveDir) {
            ...
            if (Object.keys(savedMetadata).length > 0) {
                ...
                this.postDeleteTasks(singleRetrieve);
            }
        }
        return { metadata: metadata, type: this.definition.type };
    }
    ...
    static async retrieveREST(retrieveDir, uri, templateVariables, singleRetrieve) {
        const response =
            ...
            (this.definition.restPaginationForKey && singleRetrieve)
        ...
        if (retrieveDir) {
            ...
            if (Object.keys(savedMetadata).length > 0) {
                ...
                await this.postDeleteTasks(singleRetrieve);
            }
        }
    ...
    static async validation(method, originalItem, targetDir) {
        ...
        for (const rule of Object.keys(validationRules)) {
            ...
            const fixable = Object.prototype.hasOwnProperty.call(validationRules[rule], 'fix');
            if (!fixable && validationConfig[rule] === 'fix') {
                // reduce from fix to error if not fixable
                validationConfig[rule] = 'error';
            }
            const mode = Util.OPTIONS.fix && fixable ? 'fix' : validationConfig[rule];
            let passed;
            ...
            try {
                ...
                Util.logger.errorStack(
                    ex,
                    `Error while running validation rule ${rule} (check log file for details!)`
                );
                continue;
            }
            if (mode === 'fix') {
