...
class MetadataType {
    ...
    static async getDependentFiles(
        ...
        // Check alternative file paths for keys not yet found in the primary location.
        // This enables subtypes to look in additional directories, e.g. shared DEs in _ParentBU_.
        const keysNotYetFound = keyArr.filter(
            (key) => !multiTypeKeyList[this.definition.type]?.includes(key)
        );
        if (keysNotYetFound.length) {
            const altFilePaths = (await this.getAlternativeFilesToCommit(keysNotYetFound))
                .filter(Boolean)
                .filter((p) => p.endsWith('.json'));
            for (const filePath of altFilePaths) {
                let metadataItem;
                try {
                    metadataItem = await File.readJson(filePath);
                } catch {
