 ...
 * @property {Util.MetadataTypeItem} json metadata of one item w/o code
...
class MetadataType {
    ...
    constructor(properties, buObject) {
        ...
        this.client = auth.getSDK(buObject);
    }
    /**
     * Returns file contents mapped to their filename without '.json' ending
     ...
     */
    async deploy(metadata, deployDir, retrieveDir) {
     ...
     * @param {Util.MetadataTypeMap} metadata metadata mapped by their keyField
     * @param {Util.MetadataTypeMap} originalMetadata metadata to be updated (contains additioanl fields)
     * @returns {void}
     */
     ...
     * @param {Util.MetadataTypeItem} metadata a single item
     ...
     * @returns {Util.MetadataTypeItem} cloned metadata
     */
    postRetrieveTasks(metadata, targetDir, isTemplating) {
     ...
     * @returns {void}
     */
    overrideKeyWithName(metadata, warningMsg) {
        if (
     ...
     * @returns {Promise<{metadata:Util.MetadataTypeMap,type:string}>} metadata
     */
    retrieve(retrieveDir, additionalFields, subType) {
     ...
     * @returns {Promise<{metadata:Util.MetadataTypeMap,type:string}>} metadata
     */
    retrieveChangelog(additionalFields, subType) {
        return this.retrieve(null, additionalFields, subType);
    }
     ...
     * @returns {Promise<{metadata:Util.MetadataTypeMap,type:string}>} metadata
     */
    async retrieveForCache(subType) {
        return this.retrieve(null, null, subType);
    }
    /**
     * Gets metadata cache with limited fields and does not store value to disk
     ...
     * @returns {Promise<{metadata:Util.MetadataTypeMap,type:string}>} metadata
     */
    retrieveAsTemplate(templateDir, name, templateVariables, subType) {
     ...
     * @param {Util.MetadataTypeItem} metadata a single metadata item
     * @param {string} deployDir folder where files for deployment are stored
     * @returns {Promise<Util.MetadataTypeItem>} Promise of a single metadata item
     */
    async preDeployTasks(metadata, deployDir) {
        return metadata;
    }

    /**
     * Abstract create method that needs to be implemented in child metadata type
     * @param {Util.MetadataTypeItem} metadata single metadata entry
     * @param {string} deployDir directory where deploy metadata are saved
     * @returns {void}
     */
    create(metadata, deployDir) {
     ...
     * @param {Util.MetadataTypeItem} metadata single metadata entry
     * @param {Util.MetadataTypeItem} [metadataBefore] metadata mapped by their keyField
     * @returns {void}
     */
     ...
     * @param {Util.MetadataTypeMap} metadata metadata mapped by their keyField
     * @param {string} deployDir directory where deploy metadata are saved
     * @returns {Promise<Util.MetadataTypeMap>} keyField => metadata map
     */
    async upsert(metadata, deployDir) {
     ...
     * @param {Util.MetadataTypeItem} metadataEntry a single metadata Entry
     * @param {string} uri rest endpoint for POST
     * @returns {Promise} Promise
     */
    async createREST(metadataEntry, uri) {
     ...
     * @returns {Promise} Promise
     */
    async createSOAP(metadataEntry, overrideType, handleOutside) {
        try {
            this.removeNotCreateableFields(metadataEntry);
     ...
     * @returns {Promise} Promise
     */
    async updateREST(metadataEntry, uri) {
     ...
     * @returns {Promise} Promise
     */
    async updateSOAP(metadataEntry, overrideType, handleOutside) {
        try {
            this.removeNotUpdateableFields(metadataEntry);
     ...
     * @param {string[]} [additionalFields] Returns specified fields even if their retrieve definition is not set to true
     * @param {string} [overrideType] can be used if the API type differs from the otherwise used type identifier
     * @returns {Promise<{metadata:Util.MetadataTypeMap,type:string}>} Promise of item map
     */
    async retrieveSOAP(retrieveDir, requestParams, additionalFields, overrideType) {
        requestParams = requestParams || {};
        const fields = this.getFieldNamesToRetrieve(additionalFields);
        const response = await this.client.soap.retrieveBulk(
            overrideType || this.definition.type,
            fields,
            requestParams
        );
     ...
     * @returns {Promise<{metadata:Util.MetadataTypeMap,type:string}>} Promise of item map
     */
    async retrieveREST(retrieveDir, uri, overrideType, templateVariables) {
     ...
     * @param {Util.MetadataTypeItem} metadataEntry One entry of a metadataType
     ...
     * @returns {void}
     */
    deleteFieldByDefinition(metadataEntry, fieldPath, definitionProperty, origin) {
     ...
     * @param {Util.MetadataTypeItem} metadataEntry metadata entry
     * @returns {void}
     */
     ...
     * @returns {void}
     */
    removeNotUpdateableFields(metadataEntry) {
        for (const field in metadataEntry) {
     ...
     * @returns {void}
     */
    keepTemplateFields(metadataEntry) {
        for (const field in metadataEntry) {
     ...
     * @returns {void}
     */
    keepRetrieveFields(metadataEntry) {
        for (const field in metadataEntry) {
     ...
     */
    isFiltered(metadataEntry, include) {
        if (include) {
     ...
     * @memberof MetadataType
     */
    isFilteredFolder(metadataEntry, include) {
     ...
     */
    _filterFolder(myFilter, r__folder_Path) {
     ...
     * @returns {?boolean} true: filter value found; false: filter value not found; null: no filter defined
     */
    _filterOther(myFilter, metadataEntry) {
     ...
     * @param {Util.MetadataTypeMap} results metadata results from deploy
     * @param {string} retrieveDir directory where metadata should be stored after deploy/retrieve
     * @param {string} [overrideType] for use when there is a subtype (such as folder-queries)
     * @param {Util.TemplateMap} [templateVariables] variables to be replaced in the metadata
     * @returns {Promise<Util.MetadataTypeMap>} Promise of saved metadata
     */
    async saveResults(results, retrieveDir, overrideType, templateVariables) {
        const savedResults = {};
     ...
     * @param {Util.MetadataTypeItem} metadata main JSON file that was read from file system
     * @param {Util.TemplateMap} variables variables to be replaced in the metadata
     * @param {string} templateName name of the template to be built
     * @returns {Promise<void>} Promise
     */
    async buildDefinitionForExtracts(templateDir, targetDir, metadata, variables, templateName) {
     ...
     */
    findSubType(templateDir, templateName) {
        return null;
    }
    /**
     ...
     */
    async readSecondaryFolder(templateDir, typeDirArr, templateName, fileName, ex) {
     ...
     * @returns {Promise<{metadata:Util.MetadataTypeMap,type:string}>} Promise of item map
     */
    async buildDefinition(templateDir, targetDir, templateName, variables) {
    ...
    /**
     * Gets metadata cache with limited fields and does not store value to disk
     * @param {Util.MetadataTypeMap} [metadata] a list of type definitions
     * @param {boolean} [isDeploy] used to skip non-supported message during deploy
     * @returns {void}
     */
    document(metadata, isDeploy) {
        if (!isDeploy) {
     ...
     * @returns {void} -
     */
    async postDeleteTasks(customerKey) {
     ...
     */
    async deleteByKeySOAP(customerKey, handleOutside) {
        const keyObj = {};
     ...
     */
    readBUMetadataForType(readDir, listBadKeys, buMetadata) {
...
/**
 * @type {Util.MultiMetadataTypeMap}
 */
