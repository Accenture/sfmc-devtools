...
class MetadataType {
    ...
    static async createSOAP(metadataEntry, overrideType, handleOutside) {
        try {
            ...
            this._handleSOAPErrors(ex, metadataEntry, 'creating', handleOutside);
            return {};
        }
    }
    ...
    static async updateSOAP(metadataEntry, overrideType, handleOutside) {
        try {
            ...
            this._handleSOAPErrors(ex, metadataEntry, 'updating', handleOutside);
            return {};
        }
    }
    /**
     ...
     * @param {boolean} [handleOutside] if the API reponse is irregular this allows you to handle it outside of this generic method
     */
    static _handleSOAPErrors(ex, metadataEntry, msg, handleOutside) {
        if (handleOutside) {
            throw ex;
        } else {
            const errorMsg = ex?.json?.Results?.length
                ...
                : ex.message;
            Util.logger.error(
                ` ☇ error ${msg} ${this.definition.type} '${
                    metadataEntry[this.definition.keyField]
                }': ${errorMsg}`
            );
        }
