...
class MetadataType {
     ...
     */
    static async executeREST(uri) {
        try {
            const response = await this.client.rest.post(uri, {}); //payload is empty for this request
            if (response == 'OK') {
                Util.logger.info(`\n               :: Executed ${this.definition.type}`);
            } else {
                Util.logger.info(`${this.definition.type} was not executed`);
            }
            return response;
        } catch (ex) {
            Util.logger.errorStack(ex, ` - Starting execution of ${this.definition.type} failed`);
        }
    }
