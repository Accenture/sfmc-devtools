...
class MetadataType {
     ...
     *
     * @param {string} metadataTypeSOAP type of metadata to retrieve (not always the same as this.defintion.type)
    ...
    static async getObjectIdForSingleRetrieve(key) {
        ...
        const response = await this.client.soap.retrieve(
            this.definition.typeSoap || this.definition.type,
            ['ObjectID'],
            {
                filter: {
        ...
        const objectId = response.Results[0].ObjectID;
        console.log('objectId', objectId);

        // make sure we dont retrieve in bulk
        this.definition.restPagination = false;
     ...
     * @param {TYPE.TemplateMap} [templateVariables] variables to be replaced in the metadata
     * @param {boolean|string|number} [singleRetrieve] expect a flat object OR key of single item to filter by
    ...
    static parseResponseBody(body, singleRetrieve) {
        ...
        if (body !== null) {
            if (typeof singleRetrieve === 'boolean') {
                metadataStructure[body[keyField]] = body;
            ...
            if (typeof singleRetrieve === 'string' || typeof singleRetrieve === 'number') {
                // in case we really just wanted one entry, filter it here
                const single = {};
