...
class MetadataType {
    ...
    static create(metadata, deployDir) {
        Util.logger.error('☇ create is not supported yet for ' + this.definition.type);
        return;
    }
    ...
    static update(metadata, metadataBefore) {
        Util.logger.error('☇ update is not supported yet for ' + this.definition.type);
        return;
    }
    ...
    static async createREST(metadataEntry, uri) {
        ...
        try {
            ...
            parsedErrors.forEach((msg) => Util.logger.error('  • ' + msg));
            return null;
        }
    }
