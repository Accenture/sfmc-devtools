...
class MetadataType {
     ...
     */
    static async getDependentFiles(keyArr) {
        /** @type {MultiMetadataTypeList} */
        const multiTypeKeyList = {};
        ...
        for (const filePath of filePaths) {
            const metadata = await File.readJson(filePath);
            // store current key
            multiTypeKeyList[this.definition.type].push(metadata[this.definition.keyField]);

            // get dependent files
            if (this.definition.dependencyGraph) {
                for (const depType of this.definition.dependencyGraph) {
                    MetadataTypeInfo[depType].client = this.client;
                    MetadataTypeInfo[depType].properties = this.properties;
                    MetadataTypeInfo[depType].buObject = this.buObject;

                    const dependentKeyArr = [];
                    for (const reference of this.definition.dependencyGraph[depType]) {
                        // ! this only works for fields directy on metadata - NEED TO ENHANCE THAT FOR COMPLEX REFERENCE PATHS
                        dependentKeyArr.push(metadata[reference]);
                    }
                    if (dependentKeyArr.length) {
                        const dependentMultiTypeKeyList =
                            MetadataTypeInfo[depType].getDependentFiles(dependentKeyArr);
                        for (const subtype of dependentMultiTypeKeyList) {
                            multiTypeKeyList[subtype] ||= [];
                            multiTypeKeyList[subtype].push(...dependentMultiTypeKeyList[subtype]);
                        }
                    }
                }
            }
