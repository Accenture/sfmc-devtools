...
class MetadataType {
    ...
    static async buildTemplate(retrieveDir, templateDir, key, templateVariables) {
        ...
        try {
            ...
            Util.logger.info(
                ` - templated ${this.definition.type}: ${key} (${metadata[this.definition.nameField]
                })`
            );
    ...
    static hasChangedGeneric(cachedVersion, metadataItem, fieldName, silent) {
        ...
        for (const prop in clonedMetada) {
            ...
            if (
                ...
                if (clonedMetada[prop] != cachedVersion[prop]) {
                    Util.logger.debug(
                        `${this.definition.type}:: ${identifier
        ...
        if (!silent) {
            Util.logger.verbose(
                ` ☇ skipping ${this.definition.type} ${identifier} / ${clonedMetada[this.definition.nameField] || ''
                }: no change detected`
            );
        }
        return false;
            ...
            try {
                ...
                try {
                    ...
                    if (deployableMetadata) {
                        ...
                        Util.logger.errorStack(
                            ex,
                            ` ☇ skipping ${this.definition.type} ${deployableMetadata[this.definition.keyField]
    ...
    static async retrieveRESTcollection(urlArray, concurrentRequests = 10, logAmountOfUrls = true) {
        if (logAmountOfUrls) {
            Util.logger.debug(
                Util.getGrayMsg(
                    ` - ${urlArray?.length} ${this.definition.type}${urlArray?.length === 1 ? '' : 's'
    ...
    static async saveToDisk(results, originalKey, baseDir, subtypeExtension, templateVariables) {
        ...
        if (results[originalKey].json && results[originalKey].codeArr) {
            ...
            if (Util.OPTIONS.like && !Util.fieldsLike(results[originalKey])) {
                Util.logger.info(`Filtered ${originalKey} because of --like option`);
                return;
            }
        }
    ...
    static async buildDefinition(templateDir, targetDir, templateName, variables) {
        ...
        try {
            ...
            throw new Error(
                `${this.definition.type}:: Error applying template variables on ${templateName + '.' + this.definition.type
