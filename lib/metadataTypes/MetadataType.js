...
class MetadataType {
    ...
    /**
     * Used to start a query/automation etc.
     *
     * @param {string} uri REST endpoint where the POST request should be sent
     * @returns {string} OK or Error
     */
    ///interaction/v1/interactions/publishAsync/{{journeyId}}?versionNumber={{versionNumber}}
    static async startREST(uri) {
        try {
            const response = await this.client.rest.post(uri, {}); //payload is empty for this request
            if (response == 'OK') {
                Util.logger.info(`Started: ${this.definition.type} ${this.definition.nameField}`);
            } else {
                Util.logger.info(
                    `${this.definition.type} ${this.definition.nameField} was not started`
                );
            }
            return response;
        } catch (ex) {
            Util.logger.errorStack(ex, ` - Starting ${this.definition.type} failed`);
        }
    }
