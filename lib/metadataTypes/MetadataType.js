...
class MetadataType {
     ...
     * @param {MetadataTypeItem} metadata a single metadata item
     */
    static preDeploy_setFakeSoapDate(metadata) {
        if (!metadata.CreatedDate) {
            // set to fake date to make the api accept the field - if we send any value the API will return the real value
            metadata.CreatedDate = '1982-12-16T06:30:00';
        }
        if (!metadata.ModifiedDate) {
            // set to fake date to make the api accept the field - if we send any value the API will return the real value
            metadata.ModifiedDate = '1982-12-16T06:30:00';
        }
    }
}
