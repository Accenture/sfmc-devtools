...
class MetadataType {
     ...
     *
     * @returns {string[]} returns keys of items where notification email address/notes were removed
     */
    static async clearNotifications() {
        Util.logger.error(
            ` ☇ skipping ${this.definition.type}: clearNotifications is not supported yet for ${this.definition.type}`
        );
        return [];
    }
}
