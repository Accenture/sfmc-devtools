...
class MetadataType {
     ...
     *
     * @param {string} templateDir Directory where retrieved metadata directory will be saved
     * @param {string} keyName name of the metadata file
     * @param {object} templateVariables variables to be replaced in the metadata
     * @param {string} targetDir local path to retrieve the files
     * @param {string} [subType] optionally limit to a single subtype
     * @returns {Promise<{metadata:Util.MetadataTypeItem,type:string}>} Promise of metadata
     */
    static async buildTemplate(templateDir, keyName, templateVariables, targetDir, subType) {
        const suffix = subType ? `-${subType}-meta` : '-meta';
        const fileName = keyName + '.' + this.definition.type + suffix;
        let templatedMetadata;
        let originalKey;
        try {
            const metadata = File.readJSONFile(
                targetDir + '/' + this.definition.type,
                fileName,
                true,
                false
            );
            originalKey = metadata[this.definition.keyField];
            templatedMetadata = Util.replaceByObject(metadata, templateVariables);
            this.keepTemplateFields(templatedMetadata);
            ...
            Util.metadataLogger('error', this.definition.type, 'retrieve', ex, originalKey);
        }

        return { metadata: templatedMetadata, type: this.definition.type };
    }
     ...
     * @param {'template'|'definition'|'asset'} mode name of the template to be built
