...
class MetadataType {
    ...
    static async deleteByKey(buObject, customerKey) {
        if (this.definition.type !== 'this.definition.type') {
            Util.logger.warn(`Deletion is in beta for all but DataExtensions!`);
        }

        const deletedOK = await this.deleteByKeySOAP(buObject, customerKey, false);
    ...
    static async deleteByKeySOAP(buObject, customerKey, handleOutside) {
        try {
            this.client = await Util.getETClient(buObject);
        } catch (ex) {
