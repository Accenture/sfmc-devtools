...
class MetadataType {
    ...
    static async deleteByKey(buObject, customerKey) {
        let deletedOK;
        switch (this.definition.deletionApi) {
            case 'soap':
                deletedOK = await this.deleteByKeySOAP(buObject, customerKey, false);
                break;
            case 'rest':
                Util.logger.error(
                    `Deletion is not yet supported for ${this.definition.typeName} / RESTful API!`
                );
                break;
            default:
