...
class MetadataType {
    ...
    static async getDependentFiles(
        ...
        // console.log(this.definition.type, 'getDependentFiles', keyArr.join(','));
        if (['list'].includes(this.definition.type)) {
            Util.logger.debug(`skipping ${this.definition.type} as it is currently not supported`);
            return;
        }
            ...
            if (this.definition.dependencyGraph) {
                for (const depType in this.definition.dependencyGraph) {
                    ...
                    for (const fieldReference of this.definition.dependencyGraph[depType]) {
                        ...
                        const foundKeys = this.getNestedValue(metadataItem, fieldReference);
                        if (foundKeys) {
        ...
        if (notFound && notFound.length) {
            Util.logger.warn(
                Util.getGrayMsg(
                    `    ☇ skipping ${this.definition.type} dependenc${notFound.length === 1 ? 'y' : 'ies'} ${notFound.join(', ')}: Not found on your in your project folder. Try running 'mcdev retrieve . ${this.definition.type}'`
                )
            );
     ...
     */
    static getNestedValue(obj, nestedKey) {
        const nestedKeyParts = nestedKey.split('.');
        const result = this.getNestedValueHelper(obj, nestedKeyParts);
     ...
     */
    static getNestedValueHelper(obj, nestedKeyParts) {
        ...
        return Array.isArray(obj[key])
            ? obj[key].flatMap((x) => this.getNestedValueHelper(x, [...nestedKeyParts]))
            : this.getNestedValueHelper(obj[key], [...nestedKeyParts]);
    }
