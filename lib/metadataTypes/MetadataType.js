...
class MetadataType {
        ...
        if (
            ...
            if (hasError) {
                ...
                if (Util.OPTIONS.changeKeyField) {
                    if (
                        Util.OPTIONS.changeKeyField !== this.definition.keyField &&
                        metadata[metadataKey][Util.OPTIONS.changeKeyField]
                    ) {
                        const newKey = metadata[metadataKey][Util.OPTIONS.changeKeyField].trim();
                        if (metadataKey == newKey) {
                            ...
                            metadata[metadataKey][this.definition.keyField] = newKey;

                            // ensure we can delete the old file(s) after the successful update
                            Util.renamedMetadata[this.definition.type] ||= {};
                            Util.renamedMetadata[this.definition.type][newKey] = metadataKey;
                        }
                    ...
                    } else if (!metadata[metadataKey][Util.OPTIONS.changeKeyField]) {
                        Util.logger.warn(
                    ...
                    const newKey = Util.OPTIONS.changeKeyValue.trim();
                    if (metadataKey == newKey) {
                        Util.logger.warn(
    ...
    static async _postChangeKeyTasks(metadataEntry) {
        if (
            ...
            Util.renamedMetadata?.[this.definition.type]?.[metadataEntry[this.definition.keyField]]
        ) {
            // delete file(s) of old key
            await this.postDeleteTasks(
                Util.renamedMetadata?.[this.definition.type]?.[
                    metadataEntry[this.definition.keyField]
                ]
            );
            // clean entry from to-do list
            delete Util.renamedMetadata?.[this.definition.type]?.[
                metadataEntry[this.definition.keyField]
            ];
        }
    }
