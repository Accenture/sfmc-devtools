                ...
                case 'UPDATECONTACTDATA': {
                    ...
                    for (const contactField of contactFields) {
                        ...
                        if (
                            ...
                            !cache.getCache().dataExtensionField?.[contactField.field]
                        ) {
                            ...
                            const dataExtensionField = fields.find(
                                (f) => f.ObjectID === contactField.field
                            );
                            const cachedMetadata = {};
                            for (const field of fields) {
                                cachedMetadata[field.ObjectID] = field;
                            }
                            cache.setMetadata('dataExtensionField', cachedMetadata);
                            if (dataExtensionField) {
                                contactField.r__dataExtensionField_name = dataExtensionField.Name;
                                delete contactField.field;
                            } else {
                                Util.logger.warn(
                                    ` - ${this.definition.type} '${metadata[this.definition.nameField]}' (${
                                        metadata[this.definition.keyField]
                                    }) activity-key=${activity.key}: Dependent dataExtensionField with name ${contactField.r__dataExtensionField_name} was not found on your BU`
                                );
                            }
                        } else {
                            try {
            ...
            switch (activity.type) {
                ...
                case 'UPDATECONTACTDATA': {
                    ...
                    for (const contactField of contactFields) {
                        ...
                        if (
                            contactField.dataExtensionId &&
                            !cache.getCache().dataExtensionField?.[
                            ...
                            const dataExtensionField = fields.find(
                                (f) => f.Name === contactField.r__dataExtensionField_name
                            );
                            const cachedMetadata = {};
                            for (const field of fields) {
                                cachedMetadata[field.CustomerKey] = field;
                            }
                            cache.setMetadata('dataExtensionField', cachedMetadata);
                            if (dataExtensionField) {
                                contactField.field = dataExtensionField.ObjectID;
                                ...
                                Util.logger.warn(
                                    ` - ${this.definition.type} '${metadata[this.definition.nameField]}' (${
                                        metadata[this.definition.keyField]
                                    }) activity-key=${activity.key}: Dependent dataExtensionField with field name ${contactField.r__dataExtensionField_name} was not found on your BU`
                                );
                            }
                        } else {
                            try {
