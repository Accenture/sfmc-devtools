...
class Journey extends MetadataType {
        ...
        try {
            ...
            if (
                ...
                return await super.retrieveREST(
                    retrieveDir,
                    `${uri}${singleKey}?extras=${extras}${key && key.includes('/') ? '&versionNumber=' + key.split('/', 2)[1] : ''}`,
                    null,
                    key
                );
            } else {
    ...
    static async _postRetrieveTasks_activities(metadata) {
        ...
        for (const activity of metadata.activities) {
            switch (activity.type) {
                ...
                case 'SMSSYNC':
                case 'PUSHINBOXACTIVITY':
                case 'PUSHNOTIFICATIONACTIVITY':
                case 'INAPPSYNCACTIVITY': {
                    const configurationArguments = activity.configurationArguments;
                    if (configurationArguments) {
                        // early cross-BU dup-name detection: scan the SOURCE cache for the given
                        // type/id and warn (loud) when that item's name is shared by >1 id. This is
                        // early-detection for journey-only pipelines that never run the type-level
                        // retrieve; the deploy-time hard-error remains the enforcement point.
                        // searchForField returns only the first match, so we must scan to detect >1.
                        const warnOnSourceDupName = (type, id) => {
                            const typeCache = cache.getCache()?.[type];
                            if (!typeCache) {
                                return;
                            }
                            // find the source item's name by id
                            let sourceName;
                            for (const key in typeCache) {
                                if (typeCache[key]?.id === id) {
                                    sourceName = typeCache[key]?.name;
                                    break;
                                }
                            }
                            if (!sourceName) {
                            ...
                            // collect all source items sharing that name
                            const potentials = [];
                            for (const key in typeCache) {
                                if (typeCache[key]?.name === sourceName) {
                                    potentials.push(typeCache[key]);
                                }
                            }
                            if (potentials.length > 1) {
                                const details = potentials
                                    .map(
                                        (item) =>
                                            `${item.id} (createdDate: ${item.createdDate || 'n/a'})`
                                    )
                                    .join(', ');
                                Util.logger.warn(
                                    ` - ${type}: name "${sourceName}" is used by ${potentials.length} items: ${details}. Names must be unique within a BU for cross-BU deploys — clean these up.`
                                );
                            }
                        };
                        // mobilePush
                        try {
                            if (configurationArguments.pushMessageId) {
                                // faithful id-based ref (primary/unchanged)
                                configurationArguments.r__mobilePush_key = cache.searchForField(
                                    'mobilePush',
                                    configurationArguments.pushMessageId,
                                    'id',
                                    'id'
                                );
                                // additionally write the portable name ref for cross-BU deploys
                                configurationArguments.r__mobilePush_name = cache.searchForField(
                                    'mobilePush',
                                    configurationArguments.pushMessageId,
                                    'id',
                                    'name'
                                ...
                                delete configurationArguments.pushMessageId;
                            }
                            if (configurationArguments.application?.id) {
                                // faithful id-based ref (primary/unchanged)
                                configurationArguments.r__mobilePushApp_key = cache.searchForField(
                                    'mobilePushApp',
                                    configurationArguments.application.id,
                                    'id',
                                    'key'
                                );
                                // additionally write the portable name ref for cross-BU deploys
                                configurationArguments.r__mobilePushApp_name = cache.searchForField(
                                    'mobilePushApp',
                                    configurationArguments.application.id,
                                    'id',
                                    'name'
                                ...
                                delete configurationArguments.application;
                            }
                        } catch (ex) {
                            Util.logger.warn(
                                ` - ${this.definition.type} '${metadata[this.definition.nameField]}' (${
                        ...
                        try {
                            if (configurationArguments.messageId) {
                                ...
                                // additionally write the portable name ref for cross-BU deploys
                                configurationArguments.r__mobileMessage_name = cache.searchForField(
                                    'mobileMessage',
                                    configurationArguments.messageId,
                                    'id',
                                    'name'
                ...
                case 'UPDATECONTACTDATA': {
                    ...
                    for (const contactField of contactFields) {
                        ...
                        try {
                            ...
                            let alreadyFetchedDe = false;
                            if (!cache.getCache().dataExtensionField) {
                                // no dataExtensionField cache yet: retrieve fields for this specific DE
                                tempCachedFields = true;
                                alreadyFetchedDe = true;
                                const fields =
                                    await DataExtensionField.retrieveFieldsForSingleDeAuto(
                            ...
                            let fieldName;
                            try {
                                fieldName = cache.searchForField(
                                    'dataExtensionField',
                                    contactField.field,
                                    'ObjectID',
                                    'Name'
                                ...
                                // field not in existing cache - retrieve fields for this specific DE
                                const fields =
                                    await DataExtensionField.retrieveFieldsForSingleDeAuto(
                                        contactField.r__dataExtension_key
                                    );
                                if (Object.keys(fields).length > 0) {
                                    cache.mergeMetadata('dataExtensionField', fields);
                                }
                                // throws if still not found, handled by outer catch
                                fieldName = cache.searchForField(
                                    'dataExtensionField',
                                    contactField.field,
                                    'ObjectID',
                            ...
                            contactField.r__dataExtensionField_name = fieldName;
        ...
        for (const activity of metadata.activities) {
            switch (activity.type) {
                ...
                case 'INAPPSYNCACTIVITY': {
                    const configurationArguments = activity.configurationArguments;
                    if (configurationArguments) {
                        // REFERENCE resolution helper: key-first, then name-fallback, to a single
                        // target-BU cache item. This is a reference site — 0 matches means the
                        // referenced item is MISSING (never a create). searchForField returns only
                        // the first name match, so we scan to detect >1 (ambiguous → hard error).
                        const resolveMobileRef = (type, key, name) => {
                            // try the id-based key first (works for same-BU deploys)
                            let match = key ? cache.getByKey(type, key) : undefined;
                            // key miss (e.g. cross-BU): fall back to resolving by the portable name
                            if (!match && name) {
                                const typeCache = cache.getCache()?.[type];
                                const potentials = [];
                                for (const cacheKey in typeCache) {
                                    if (typeCache[cacheKey]?.name === name) {
                                        potentials.push(typeCache[cacheKey]);
                                    }
                                }
                                if (potentials.length > 1) {
                                    // ambiguous name → hard-error listing all matching ids
                                    throw new Error(
                                        `found multiple ${type} name matches in cache for name "${name}". Identified ids: ${potentials
                                            .map((p) => p.id)
                                            .join(
                                                ', '
                        ...
                        // mobilePush REFERENCE resolution: resolve to the target-BU id (key==id).
                        // delete-parity: retain r__mobilePush_key + r__mobilePush_name.
                        if (
                            configurationArguments.r__mobilePush_key ||
                            configurationArguments.r__mobilePush_name
                        ) {
                            const mobilePush = resolveMobileRef(
                                'mobilePush',
                                configurationArguments.r__mobilePush_key,
                                configurationArguments.r__mobilePush_name
                            );
                            // 0 matches on key AND name → referenced item is missing on target BU
                            if (!mobilePush) {
                                throw new Error(
                                    `mobilePush '${configurationArguments.r__mobilePush_name || configurationArguments.r__mobilePush_key}' not found on target BU — must be deployed first`
                                );
                            }
                            configurationArguments.pushMessageId = mobilePush.id;
                        }
                        // mobilePushApp REFERENCE resolution: resolve to the target-BU app object.
                        // shape parity: keep application = { id, name } (this site's existing shape).
                        // delete-parity: retain r__mobilePushApp_key + r__mobilePushApp_name.
                        if (
                            configurationArguments.r__mobilePushApp_key ||
                            configurationArguments.r__mobilePushApp_name
                        ) {
                            const mobilePushApp = resolveMobileRef(
                                'mobilePushApp',
                                configurationArguments.r__mobilePushApp_key,
                                configurationArguments.r__mobilePushApp_name
                            );
                            // 0 matches → mobilePushApp is retrieve-only/GUI-created; hard error
                            if (!mobilePushApp) {
                                throw new Error(
                                    `mobilePushApp '${configurationArguments.r__mobilePushApp_name || configurationArguments.r__mobilePushApp_key}' not found on target BU — must be created via GUI first`
                                );
                            }
                            configurationArguments.application = {
                                id: mobilePushApp.id,
                                name: mobilePushApp.name,
                            };
                        }

                        // mobileMessage REFERENCE resolution: resolve to the target-BU id (key==id).
                        // delete-parity: DELETE r__mobileMessage_key + r__mobileMessage_name.
                        if (
                            configurationArguments.r__mobileMessage_key ||
                            configurationArguments.r__mobileMessage_name
                        ) {
                            const mobileMessage = resolveMobileRef(
                                'mobileMessage',
                                configurationArguments.r__mobileMessage_key,
                                configurationArguments.r__mobileMessage_name
                            );
                            ...
                            if (!mobileMessage) {
                                throw new Error(
                                    `mobileMessage '${configurationArguments.r__mobileMessage_name || configurationArguments.r__mobileMessage_key}' not found on target BU — must be deployed first`
                                );
                            }
                            configurationArguments.messageId = mobileMessage.id;
                            delete configurationArguments.r__mobileMessage_key;
                            delete configurationArguments.r__mobileMessage_name;
                        }
                        // mobileKeyword
                ...
                case 'UPDATECONTACTDATA': {
                    ...
                    let tempCachedFields = false;
                    for (const contactField of contactFields) {
                        if (!contactField.r__dataExtension_key) {
                            continue;
                        }
                        ...
                        if (!contactField.r__dataExtensionField_name) {
                            continue;
                        }
                        DataExtensionField.buObject = this.buObject;
                        DataExtensionField.client = this.client;
                        ...
                        if (!cache.getCache().dataExtensionField) {
                            ...
                            tempCachedFields = true;
                            alreadyFetchedDe = true;
                            const fields = await DataExtensionField.retrieveFieldsForSingleDeAuto(
                                contactField.r__dataExtension_key
                            );
                            cache.setMetadata('dataExtensionField', fields);
                        }
                        let fieldObjectId;
                        try {
                            fieldObjectId = cache.searchForField(
                                'dataExtensionField',
                        ...
                        contactField.field = fieldObjectId;
    ...
    static async postDeployTasks(upsertResults) {
        ...
        // override the response from update by createdDate from cache (the date from update-response is incorrect)
        for (const [key, journey] of Object.entries(upsertResults)) {
            const cachedJourney = cache.getByKey(this.definition.type, key);
            if (cachedJourney) {
                journey.createdDate = cachedJourney.createdDate;
            }
        }
    ...
    static _showPublishStatusDetails(response) {
        ...
        for (const type in types) {
            ...
            for (const msg of response[type]) {
                messages[type].push(
                    `     #${counter++}: ${msg.errorDetail.replaceAll(' EmailID: ', '\n       EmailID: ').replaceAll(' Personalization error: ', '\n       Personalization error: ')}`,
