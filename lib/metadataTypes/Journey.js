                    ...
                    case 'email': {
                        if (
                            ...
                            try {
                                ...
                                activity.configurationArguments.r__transactionalEmail_definitionKey =
                                    activity.configurationArguments.triggeredSendKey;
                                ...
                                if (activity.metaData?.highThroughput?.dataExtensionId) {
                                    activity.metaData.highThroughput.r__dataExtension_CustomerKey =
            ...
            switch (activity.type) {
                case 'EMAILV2': {
                    ...
                    if (configurationArguments) {
                        try {
                            if (configurationArguments.triggeredSendId) {
                                ...
                                configurationArguments.r__triggeredSend_CustomerKey =
                    ...
                    if (triggeredSend) {
                        ...
                        triggeredSend.r__dataExtension_CustomerKey = {};
                        if (
                            ...
                            let errors = 0;
                            triggeredSend.r__dataExtension_CustomerKey.domainExclusions =
                        ...
                        if (!Object.keys(triggeredSend.r__dataExtension_CustomerKey).length) {
                            // in case we found no linked dataExtensions, remove this empty shell
                            delete triggeredSend.r__dataExtension_CustomerKey;
                        }

                        // sender profile
                        if (triggeredSend.senderProfileId) {
                            try {
                                triggeredSend.r__senderProfile_CustomerKey = cache.searchForField(
                                    'senderProfile',
                        ...
                        if (triggeredSend.sendClassificationId) {
                            try {
                                triggeredSend.r__sendClassification_CustomerKey =
                                    cache.searchForField(
                                        'sendClassification',
                        ...
                        try {
                            ...
                            triggeredSend.r__asset_customerKey = cache.searchForField(
                                'asset',
                                triggeredSend.emailId,
                                'legacyData.legacyId',
                                'customerKey'
                    ...
                    if (configurationArguments) {
                        ...
                        try {
                            if (configurationArguments.messageId) {
                                configurationArguments.r__mobileMessage_id = cache.searchForField(
                                    'mobileMessage',
                        ...
                        // mobileKeyword
                        configurationArguments.r__mobileKeyword_c__codeKeyword = {};
                        try {
                            if (configurationArguments.keywordId) {
                                configurationArguments.r__mobileKeyword_c__codeKeyword.current =
                                    cache.searchForField(
                                        'mobileKeyword',
                        ...
                        try {
                            if (configurationArguments.nextKeywordId) {
                                configurationArguments.r__mobileKeyword_c__codeKeyword.next =
                                    cache.searchForField(
                                        'mobileKeyword',
                        ...
                        if (
                            !Object.keys(configurationArguments.r__mobileKeyword_c__codeKeyword)
                                .length
                        ) {
                            // in case we found no linked dataExtensions, remove this empty shell
                            delete configurationArguments.r__mobileKeyword_c__codeKeyword;
                        }
                        // mobileCode
                        try {
                            if (configurationArguments.codeId) {
                                configurationArguments.r__mobileCode_code = cache.searchForField(
                                    'mobileCode',
                        ...
                        try {
                            ...
                            if (configurationArguments.assetId) {
                                ...
                                configurationArguments.r__asset_customerKey = cache.searchForField(
                                    'asset',
                                    configurationArguments.assetId,
                                    'id',
                                    'customerKey'
        ...
        switch (metadata.definitionType) {
            ...
            case 'Transactional': {
                ...
                switch (metadata.channel) {
                    case 'email': {
                        const activity = metadata.activities[0];
                        if (activity.configurationArguments?.r__transactionalEmail_definitionKey) {
                            // trigger found; there can only be one entry in this array
                        ...
                        if (activity.metaData?.highThroughput?.r__dataExtension_CustomerKey) {
                            ...
                            delete activity.metaData.highThroughput.r__dataExtension_CustomerKey;
                        }
            ...
            switch (activity.type) {
                case 'EMAILV2': {
                    ...
                    if (
                        ...
                        configurationArguments.triggeredSendKey =
                            configurationArguments.r__triggeredSend_CustomerKey;
                        delete configurationArguments.r__triggeredSend_CustomerKey;
                    }
                    const triggeredSend = configurationArguments?.triggeredSend;
                    if (triggeredSend) {
                        // List (optional)
                        ...
                        // DataExtension (optional)
                        if (triggeredSend.r__dataExtension_CustomerKey?.length) {
                            triggeredSend.domainExclusions =
                                triggeredSend.r__dataExtension_CustomerKey.map((key) => {
                        ...
                        // sender profile
                        if (triggeredSend.r__senderProfile_CustomerKey) {
                            triggeredSend.senderProfileId = cache.searchForField(
                                'senderProfile',
                                triggeredSend.r__senderProfile_CustomerKey,
                                'CustomerKey',
                                'ObjectID'
                            );
                            delete triggeredSend.r__senderProfile_CustomerKey;
                        }
                        // send classification
                        if (triggeredSend.r__sendClassification_CustomerKey) {
                            ...
                            delete triggeredSend.r__sendClassification_CustomerKey;
                        }
                        // message priority
                        ...
                        // email
                        if (triggeredSend.r__asset_customerKey) {
                            triggeredSend.emailId = cache.searchForField(
                                'asset',
                                triggeredSend.r__asset_customerKey,
                                'customerKey',
                                'legacyData.legacyId'
                            );
                            delete triggeredSend.r__asset_customerKey;
                ...
                case 'SMSSYNC': {
                    ...
                    if (configurationArguments) {
                        // mobileMessage
                        if (configurationArguments.r__mobileMessage_id) {
                            configurationArguments.messageId = cache.searchForField(
                                'mobileMessage',
                                configurationArguments.r__mobileMessage_id,
                                'id',
                                'id'
                            );
                            delete configurationArguments.r__mobileMessage_id;
                        }
                        // mobileKeyword
                        if (configurationArguments.r__mobileKeyword_c__codeKeyword?.current) {
                        ...
                        if (configurationArguments.r__mobileKeyword_c__codeKeyword?.next) {
                        ...
                        delete configurationArguments.r__mobileKeyword_c__codeKeyword;
                        // mobileCode
                        if (configurationArguments.r__mobileCode_code) {
                            configurationArguments.codeId = cache.searchForField(
                                'mobileCode',
                                configurationArguments.r__mobileCode_code,
                                'code',
                                'id'
                            );
                            delete configurationArguments.r__mobileCode_code;
                        }
                        // asset-asset: jsonmessage
                        if (configurationArguments.r__asset_customerKey) {
                            configurationArguments.assetId = cache.searchForField(
                                'asset',
                                configurationArguments.r__asset_customerKey,
                                'customerKey',
                                'id'
                            );
                            delete configurationArguments.r__asset_customerKey;
