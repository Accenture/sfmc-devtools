...
class Journey extends MetadataType {
    ...
    static async retrieve(retrieveDir, _, __, key) {
        ...
        const onlyPublished = Util.OPTIONS.onlyPublished;
        ...
        try {
            ...
            if (
                (!onlyPublished && singleKey && (mode === 'key' || mode === 'id')) ||
                !retrieveDir
            ) {
                ...
                const url = `${uri}?${additionalParams.toString()}`;
                // retrieve all
                ...
                if (retrieveDir) {
                    ...
                    if (mode === 'key') {
                        journey = results.items.find(
                            (/** @type {object} */ a) =>
                                a[this.definition.keyField] === singleKey.slice(4)
                        );
                    } else if (mode === 'id') {
                        journey = results.items.find(
                            (/** @type {object} */ a) => a[this.definition.idField] === singleKey
                        );
                    } else {
