    ...
    static async publish(keyArr, upsertResults) {
        ...
        if (executedKeyArr.length) {
            ...
            try {
                ...
                if (updatedJourneys) {
                    // regardless of upsert vs publish-only mode, we need to retrieve the events and their dependencies
                    const updatedEvents = [];
                    ...
                    if (updatedEvents.filter(Boolean).length) {
                        Util.logger.info('Retrieving relevant events');
                        await retriever.retrieve(['event'], updatedEvents.filter(Boolean));
                    } else {
        ...
        if (resultsTransactional.length) {
            ...
            if (transactionalKeyArr.length) {
                ...
                Util.logger.info('Re-retrieving published transactional journeys');
                const retriever = new Retriever(this.properties, this.buObject);
                try {
                    /** @type {MetadataTypeItem[]} */
                    let updatedJourneys;
