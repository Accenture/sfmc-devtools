                    ...
                    if (journeyCache.metadata[key]) {
                        ...
                        switch (journeyCache.metadata[key].definitionType) {
                            case 'Transactional': {
                                ...
                                toBeAuditedVersions.push(journeyCache.metadata[key].version);
                                break;
                            }
                            case 'Multistep': {
                                if (version === '*' || !version) {
                                    ...
                                    if (responseAllVersions?.items?.length) {
                                        ...
                                        const allVersions = responseAllVersions.items.map(
                                            (item) => item.version
                                        );
                                        if (allVersions.length) {
                                            // add versions with highest version first because the log output is also latest first
                                            toBeAuditedVersions.push(
                                ...
                                toBeAuditedVersions.map((version) =>
                                    rateLimitActivities(async () => {
                                        try {
                                            ...
                                            Util.logger.info(
                                                ` - Audit log for ${this.definition.type} ${key} / ${journeyCache.metadata[key].name} - Version ${version}:\n  - ` +
                                                    response.items
                                                        .map((log) => {
                                                            let msg =
                                                            ...
                                                            for (const key in log) {
                                                                if (
                                                                    ...
                                                                    !preformatedProperties.includes(
                                                                        key
                                                                    )
                                                                ) {
                                                                    msg +=
