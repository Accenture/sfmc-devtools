        ...
        try {
            ...
            switch (response.status) {
                case 'PublishCompleted': {
                    spinner.success('done.');
                    Util.logger.info(
                        ` - ${this.definition.type} ${key} / ${name}: publishing succeeded`
                    );
                ...
                case 'PublishInProcess': {
                    Util.logger.debug(
                        ` - ${this.definition.type} ${key} / ${name}: publishing still in progress`
                    );
                    if (tries < 50) {
                ...
                case 'Error': {
                    ...
                    Util.logger.error(
                        ` - ${this.definition.type} ${key} / ${name}: publishing failed.`
                    );
                    this._showPublishStatusDetails(response);
                    return;
                }
