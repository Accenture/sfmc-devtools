    ...
    static async retrieve(retrieveDir, _, __, key) {
        ...
        try {
            ...
            if (
                ...
                const extras =
                    (retrieveDir && singleKey) || Util.OPTIONS.cacheJourneysWithActivities
                        ? extrasDefault
                        : '';
                let url = `${uri}${singleKey}?extras=${extras}${key && key.includes('/') ? '&versionNumber=' + key.split('/')[1] : ''}`;
                if (Util.OPTIONS.onlyPublished) {
                    url += additionalParams.size > 0 ? '&' + additionalParams.toString() : '';
                }
                // caching or single retrieve
                return await super.retrieveREST(retrieveDir, url, null, key);
            } else {
                const url =
