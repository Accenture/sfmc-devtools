                ...
                case 'EMAILV2': {
                    ...
                    if (triggeredSend) {
                        ...
                        if (triggeredSend.publicationListId) {
                            try {
                                triggeredSend.r__list_PathName = cache.getListPathName(
                                    triggeredSend.publicationListId,
                                    'ID'
                                );
                        ...
                        if (triggeredSend.senderProfileId) {
                            try {
                                ...
                                Util.logger.warn(
                                    ` - ${this.definition.type} ${metadata.key}: ${ex.message}`
                                );
                            }
                        }
                        // send classification
                        ...
                        try {
                            ...
                            try {
                                ...
                                Util.logger.warn(
                                    ` - ${this.definition.typeName} '${metadata.Name}'/'${metadata.CustomerKey}': Could not find email with ID ${triggeredSend.emailId} in Classic nor in Content Builder. This TSD cannot be republished but potentially restarted with its cached version of the email.`
                                );
                            }
                        }
                    }
