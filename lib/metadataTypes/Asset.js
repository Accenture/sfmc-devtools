...
class Asset extends MetadataType {
        ...
        if (metadata.assetType.name === 'templatebasedemail') {
            ...
            try {
                if (metadata.views?.html?.template?.id) {
                    metadata.views.html.template.r__asset_customerKey = cache.searchForField(
                        'asset',
    ...
    static async preDeployTasks(metadata, deployDir) {
        ...
        if (
            ...
            delete metadata.views.html.template.r__asset_customerKey;
        }
    ...
    static async resolveId(id) {
        ...
        if (response?.id) {
            ...
            let path = this.#getPath(subType, item, ownerBUName);
            ...
            if (!json.isShared && !path) {
                ...
                // try again
                path = this.#getPath(subType, item, this.buObject.businessUnit);
            }
            ...
            const fileContent = this.#getJson(subType, item);
     ...
     */
    static #getPath(subType, item, buName) {
    ...
    static #getJson(subType, item) {
        ...
        const fileContent = File.readJSONFile(pathArr.join('/'), fileName, true, false);
        return fileContent;
    }
}
