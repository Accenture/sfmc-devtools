...
class Asset extends MetadataType {
    ...
    static async retrieve(retrieveDir, _, subTypeArr, key, loadShared = false) {
        ...
        if (retrieveDir && !cache.getCache()?.asset) {
            ...
            //  1) subtypes asset, email and template reference other content blocks by id/key/objectId as part of their views
            //  2) subtype email references a template if it is template-based
    ...
    static async retrieveAsTemplate(templateDir, name, templateVariables, selectedSubType) {
        if (Util.OPTIONS.shareAsset) {
            this._toggleShareAssetTemplating(true);
        }
        ...
        if (Util.OPTIONS.shareAsset) {
            ...
            this._toggleShareAssetTemplating(false);
        }
        const metadata = this.parseResponseBody({ items: items });
     ...
     *
     * @param {string} retrieveDir Directory where retrieved metadata directory is stored
    ...
    static async buildTemplate(retrieveDir, templateDir, key, templateVariables) {
        ...
        const result = await super.buildTemplate(retrieveDir, templateDir, key, templateVariables);
        if (Util.OPTIONS.shareAsset) {
            // return values to normal
            this._toggleShareAssetTemplating(false);
        }
    ...
    static _extractCode(metadata) {
        ...
        const customerKey = metadata.customerKey.trim();
        switch (metadata.assetType.name) {
            case 'templatebasedemail': // email-templatebasedemail
            case 'htmlemail': {
                // email-htmlemail
                // metadata.views.html.content (mandatory)
     ...
     * @param {string[]} [selectedSubType] asset, email, ...
     ...
     * @returns {void}
     */
    static _toggleShareAssetTemplating(keepSharingInfo) {
        ...
        // the above fields are already creatable, updateable and and retrievable - but they are filtered for templating which we seek to override
        for (const property of sharingProperties) {
            this.definition.fields[property] = {
                // isCreateable: true,
                // isUpdateable: true,
                // retrieving: true,
                template: keepSharingInfo,
            };
        }
    }
}
