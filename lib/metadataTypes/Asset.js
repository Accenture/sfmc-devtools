...
class Asset extends MetadataType {
    ...
    static async requestSubType(subType, retrieveDir, key, templateVariables, loadShared = false) {
        ...
        if (retrieveDir) {
            ...
            Util.logger.info(` - Caching Subtype: ${subTypeStr}`);
        }
        /** @type {AssetSubType[]} */
    ...
    static async preDeployTasks(metadata, deployDir) {
        ...
        await this._preDeployTasksBocks(metadata, deployDir);
     ...
     */
    static async _preDeployTasksBocks(metadata, deployDir) {
        switch (metadata.assetType.name) {
            ...
            case 'webpage': {
                ...
                if (metadata?.views?.html?.slots) {
                    await this._preDeployTasksBocks_slots(metadata.views.html.slots, deployDir);
                }
                break;
            }
            case 'template': {
                ...
                if (metadata?.slots) {
                    await this._preDeployTasksBocks_slots(metadata.slots, deployDir);
                }
     ...
     * @returns {Promise.<void>} -
     */
    static async _preDeployTasksBocks_slots(metadataSlots, deployDir) {
                ...
                if (slotObj.blocks) {
                    for (const block in slotObj.blocks) {
                        if (Object.prototype.hasOwnProperty.call(slotObj.blocks, block)) {
                            ...
                            // TODO: test if we should run preDeployTasks here instead of calling _preDeployTasksBocks again
                            // await this.preDeployTasks(asset, deployDir);
                            ...
                            await this._preDeployTasksBocks(asset, deployDir);
                        }
                    }
                }
            }
