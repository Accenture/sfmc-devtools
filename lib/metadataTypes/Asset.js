...
class Asset extends MetadataType {
    ...
    static async preDeployTasks(metadata, deployDir, buObject) {
        ...
        if (
            ...
            buObject.eid &&
            metadata.enterpriseId === buObject.eid
        ) {
            console.log('+++OLD', metadata.customerKey, metadata.name);
            // only execute #3+4 if we are deploying / copying from one BU to another inside of the same instance
            // #3 make sure the name is unique
            ...
            const namesInFolder = Object.keys(assetCache)
                ...
                .map((key) => ({
                    ...
                    type: assetCache[key].assetType.name,
                    key: key,
                }));
            console.log(
                'namesInFolder',
            ...
            console.log('newName', metadata[this.definition.nameField]);
            // }

            // #4 make sure customer key is unique by prefixing it with target MID (unless we are deploying to the same MID)
            ...
            console.log('---NEW', metadata.customerKey, metadata.name);
        }
        throw new Error();
        return metadata;
    }
    ...
    static _findUniqueName(key, name, type, namesInFolder) {
        ...
        console.log(
            'find',
            namesInFolder.find(
                (name) => name.name === newName && name.type === type && name.key !== key
            )
        );
        while (
            namesInFolder.find(
