...
class Asset extends MetadataType {
     ...
     */
    static async buildTemplate(retrieveDir, templateDir, key, templateVariables, verifiedFiles) {
        if (verifiedFiles === undefined) {
            return super.buildTemplate(retrieveDir, templateDir, key, templateVariables);
        }
        return this.buildBoundAsset(
            retrieveDir,
            templateDir,
            key,
            templateVariables,
    ...
    static async buildBoundAsset(
        ...
        const verifiedFiles = Array.isArray(selection) ? selection : selection.files;
        const inputs = verifiedFiles.map((filename) => {
            const relative = path.relative(source, filename);
            if (!path.isAbsolute(filename) || !relative.startsWith('asset' + path.sep)) {
                throw new Error(`Invalid verified asset input: ${filename}`);
            }
            return { filename, relative };
        });
        if (!inputs.length) {
            throw new Error(`Asset ${key} has no verified template inputs`);
        }
        const owners = [];
        for (const input of inputs) {
            const match = /^(.*)\.asset-([^-]+)-meta\.json$/.exec(path.basename(input.filename));
            if (!match) {
                continue;
            }
            const metadata = JSON.parse(await fs.readFile(input.filename, 'utf8'));
            if (metadata.customerKey === (definition ? selection.customerKey : key)) {
                owners.push({ ...input, metadata, token: match[1], subtype: match[2] });
            }
        }
        if (owners.length !== 1) {
            throw new Error(
                `Asset ${key} requires one unique verified owner; found ${owners.length}`
            );
        }
        const owner = owners[0];
        const { token, subtype } = owner;
        const nested = path.dirname(owner.relative) !== path.join('asset', subtype);
        const expectedOwner = path.join(
            'asset',
            subtype,
            ...(nested ? [token] : []),
            `${token}.asset-${subtype}-meta.json`
        );
        if (owner.relative !== expectedOwner || this._getSubtype(owner.metadata) !== subtype) {
            throw new Error(`Asset ${key} has an invalid verified owner path`);
        }
        if (
            definition &&
            (path.resolve(selection.ownerPath) !== owner.filename ||
                selection.token !== token ||
                selection.subtype !== subtype ||
                selection.nested !== nested)
        ) {
            throw new Error(`Asset ${key} no longer matches its verified definition binding`);
        }
        // Enumeration is inherited, but literal approved paths are read without a second encoding.
        const files = await this._mergeCode(
            structuredClone(owner.metadata),
            source,
            subtype,
            token,
            true
        );
        if (owner.metadata.fileProperties?.extension) {
            files.push({
                subFolder: ['asset', subtype],
                fileName: token,
                fileExt: owner.metadata.fileProperties.extension.split(' ', 1)[0],
                content: '',
                encoding: 'base64',
            });
        }
        const approved = new Set(inputs.map((input) => path.resolve(input.filename)));
        for (const file of files) {
            ...
            if (!approved.has(filename)) {
                throw new Error(`Asset ${key} companion is not verified: ${filename}`);
            }
            file.content = await fs.readFile(filename, file.encoding || 'utf8');
        }
        let metadata = definition
            ? JSON.parse(this.applyTemplateValues(JSON.stringify(owner.metadata), variables))
            : JSON.parse(Util.replaceByObject(JSON.stringify(owner.metadata), variables));
        if (definition) {
            ...
            if (!metadata) {
                throw new Error(`Asset ${key} failed bound definition validation`);
            }
        } else {
            this.keepTemplateFields(metadata);
        }
        const outputToken = definition ? File.filterIllegalFilenames(metadata.customerKey) : token;
        const outputFiles = [];
        const targets = Array.isArray(targetDir) ? targetDir : [targetDir];
        for (const target of targets) {
            for (const file of files) {
                const subFolder = [...file.subFolder];
                // Only the verified owner directory carries the key; deeper names are structural.
                if (nested && subFolder[2] === token) {
                    subFolder[2] = outputToken;
                }
                const ownerStem =
                    subFolder.length === 2 &&
                    (file.fileName === token || file.fileName === `${token}.asset-${subtype}-meta`);
                const filename = ownerStem
                    ? outputToken + file.fileName.slice(token.length)
                    : file.fileName;
                ...
                let content =
                    file.encoding === 'base64'
                        ? file.content
                        : definition
                          ? this.applyTemplateValues(file.content, variables)
                          : this.applyTemplateNames(file.content, variables);
                if (file.fileExt === 'ssjs') {
                    content = Util.getSsjs(content) || content;
                }
                await fs.mkdir(path.dirname(destination), { recursive: true });
                await fs.writeFile(destination, content, file.encoding || 'utf8');
            ...
            const destination = path.resolve(
                ...
                subtype,
                ...(nested ? [outputToken] : []),
                `${outputToken}.asset-${subtype}-meta.json`
            );
            await fs.mkdir(path.dirname(destination), { recursive: true });
            await fs.writeFile(destination, JSON.stringify(metadata, null, 4));
            outputFiles.push(destination);
        }
        return {
            metadata,
                ...
                Util.logger.warn(
                    `   - "${fail.item.name}" (${fail.item.customerKey}): ${fail.error.message} (${
                        ...
                        fail.error.endpoint
                            ? Util.getGrayMsg(
                                  ' - ' +
                                      fail.error.endpoint.split('rest.marketingcloudapis.com', 2)[1]
                              )
                            : ''
                    }`
                );
    ...
    static async _retrieveExtendedFile(metadata, subType, retrieveDir) {
        ...
        const fileExt = metadata.fileProperties.extension.split(' ', 1)[0];
    ...
    static async preDeployTasks(metadata, deployDir) {
        ...
        if (!cache.getByKey(this.definition.type, metadata[this.definition.keyField])) {
            ...
            if (this.definition.extendedSubTypes.webstudio.includes(metadata.assetType.name)) {
        ...
        for (const slot in metadataSlots) {
            ...
            if (slotObj.blocks) {
                for (const block in slotObj.blocks) {
                    ...
                    if (
                        await File.pathExists(
                            File.normalizePath([...readDirArr, 'blocks', `${fileName}.html`])
                        )
                    ) {
                        // the main content can be empty (=not set up yet) hence check if we did extract sth or else readFile() will print error msgs
    ...
    static _extractCode_slots(prefix, metadataSlots, codeArr) {
        for (const slot in metadataSlots) {
            ...
            for (const block in slotObj.blocks) {
                ...
                if (asset.slots) {
                    // * recursion: each block can have slots of its own
                    this._extractCode_slots(`${prefix}.[${slot}-${block}]`, asset.slots, codeArr);
                }
            }
        }
    }
        ...
        for (const dirent of files) {
            ...
            try {
                if (dirent.isDirectory()) {
                    ...
                    const errorDir = currentdir.replaceAll('\\', '/');
            ...
            await Promise.all(
                keyArr.map(async (key) => {
                    ...
                    if (
                        ...
                        // ensure we dont overlook other versions when using this logic to auto-delete older files
                        const fileListNestedSet = new Set(fileListNested);
                        for (const filePath of fileListNested) {
                            // allowed file extensions at this point are only html, ssjs and amp
                            const baseFilePath = filePath.replace(/\.[^/.]+$/, '');
                            fileListNestedSet.add(`${baseFilePath}.html`);
                            fileListNestedSet.add(`${baseFilePath}.ssjs`);
                            fileListNestedSet.add(`${baseFilePath}.amp`);
                        }
    ...
    static async postDeleteTasks(key) {
        ...
        if (!Array.isArray(fileArr) || fileArr.length === 0) {
            Util.logger.verbose(` - ${this.definition.type} ${key} does not exist on disk`);
            return;
        }
    ...
    static getCacheMatchedByName(metadataItem) {
        ...
        if (Util.OPTIONS.matchName) {
            ...
            if (potentials.length >= 1) {
                deployFolderPath = cache.searchForField(
                    'folder',
            ...
            if (potentials.length > 1) {
                const potentialsWithMatchingFolder = potentials.filter(
                    (el) => el.category.id === metadataItem.category.id
                );
                throw new Error(
                    `found multiple name matches in cache. New folder: "${deployFolderPath}". Identified keys: ${potentials.map((p) => p[this.definition.keyField]).join(', ')}. ` +
                        (potentialsWithMatchingFolder.length
                            ? `Folder for ${potentialsWithMatchingFolder[0][this.definition.keyField]} matches the new folder. `
                            : '')
                );
                ...
                if (potentials[0].category.id === metadataItem.category.id) {
                    ...
                    throw new Error(
                        `found match in cache by name but folder is different. New folder: "${deployFolderPath}". Existing: "${cacheFolderPath}": Identified key: ${potentials[0][this.definition.keyField]}`
                    );
                }
            } else {
                Util.logger.debug(
