...
class Asset extends MetadataType {
     ...
     */
    static async _findEmailsUsingBlock(keys, activeEmailKeys) {
        const uri = '/asset/v1/content/assets/query';
        if (!activeEmailKeys.length || !keys.length) {
            // @ts-ignore
            return {};
        }
        // Build active emails condition
        const activeEmailsCondition = activeEmailKeys.reduce((acc, key, index) => {
            const condition = {
        ...
        const query = {
            leftOperand: activeEmailsCondition,
            logicalOperator: 'AND',
    ...
    static async refresh(keyArr) {
        if (!cache.getCache()?.asset) {
            Util.logger.info(' - Caching dependent Metadata: asset-message');
            const assets = await this.retrieveForCache(undefined, null, undefined, false);
            cache.mergeMetadata('asset', assets.metadata);
        }
        let toPublish = {};
        try {
            // cache all published journeys with activites to filter out email activities later
            // only emails in running journeys need to be refreshed
            Util.OPTIONS.onlyPublished = true;
            Util.OPTIONS.cacheJourneysWithActivities = true;
            Journey.buObject = this.buObject;
            Journey.client = this.client;
            const result = await Journey.retrieveForCache();
            cache.setMetadata('journey', result.metadata);
            const activeEmails = Object.values(cache.getCache()?.journey ?? {}).flatMap(
                (j) => j.activities?.filter((a) => a.type === 'EMAILV2') ?? []
            );
            const activeEmailsKeys = activeEmails.map((email) => {
                try {
                    const emailKey = cache.searchForField(
                        'asset',
                        email.name,
                        'name',
                        'customerKey'
                    ...
                    Util.logger.error(
                        `Could not find asset for email activity ${email.name} with error: ${ex.message}`
                    );
                }
            });
            Util.logger.info(` - Found ${activeEmailsKeys.length} emails in running journeys`);
            toPublish = await this._findEmailsUsingBlock(keyArr, activeEmailsKeys);
        } catch (ex) {
            Util.logger.error(ex.message);
        }
        Util.logger.info(` - Found ${Object.keys(toPublish).length} assets to publish`);
        return Object.keys(toPublish).length ? await this._refreshTriggeredSend(toPublish) : [];
    }
}
