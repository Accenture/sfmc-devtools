...
class Asset extends MetadataType {
    ...
    static async resolveId(id) {
        ...
        if (response?.id) {
            ...
            let path = json.isShared ? null : this.#getPath(subType, item);
            ...
            if (!json.isShared && !path) {
                ...
                // try again
                path = this.#getPath(subType, item);
            }
            ...
            if (Util.OPTIONS.json) {
                ...
                if (json.isShared) {
                    Util.logger.warn(' - Shared from MID ' + json.mid);
                }
                if (json.folder) {
                    Util.logger.info(
