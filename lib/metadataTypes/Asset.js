...
class Asset extends MetadataType {
    ...
    constructor(properties, buObject) {
        ...
        this.definition = require('../MetadataTypeDefinitions').asset;
    }
    /**
     ...
     */
    async retrieve(retrieveDir, _, __, selectedSubType) {
        const items = [];
     ...
     * @returns {Promise<{metadata:AssetMap,type:string}>} Promise
     */
    retrieveForCache(selectedSubType) {
     ...
     * @returns {Promise<{metadata:AssetMap,type:string}>} Promise
     */
    async retrieveAsTemplate(templateDir, name, templateVariables, selectedSubType) {
        const items = [];
        const subTypes = selectedSubType ? [selectedSubType] : this._getSubTypes();
        await File.initPrettier();
        // loop through subtypes and return results of each subType for caching (saving is handled per subtype)
     ...
     * @returns {Promise} Promise
     */
    async requestSubType(subType, subTypeArray, retrieveDir, templateName, templateVariables) {
        if (retrieveDir) {
     ...
     * @returns {Promise} Promise
     */
    async requestAndSaveExtended(items, subType, retrieveDir, templateVariables) {
     ...
     */
    async _retrieveExtendedFile(metadata, subType, retrieveDir) {
     ...
     * @returns {Promise<void>} -
     */
    async _readExtendedFileFromFS(metadata, subType, deployDir) {
     ...
     */
    async _mergeCode(metadata, deployDir, subType, templateName) {
     ...
     * @returns {void}
     */
    _extractCode_slots(prefix, metadataSlots, codeArr) {
        for (const slot in metadataSlots) {
            if (Object.prototype.hasOwnProperty.call(metadataSlots, slot)) {
                const slotObj = metadataSlots[slot];
                // found slot
     ...
     */
    async readSecondaryFolder(templateDir, typeDirArr, templateName, fileName) {
