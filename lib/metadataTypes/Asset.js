...
class Asset extends MetadataType {
     ...
     */
    static async _findEmailsUsingBlock(blocks, _foundEmails = {}, _searchedKeys = new Set()) {
        const uri = '/asset/v1/content/assets/query';

        // Mark all current blocks as searched
        for (const block of blocks) {
            _searchedKeys.add(block.customerKey);
        }
        ...
        const payload = {
            ...
            query: blocks
                .flatMap((obj) => [
                    {
                        ...
                        value: `ContentBlockByKey("${obj.customerKey}")`,
                    },
                    ...(obj.id
                        ? [
                              {
                        ...
                        ? [
                              {
                                  property: 'content',
                                  simpleOperator: 'mustContain',
                                  value: `ContentBlockByName("${obj.name}")`,
                              },
                          ]
                        : []),
                ])
                .reduce((acc, condition, index) => {
                    ...
                    return { leftOperand: acc, logicalOperator: 'OR', rightOperand: condition };
                }, null),
        };
        ...
        const emailTypes = this.definition.extendedSubTypes.message;
        const foundEmails = items.filter((item) => emailTypes.includes(item.assetType.name));
        const foundBlocks = items.filter((item) => !emailTypes.includes(item.assetType.name));

        for (const email of foundEmails) {
            _foundEmails[email.customerKey] = email;
        }

        // Recurse with all unsearched blocks in one call
        const unsearchedBlocks = foundBlocks.filter(
            (block) => !_searchedKeys.has(block.customerKey)
        );
        if (unsearchedBlocks.length) {
            await this._findEmailsUsingBlock(unsearchedBlocks, _foundEmails, _searchedKeys);
        }
    ...
    static async refresh(keyArr) {
        ...
        if (codeBlocks.length > 0) {
            toPublish.push(...Object.values(await this._findEmailsUsingBlock(codeBlocks)));
        }
        Util.logger.info(
            ` - Found ${toPublish.length} assets to publish. Looking up triggered sends for the following emails: ${toPublish.map((e) => e.name)}`
        );
