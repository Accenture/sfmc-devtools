...
class Asset extends MetadataType {
     ...
     */
    static async getUpsertOrder(metadataMap, deployDir) {
        /**
        ...
        for (const key in metadataMap) {
            ...
            try {
                ...
                if (dependentKeys.length > 0) {
                    ...
                    Util.logger.debug('Asset.getUpsertOrder: this case should not happen');
    ...
    static async upsert(metadataMap, deployDir) {
        if (Object.keys(metadataMap).length > 1) {
            ...
            metadataMap = await this.getUpsertOrder(metadataMap, deployDir);
        }
