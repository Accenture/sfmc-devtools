...
class AccountUser extends MetadataType {
    ...
    static async preDeployTasks(metadata) {
        ...
        if (metadata.RoleNames__c?.length) {
            metadata.Roles = {
                Role: metadata.RoleNames__c.map((roleName) => {
                    try {
    ...
    static async document(metadata) {
        ...
        for (const id in metadata) {
            ...
            let roles = '';
            if (user.RoleNames__c) {
                roles = '<nobr>' + user.RoleNames__c.join(',</nobr><br> <nobr>') + '</nobr>';
            }
            let associatedBus = '';
