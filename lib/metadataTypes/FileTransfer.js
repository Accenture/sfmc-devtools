...
class FileTransfer extends MetadataType {
   ...
   //  * @param {boolean} [listBadKeys=false] do not print errors, used for badKeys()
     ...
     * @param {String} name name of the metadata file
     * @param {Object} templateVariables variables to be replaced in the metadata
     * @param {String} targetDir variables to be replaced in the metadata
     *
     * @returns {Promise} Promise
     */
    static async buildTemplate(templateDir, name, templateVariables, targetDir) {
        const typeDirArr = [this.definition.type];
        targetDir = targetDir + '/' + typeDirArr;
        const extended = File.readJSONFile(targetDir, name, true, false);
        let val;
        try {
            const originalKey = extended[this.definition.keyField];
            val = JSON.parse(Util.replaceByObject(JSON.stringify(extended), templateVariables));
