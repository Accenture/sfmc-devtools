...
class Query extends MetadataType {
     ...
     */
    static async start(keyArr) {
        if (keyArr[0] == null) {
            keyArr = await this.findItemsToStart();
        }
     ...
     *
     * @private
     * @returns {string[]} array of query keys to start
     */
    static async findItemsToStart() {
        Util.logger.info(`Finding items to start`);
