...
class Query extends MetadataType {
    ...
    static async execute(keyArr) {
        if (keyArr[0] == null) {
            keyArr = await this.findItemsToExecute();
        }
        for (const key of keyArr) {
            const uri = `/automation/v1/queries/${key}/actions/start/`;
            Util.logger.info(`Starting item ${key}`);
            super.executeREST(uri);
        }
        return 'OK';
    }
    /**
     ...
     *
     * @private
     * @returns {string[]} array of query keys to start executing
     */
    static async findItemsToExecute() {
        Util.logger.info(`Finding items to execute`);
        const response = await super.retrieveREST(null, '/automation/v1/queries/');
        return Object.values(response.metadata).map((item) => item.queryDefinitionId);
    }
    /**
