...
class Query extends MetadataType {
    ...
    // /automation/v1/queries/{{queryID}}/actions/start/
    //automations/{id}/actions/start
    /**
    ...
    static async findItemsToStart() {
        const it = await super.retrieveREST(null, '/automation/v1/queries/');
        const idList = [];
        for (const item of Object.values(it.metadata)) {
            idList.push(item.queryDefinitionId);
        }
        return idList;
    }
    /**
