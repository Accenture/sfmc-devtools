...
class Event extends MetadataType {
    ...
    static async postRetrieveTasks(metadata) {
        ...
        try {
            ...
            Util.logger.warn(
                ` - ${this.definition.type} '${metadata[this.definition.nameField]}' (${metadata[this.definition.keyField]}) with ${metadata.publishedInteractionCount} active journeys: ${ex.message}`
            );
        }
        try {
            await this.compareSalesforceEntryEvents_dataExtension(
