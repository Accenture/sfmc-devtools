...
class Event extends MetadataType {
    ...
    static async preDeployTasks(metadata) {
        ...
        const warnings = await this.preDeployTasks_SalesforceEntryEvents(
            metadata.type,
    ...
    static async postRetrieveTasks(metadata) {
        ...
        try {
            ...
            const msg = ` - ${this.definition.type} '${metadata[this.definition.nameField]}' (${metadata[this.definition.keyField]}) with ${metadata.publishedInteractionCount} active journeys: ${ex.message}`;
            Util.logger.warn(metadata.publishedInteractionCount === 0 ? Util.getGrayMsg(msg) : msg);
        }
        try {
            await this.compareSalesforceEntryEvents_dataExtension(
                metadata.type,
            ...
            if (!this.sfObjects.workflowObjects.includes(referencedObject)) {
                ...
                for (const fieldName of object.fields) {
                    ...
                    if (!ca.eventDataSummary.includes(object.dePrefix + fieldName)) {
                        // we could auto-create eventDataSummary but frankly this is good for code reviews and for searching for fields
                        errors.push(
     ...
     * @returns {Promise.<void>} -
     */
    static async compareSalesforceEntryEvents_dataExtension(
        triggerType,
        eventDataSummary,

        deKey
    ) {
        if (triggerType !== 'SalesforceObjectTriggerV2' || !eventDataSummary) {
            return;
        }

        /** @type {DataExtensionItem} */
        const deItem = deKey ? cache.getByKey('dataExtension', deKey) : null;
        if (!deItem) {
            return;
        }
    ...
    static async preDeployTasks_SalesforceEntryEvents(triggerType, ca) {
        ...
        const warnings = this.checkSalesforceEntryEvents(ca, false);
