...
class Event extends MetadataType {
    ...
    static preDeployTasks(metadata) {
        ...
        this.preDeployTasks_SalesforceEntryEvents(metadata.type, metadata.configurationArguments);
    ...
    static async getSalesforceObjects(objectAPIName) {
        ...
        if (!this.sfObjects.workflowObjects) {
            ...
            this.sfObjects.workflowObjects = workflowObjectsResponse
                ...
                : [];
            Util.logger.debug(
                'sfObjects.workflowObjectApiNames: ' +
                    JSON.stringify(this.sfObjects.workflowObjects)
            );
        }
        ...
        if (!this.sfObjects.referencedObjects?.[objectAPIName]) {
            ...
            try {
                ...
                const selfReference = { referenceObjectName: objectAPIName };
                this.sfObjects.referencedObjects[objectAPIName] = referenceObjectsResponse
                    ? [selfReference, ...referenceObjectsResponse]
                    : [selfReference];
                Util.logger.debug(
                    `sfObjects.referencedObjects[${objectAPIName}]: ` +
                        JSON.stringify(this.sfObjects.referencedObjects[objectAPIName])
                );
            } catch (ex) {
            ...
            await Promise.all(
                uniqueSfObjectNames.map((objectAPIName) =>
                    rateLimit(async () => {
                        if (this.sfObjects.objectFields[objectAPIName]) {
                            return;
                        }
                        Util.logger.info(
                            Util.getGrayMsg(
    ...
    static checkSalesforceEntryEvents(ca) {
        ...
        const referencedContactObj = this.sfObjects.referencedObjects[ca.objectAPIName].find(
            (el) => el.relationshipName === ca.contactKey.relationshipName
        );
        if (referencedContactObj) {
            if (referencedContactObj.isPolymorphic !== ca.contactKey.isPolymorphic) {
                errors.push(
            ...
            if (
                !this.sfObjects.objectFields[ca.contactKey.referenceObjectName][
                    ca.contactKey.fieldName
                ]
            ) {
                errors.push(
                    `configurationArguments.contactKey states the invalid fieldName '${ca.contactKey.fieldName}' value that does not exist on ${ca.contactKey.referenceObjectName}`
                );
            }
        } else {
            errors.push(
                `configurationArguments.contactKey references ${ca.contactKey.relationshipName} which is not found in related salesforce objects`
            );
        }
        ...
        // 6 compare primaryObjectFilterSummary to primaryObjectFilterCriteria
        // TODO
     ...
     * @returns {Promise.<void>} -
     */
    static async postRetrieveTasks_SalesforceEntryEvents(triggerType, ca, key) {
        ...
        for (const attribute of this.requiredConfigurationArguments) {
            if (!ca[attribute]) {
                Util.logger.error(
                    ` - ${this.definition.type} ${key}: required field configurationArguments.${attribute} not set`
                );
            }
        }
