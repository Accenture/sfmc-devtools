...
class Automation extends MetadataType {
    ...
    static validateDeployMetadata(metadata) {
            ...
            for (const step of metadata.steps) {
                ...
                for (const activity of step.activities) {
                    ...
                    // cannot deploy because it is not supported
                        ...
                        errors.push(
                            `  • not supported activity '${activity.name}' in step ${stepNumber}.${displayOrder}`
                        );
                        deployable = false;
                    }
                }
        ...
        if (!deployable) {
            Util.logger.error(`☇ skipping ${this.definition.type} '${metadata.name}':`);
            errors.forEach((error) => Util.logger.error(error));
        }
        return deployable;
    }
