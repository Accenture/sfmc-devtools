...
class Automation extends MetadataType {
    ...
    static _buildSchedule(scheduleObject) {
        ...
        if (!['Minute', 'Hour', 'Dai', 'Week', 'Month'].includes(keyStem)) {
        ...
        if (new Date(inputStartDateString) > new Date()) {
            ...
            switch (keyStem) {
                case 'Month': {
                    ...
                    if (scheduledDate.getDate() <= futureDate.getDate()) {
                ...
                case 'Week': {
                    // keep time from template and start today if possible
                    const wantedDayOfWeek = scheduledDate.getDate();
                    do {
                        futureDate.setDate(futureDate.getDate() + 1);
                    } while (
                        futureDate.getDay() !== wantedDayOfWeek &&
                        (scheduledDate.getDate() < futureDate.getDate() ||
                            (scheduledDate.getDate() == futureDate.getDate() &&
                                scheduledDate.getHours() <= futureDate.getHours()))
                    );
                    futureDate.setHours(scheduledDate.getHours());
                    futureDate.setMinutes(scheduledDate.getMinutes());
