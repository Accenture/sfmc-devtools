...
class Automation extends MetadataType {
    ...
    static async retrieveAsTemplate(templateDir, name, templateVariables) {
        ...
        if (Array.isArray(results?.Results)) {
            ...
            const [metadata] = results.Results.filter((item) => item.Name === name);
            if (!metadata) {
            ...
            for (const step of metadata.steps) {
                ...
                for (const activity of step.activities) {
                    ...
                    if (this.definition.manualDeployTypes.includes(activity.r__type)) {
                        Util.logger.warn(
                            `Automation '${metadata.name}' requires additional manual configuration: '${activity.name}' in step ${stepNumber}.${displayOrder}`
                        );
                    }
                    // cannot deploy because it is not supported
                    else if (!this.definition.dependencies.includes(activity.r__type)) {
                        Util.logger.error(
                            `Automation '${metadata.name}' cannot be deployed as the following activity is not supported: '${activity.name}' in step ${stepNumber}.${displayOrder}`
                        );
                        deployable = false;
                    }
                }
    ...
    static _generateDocMd(json, tabled) {
        ...
        output += `**Started by:** ${
            automationType[json.type] ? automationType[json.type] : 'Not defined'
        }\n\n`;
        const tz =
        ...
        if (json.schedule?.icalRecur) {
            ...
            const ical = {};
            json.schedule.icalRecur.split(';').forEach((item) => {
    ...
    static async document(buObject, metadata, isDeploy) {
        if (['md', 'both'].includes(this.properties.options.documentType)) {
            ...
            Util.logger.info(`Created ${docPath}/*.automation-doc.md`);
        }
    }
}
