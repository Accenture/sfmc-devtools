...
class Automation extends MetadataType {
     ...
     * @returns {string}
     */
    static async setNotifEmailAddr(keys, emailFailed, emailComplete) {
        Util.logger.info(`Setting notification email address`);
        await this.retrieve(undefined, undefined, undefined, keys);
        Util.logger.info(`${emailFailed} ${emailComplete}`);
        // this.#updateNotificationInfoREST(); //[ { email: [ 'test@test.com' ], message: '', type: 'Error' } ]
        // results.metadata[key].notifications[0].email = Util..;
