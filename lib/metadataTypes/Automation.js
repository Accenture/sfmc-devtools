...
class Automation extends MetadataType {
            ...
            if (ex.message == 'socket hang up' || ex.code == 'ERR_BAD_RESPONSE') {
                ...
                Util.logger.info(
                    ...
                    ${
                        ex.endpoint
                            ? Util.getGrayMsg(
                                  ' - ' + ex.endpoint.split('rest.marketingcloudapis.com', 2)[1]
                              )
                            : ''
                    }`
                );
                        ...
                        if (Array.isArray(extended?.processes)) {
                            for (const step of extended.processes) {
                                ...
                                for (const activity of step.workers) {
                                    // activties
                                    if (!(
                                        activity.objectTypeId === 467 && activity.serializedObject
                                    )) {
                                        continue;
                                    }
                    ...
                    for (const activity of step.activities) {
                        ...
                        if (
                            ...
                            if (activity.r__type === 'wait') {
                                ...
                                let waitTime24;
                                const [waitDuration, waitUnit] = activity.name.split(' ', 2);
                                if (waitUnit === 'AM' || waitUnit === 'PM') {
                                    waitTime24 = waitDuration;
                                    if (waitUnit === 'PM') {
                                        waitTime24 =
                                            (Number(waitTime24.split(':', 1)[0]) + 12).toString() +
                                            ':00';
                                    }
    ...
    static getErrorsREST(ex) {
        ...
        if (errors?.length > 0) {
            return errors.map((msg) =>
                msg.replaceAll(
                    '403 Forbidden',
                    '403 Forbidden: Please check if the automation is currently running.'
                )
            );
        }
        return errors;
    ...
    static _calcTime(offsetServer, dateInput, offsetInput) {
        ...
        return dateServer.toISOString().slice(0, -1).split('.', 1)[0];
    }
    ...
    static _generateDocMd(json, tabled) {
        ...
        if (json.type === 'scheduled' || json.schedule) {
            ...
            if (json.schedule?.icalRecur) {
                output += `**Schedule:**\n\n`;
                output += `* Start: ${json.schedule.startDate.replaceAll('T', ' ')} ${tz}\n`;
                output += `* End: ${json.schedule.endDate.replaceAll('T', ' ')} ${tz}\n`;
    ...
    static async _writeDoc(directory, filename, json, mode) {
        ...
        if (mode === 'md') {
            const output = this._generateDocMd(json, tabled);
            try {
                // write to disk
