...
class Automation extends MetadataType {
    ...
    constructor(properties, buObject) {
        ...
        this.definition = require('../MetadataTypeDefinitions').automation;
    }
    /**
     ...
     * @returns {Promise<{metadata:AutomationMap,type:string}>} Promise of metadata
     */
    async retrieveChangelog() {
        const results = await this.client.soap.retrieve('Program', ['ObjectID'], {});
        const details = [];
        (
            await Promise.all(
                results.map(async (a) => {
     ...
     */
    validateDeployMetadata(metadata) {
        let deployable = true;
        if (metadata.steps) {
            for (const step of metadata.steps) {
                for (const activity of step.activities) {
     ...
     */
    async postDeployTasks(metadata, originalMetadata) {
     ...
     */
    _calcTime(offsetServer, dateInput, offsetInput) {
