            ...
            for (const step of metadata.steps) {
                ...
                for (const activity of step.activities) {
                    ...
                    // cannot deploy because it is not supported
                        ...
                        Util.logger.error(
                            `☇ skipping Automation '${metadata.name}': not supported activity '${activity.name}' in step ${stepNumber}.${displayOrder}`
                        );
                        deployable = false;
                    }
                }
        ...
        for (const key in metadata) {
            ...
            if (originalMetadata[key]?.type === 'scheduled') {
                ...
                if (originalMetadata[key].status === 'Scheduled') {
                    ...
                    Util.logger.warn(
                        `- Automation '${originalMetadata[key].name}' deployed Paused`
                    );
                }
            }
