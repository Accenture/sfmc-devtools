...
class Automation extends MetadataType {
     ...
     */
    static async retrieve_new(retrieveDir, _, __, key) {
        if (key) {
            ...
            if (results.Results?.length) {
                ...
                const id = results.Results[0].ObjectID;
                return super.retrieveREST(
                    retrieveDir,
                    '/automation/v1/automations/' + id,
                    null,
                    key
                );
            }
    ...
    static async retrieve(retrieveDir, _, __, key) {
        ...
        if (Object.keys(metadataMap).length) {
            ...
            await this.#getAutomationNotificationsSOAP(metadataMap);
        }
     ...
     */
    static async #getAutomationNotificationsSOAP(metadataMap) {
        Util.logger.info(`Retrieving Automation Notification information...`);
        // ! this retrieve fails if it's filtered by key
        const requestParams = {
            filter: {
                leftOperand: 'Status',
                operator: 'IN',
        ...
        const resultsNotifications = await this.client.soap.retrieveBulk(
            'Automation',
            ['*', 'CustomerKey', 'Name', 'Description'],
            requestParams
        );
        const keyFieldBackup = this.definition.keyField;
        this.definition.keyField = 'CustomerKey';
        const notificationsMap = this.parseResponseBody({ items: resultsNotifications.Results });
        this.definition.keyField = keyFieldBackup;

        for (const key in notificationsMap) {
            if (notificationsMap[key].Notifications?.Notification) {
                if (!Array.isArray(notificationsMap[key].Notifications.Notification)) {
                    // the SFMC-SDK returns single values not as an array, so we need to convert it
                    notificationsMap[key].Notifications.Notification = [
                        notificationsMap[key].Notifications.Notification,
                    ];
                }
                notificationsMap[key] = {
                    notifications: notificationsMap[key].Notifications.Notification.map((n) => ({
                        email: n.Address.split(',').map((item) => item.trim()),
                        message: n.Body,
                        type: n.NotificationType,
                    })),
                };
            } else {
                delete notificationsMap[key];
            }
        }

        if (Object.keys(notificationsMap).length) {
            for (const key in notificationsMap) {
                const automation = metadataMap[key];
                if (automation) {
                    automation.notifications = notificationsMap[key].notifications;
                }
            }
            Util.getGrayMsg(
                ` - Found E-mail Notifications for ${
                    Object.keys(notificationsMap).length
                } Automations`
            );
        }
    }
    ...
    static async #getAutomationNotificationsREST(metadataMap) {
        ...
        const encodedAutomations = Object.keys(automationsLegacy.metadata)
            ...
            .map((key) => ({
                id: automationsLegacy.metadata[key].id,
                key,
            }));
            ...
            encodedAutomations.map((encAutomation) =>
                rateLimit(async () => {
                    ...
                    try {
                        ...
                        Util.logger.debug(
                            ` ☇ skipping Notifications for Automation ${encAutomation.key}: ${ex.message} ${ex.code}`
                        );
                        skipped++;
                    }
                })
        ...
        Util.logger.info(`Notifications found for ${found} automations`);
        Util.logger.debug(`Notifications not found for ${skipped} automations`);
        return promiseMap;
    }
     ...
     * @returns {Promise.<void>} -
     */
    static async #updateNotificationInfoREST(metadata, key) {
        if (this.notificationUpdates[key]) {
            const cachedAutomation = cache.getByKey(this.definition.type, key);
            const programId = cachedAutomation?.programId; // TODO
            Util.logger.warn('programId: ' + programId);
            if (programId) {
                ...
                try {
                    ...
                    Util.logger.error(
                        `Error updating notifications for automation '${metadata[key].name}': ${ex.message} (${ex.code}))`
                    );
                }
                Util.logger.info(
                    Util.getGrayMsg(
                        ` - updated notifications for automation '${metadata[key].name}'`
                    )
                );
            }
        }
     ...
     * @returns {Promise.<void>} -
     */
    static async #createNotificationInfoSOAP(metadata, key) {
        if (this.notificationUpdates[key]) {
            const automation = {
                Client: {
                ...
                CustomerKey: key,
                Notifications: {
                    Notification: this.notificationUpdates[key].map((notification) => {
                        notification.ChannelType = 1;
                        return notification;
                    }),
                },
            };
            await this.client.soap.update('Automation', automation);
            Util.logger.info(
                Util.getGrayMsg(` - updated notifications for automation '${metadata[key].name}'`)
            );
        }
    }
