...
class Verification extends MetadataType {
    ...
    static async preDeployTasks(metadata) {
        ...
        metadata[this.definition.idField] = cache.getByKey(
            'verification',
            metadata[this.definition.keyField]
        )
            ? // set the id to enable proper updates
              (metadata[this.definition.idField] = Util.inverseGet(
                  this.verificationIdKeyMap,
                  metadata[this.definition.keyField]
              ))
            : // store the key in the idField to ensure we have it after creation
              (metadata[this.definition.idField] = metadata[this.definition.keyField]);
