...
class SenderProfile extends MetadataType {
    ...
    static postRetrieveTasks(metadata) {
        ...
        if (fromAddress && !fromAddress.startsWith('%%') && fromAddress.includes('@')) {
            const domainVerification = cache.getByKey('domainVerification', fromAddress.trim());
            if (domainVerification) {
                if (domainVerification.status !== 'Verified') {
                    Util.logger.warn(
                        ` - ${this.definition.type} ${metadata[this.definition.nameField]} (${metadata[this.definition.keyField]}): FromAddress ${fromAddress} is currently in status ${domainVerification.status}. This SenderProfile will not be usable in Journey Builder. To fix, please go to Setup > Feature Settings > Email Studio > From Address Management to verify the email.`
                    );
                }
                if (domainVerification.isSendable === false) {
                    Util.logger.warn(
                        ` - ${this.definition.type} ${metadata[this.definition.nameField]} (${metadata[this.definition.keyField]}): FromAddress ${fromAddress} is currently not sendable. This SenderProfile will not be usable in Journey Builder. To fix, please go to Setup > Feature Settings > Email Studio > From Address Management to verify the email. You can also update this via mcdev`
                    );
                }
            } else {
                Util.logger.warn(
                    ` - ${this.definition.type} ${metadata[this.definition.nameField]} (${metadata[this.definition.keyField]}): FromAddress ${fromAddress} is not verified. This SenderProfile will not be usable in Journey Builder. To fix, please go to Setup > Feature Settings > Email Studio > From Address Management to verify the email.`
                );
            }
        }
        return metadata;
    ...
    static async postDeployTasks(upsertResults) {
        ...
        for (const key of Object.keys(upsertResults)) {
            ...
            const fromAddress = upsertResults[key].FromAddress.trim() + '';

            if (fromAddress && !fromAddress.startsWith('%%') && fromAddress.includes('@')) {
                const domainVerification = cache.getByKey('domainVerification', fromAddress.trim());
                if (!domainVerification) {
                    domainVerificationCreateMap[fromAddress] = {
                        domain: fromAddress,
                    };
                }
            }
        }
        if (Object.keys(domainVerificationCreateMap).length) {
            Util.logger.info(
            ...
            Util.logger.debug(JSON.stringify(domainVerificationCreateResult));
        }
    }
