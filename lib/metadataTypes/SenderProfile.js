            ...
            if (email && !email.startsWith('%%') && email.includes('@')) {
                ...
                if (domainVerification) {
                    ...
                    if (domainVerification.isSendable === false) {
                        Util.logger.warn(
                            Util.getMsgPrefix(definition, metadataCaller) +
                                `: ${field} ${email} is currently not sendable. This ${definition.typeName} will not be usable in Journey Builder. To auto-fix run any update on the senderProfile via mcdev.`
                        );
                    }
                } else {
                    Util.logger.warn(
                        Util.getMsgPrefix(definition, metadataCaller) +
                            `: ${field} ${email} is not verified. This ${definition.typeName} will not be usable in Journey Builder. To auto-fix run any update on the senderProfile via mcdev.`
                    );
                }
            }
        }
