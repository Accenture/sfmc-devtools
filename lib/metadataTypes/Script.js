...
class Script extends MetadataType {
     ...
     * @returns {Promise<{metadata:Script,type:string}>} Promise
     */
     ...
     */
    static _applyMarket(code, metadata, templateVariables) {
     ...
     * @returns {string} code with markets applied
     */
    static _applyTemplateVariables(code, metadata, templateVariables) {
        // replace template variables with their values
     ...
     * @returns {Promise<void>} Promise
     */
    static async buildXForExtracts(
        templateDir,
        targetDir,
        metadata,
        templateVariables,
        ...
        try {
            if (mode === 'definition') {
                // replace template variables with their values
                code = this._applyMarket(code, metadata, templateVariables);
            } else if (mode === 'template') {
                // replace template values with corresponding variable names
                code = this._applyTemplateVariables(code, metadata, templateVariables);
            }
        } catch (ex) {
            throw new Error(
