...
class User extends MetadataType {
    ...
    static async preDeployTasks(metadata) {
        ...
        if (metadata.RoleNamesGlobal__c?.length) {
            metadata.Roles = {
                Role: metadata.RoleNamesGlobal__c.filter(
        ...
        if (metadata.AssociatedBusinessUnits__c.length) {
            // ensure we do not have duplicates in the list - could happen due to user error or due to templating
            metadata.AssociatedBusinessUnits__c = [...new Set(metadata.AssociatedBusinessUnits__c)];
        }
    ...
    static createOrUpdate(metadata, metadataKey, hasError, metadataToUpdate, metadataToCreate) {
        ...
        if (action === 'create') {
            ...
            User._handleRoleAssignments(updateItem);
        ...
        if (configs.length > 0) {
            ...
            if (buResponse.OverallStatus === 'OK') {
                ...
                for (const [userId, buResult] of Object.entries(userBUresults)) {
                    // show CLI log
                    Util.logger.info(
                        ` - user ${userIdNameMap[userId]}: MID ${buResult['add'].join(
                            ', '
                        )} access granted / MID ${buResult['delete'].join(', ')} access removed`
                    );
                    // update BU map in local variable
                    if (buResult['add']?.length) {
                        this.userIdBuMap[userId].push(...buResult['add']);
                    }
     ...
     * @returns {void}
     */
    static _handleRoleAssignments(item) {
        ...
        for (const newRole of item.after.Roles.Role) {
            ...
            if (!roleAlreadyExisting) {
                ...
                // keep the following here either way, in case SFMC ever fixes the issue with "Marketing Cloud" role assignment - it's not causing API errors
                newRole.AssignmentConfigurations = this._getRoleObjectForDeploy(
                    newRole.ObjectID,
                    newRole.Name,
    ...
    /**
     * helper for {@link User._handleRoleAssignments}
     *
