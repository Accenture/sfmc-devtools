...
class User extends MetadataType {
    ...
    static _setPasswordForNewUser(metadata) {
        ...
        Util.logger.info(` - Password for ${metadata.UserID} was not given. Generated password:`);
    ...
    static async document(metadata) {
        ...
        for (const id in metadata) {
            ...
            const LastSuccessfulLogin = user.LastSuccessfulLogin.split('.', 1)[0];
            ...
            users.push({
                ...
                LastSuccessfulLogin: user.LastSuccessfulLogin
                    ? // on create & update, LastSuccessfulLogin often gets overwritten with the current date
                      LastSuccessfulLogin === user.CreatedDate.split('.', 1)[0] ||
                      LastSuccessfulLogin === user.ModifiedDate.split('.', 1)[0]
                        ? 'unknown'
                        : this.#timeSinceDate(user.LastSuccessfulLogin)
                    : 'never',
                CreatedDate: user.CreatedDate ? user.CreatedDate.replaceAll('T', ' ') : 'n/a',
                ModifiedDate: user.ModifiedDate ? user.ModifiedDate.replaceAll('T', ' ') : 'n/a',
