...
class User extends MetadataType {
    ...
    static createOrUpdate(metadata, metadataKey, hasError, metadataToUpdate, metadataToCreate) {
        ...
        User._setPasswordForNewUsers(metadataToCreate);
        User._unassignRemovedRoles(metadataToUpdate);
     ...
     * @param {TYPE.MetadataTypeItem} metadata single metadata itme
     */
    static prepareBuAssignments(metadataToUpdate, metadataToCreate, metadata) {
        this.userBUassignments = { add: {}, delete: {} };
        for (const item of metadataToUpdate) {
            // remove business units that were unassigned
            const deletedBUs = [];
            item.before.AssociatedBusinessUnits__c = this.userIdBuMap[item.before.ID] || [];
            for (const oldBuAssignment of item.before.AssociatedBusinessUnits__c) {
                // check if oldRole is missing in list of new roles
                if (!item.after.AssociatedBusinessUnits__c.includes(oldBuAssignment)) {
                    deletedBUs.push(oldBuAssignment);
                }
            }
            if (deletedBUs.length > 0) {
                this.userBUassignments['delete'][item.before.AccountUserID] = deletedBUs;
            }
            // add business units that were newly assigned
            const addedBUs = [];
            for (const newBuAssignment of item.after.AssociatedBusinessUnits__c) {
                // check if oldRole is missing in list of new roles
                if (!item.before.AssociatedBusinessUnits__c.includes(newBuAssignment)) {
                    addedBUs.push(newBuAssignment);
                }
            }
            if (addedBUs.length > 0) {
                this.userBUassignments['add'][item.before.AccountUserID] = addedBUs;
            }
        }
        // add BUs for new users
        for (const item of metadataToCreate) {
            const addedBUs = item.AssociatedBusinessUnits__c || [];
            if (addedBUs.length > 0) {
                this.userBUassignments['add']['key:' + item.CustomerKey] = addedBUs;
            }
        }
    ...
    static async _handleBuAssignments(metadata) {
        ...
        for (const action in this.userBUassignments) {
            let userId;
            let buIds;
            for ([userId, buIds] of Object.entries(this.userBUassignments[action])) {
                if (!userId) {
                    continue;
                }
                if (userId.startsWith('key:')) {
                    // for newly created users we did not have the ID when creating this.userBUassignments but we do now
                    userId = metadata[userId.slice(4)].ID;
                }
                configs.push(
                    /** @type {TYPE.UserConfiguration} */ {
     ...
     *
     * @private
     * @param {TYPE.UserDocument[]} metadataToCreate list of created users
     * @returns {void}
     */
     ...
     *
     * @private
     * @param {TYPE.UserDocumentDiff[]} metadataToUpdate list of updated users
     * @returns {void}
     */
