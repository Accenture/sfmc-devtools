...
class DataExtract extends MetadataType {
    ...
    static async retrieveAsTemplate(templateDir, name, variables) {
        const res = await sdk().rest.get(
            '/automation/v1/dataextracts/?$filter=name%20eq%20' + name.split(' ').join('%20')
        );
        if (res.body && Array.isArray(res.items) && res.items.length) {
            ...
            // get full definition
            const extended = await sdk().rest.get('/automation/v1/dataextracts/' + metadata.id);
