...
const { getDefinition } = require('../MetadataTypes');
const Util = require('../../util/util');
const File = require('../../util/file');
const { sdk } = require('../../util/auth');
...
class Automation extends MetadataType {
    ...
    static async retrieve(retrieveDir) {
        const results = await sdk().soap.retrieveBulk('Program', ['ObjectID']);

        const details = results.Results
            ? await Promise.all(
                  results.Results.map((a) =>
                      sdk.rest.get('/automation/v1/automations/' + a.ObjectID)
                  )
              )
            : [];
    ...
    static async retrieveForCache() {
        const results = await sdk().soap.retrieveBulk('Program', [
            'ObjectID',
            'CustomerKey',
            'Name',
        ]);
    ...
    static async retrieveAsTemplate(templateDir, name, variables) {
        const results = await sdk().soap.retrieve('Program', ['ObjectID', 'Name'], {
            filter: {
                leftOperand: 'Name',
                operator: 'equals',
                rightOperand: name,
        ...
        if (results && Array.isArray(results.Results)) {
            ...
            const details = await sdk().rest.get('/automation/v1/automations/' + metadata.ObjectID);
            let val = null;
                ...
                for (const activity of step.activities) {
                    if (activity.name && this.definition.dependencies.includes(activity.r__type)) {
                        ...
                        activity.activityObjectId = Util.getFromCache(
                            ...
                            activity.r__type,
                            activity.name,
                            getDefinition(activity.r__type).nameField,
                            getDefinition(activity.r__type).idField
                        );
                    }
                    ...
                    for (const activity of step.activities) {
                        try {
                            ...
                            // / if managed by cache we can update references to support deployment
                                ...
                                getDefinition(activity.r__type) &&
                                getDefinition(activity.r__type)['idField'] &&
