...
const { getDefinition, getType } = require('../index');
const DataExtensionField = getType('DataExtensionField');
const Util = require('../../util/util');
const File = require('../../util/file');
const { sdk, initSDK } = require('../../util/auth');
...
class DataExtension extends MetadataType {
    ...
    static async create(metadata) {
        ...
        try {
            const res = await sdk().soap.create('DataExtension', metadata, {});
    ...
    static async update(metadata) {
        ...
        try {
            const res = await sdk().soap.update('DataExtension', metadata, {});
    ...
    static async retrieve(retrieveDir, additionalFields, buObject) {
        ...
        if (!retrieveDir && buObject.eid !== buObject.mid) {
            ...
            // change sdk to EID for retrieve all
            await initSDK(this.properties.credentials[buObject.credential].eid);
            const metadataParentBu = await this._retrieveAll(additionalFields);

            // get the types and clean out non-shared ones
            const folderTypesFromParent = getDefinition('folder').folderTypesFromParent;
    ...
    static async deleteByKey(buObject, customerKey) {
        await initSDK(buObject.mid);
        try {
            const res = await sdk().soap.delete('DataExtension', { CustomerKey: customerKey }, {});

            if (res && res.Results && res.Results[0]) {
                Util.logger.info(
                    `mcdev.deleteDE:: Success: ${res.Results[0].StatusMessage} (${customerKey})`
                );
                ...
                Util.logger.warn(
                    'mcdev.deleteDE:: No Deletion was done but no error Occured: ' +
                        JSON.stringify(res)
                );
            }
        } catch (ex) {
            if (ex.response && ex.response.Results && ex.response.Results[0]) {
                if (ex.response.Results[0].ErrorCode === '310007') {
                    Util.logger.error(
                    ...
                    Util.logger.error('mcdev.deleteDE:: ' + ex.response.Results[0].StatusMessage);
                }
            }
            Util.logger.error('mcdev.deleteDE:: ' + JSON.stringify(ex.response));
        }
    }
