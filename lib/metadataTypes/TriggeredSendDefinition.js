...
class TriggeredSendDefinition extends MetadataType {
    ...
    static async _findRefreshableItems() {
        ...
        for (const [type, subTypeArr] of Object.entries(requiredCache)) {
            if (!cache.getCache()?.[type]) {
                ...
                const result = await cacheTypes[type].retrieveForCache(this.buObject, subTypeArr);
    ...
    static async _refreshItem(key, checkKey) {
        ...
        if (checkKey) {
            ...
            try {
                test = (
                    await super.retrieveSOAP(null, null, requestParams, [
                        'CustomerKey',
                        'TriggeredSendStatus',
                    ])
                )?.metadata;
        ...
        // wait for pause to finish on the server's internal queue
        ...
        // wait for publish to finish on the server's internal queue
        await Util.sleep(3000);
