...
class Folder extends MetadataType {
    ...
    static async create(metadata) {
        ...
        if (metadata.Parent && metadata.Parent.ID && metadata.Parent.ID === 0) {
            ...
            try {
                const response = await this.client.soap.create('DataFolder', metadata, null);
                ...
                Util.logger.info(`- created folder: ${metadata.Path}`);
                return response;
            } catch (ex) {
                if (ex.results) {
                    Util.logger.error(
                        `${this.definition.type}-${metadata.ContentType}.create:: error creating: ${metadata.Path}. ${ex.results[0].StatusMessage}`
                    );
                } else {
                    Util.logger.error(
                        `${this.definition.type}-${metadata.ContentType}.create:: error creating: ${metadata.Path}. ${ex.message}`
                    );
                }
                return;
            }
    ...
    static async update(metadata) {
        ...
        try {
            const response = await this.client.soap.update('DataFolder', metadata, null);
            response.Results[0].Object = metadata;
            response.Results[0].Object.CustomerKey = metadata.CustomerKey;
            delete response.Results[0].Object.$;
            Util.logger.info(`- updated folder: ${metadata.Path}`);
            return response;
        } catch (ex) {
            if (ex.results) {
                Util.logger.error(
                    `${this.definition.type}-${metadata.ContentType}.update:: error updating: '${metadata.Path}'. ${ex.results[0].StatusMessage}`
                );
            } else {
                Util.logger.error(
                    `${this.definition.type}-${metadata.ContentType}.update:: error updating: '${metadata.Path}'. ${ex.message}`
                );
            }
            return;
        }
    ...
    static async retrieveHelper(overrideFields, QueryAllAccounts) {
        const response = await this.client.soap.retrieveBulk(
            'DataFolder',
            this.getFieldNamesToRetrieve(overrideFields).filter((field) => !field.includes('Path')),
            {},
