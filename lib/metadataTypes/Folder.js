...
class Folder extends MetadataType {
    ...
    static async create(metadata) {
        ...
        let response;
        try {
            response = await super.createSOAP(metadata, 'DataFolder', true);
            if (response) {
            ...
            if (ex && ex.results) {
                Util.logger.error(
                    `${this.definition.type}-${metadata.ContentType}.create:: error updating: '${path}'. ${ex.results[0].StatusMessage}`
                );
            } else if (ex) {
                Util.logger.error(
                    `${this.definition.type}-${metadata.ContentType}.create:: error updating: '${path}'. ${ex.message}`
                );
            }
        }
    }
    ...
    static async update(metadata) {
        ...
        let response;
        try {
            response = await super.updateSOAP(metadata, 'DataFolder', true);
            if (response) {
                response.body.Results[0].Object = metadata;
                response.body.Results[0].Object.CustomerKey = tempCustomerKey;
                delete response.body.Results[0].Object.$;
