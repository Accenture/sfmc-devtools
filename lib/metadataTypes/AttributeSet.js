...
class AttributeSet extends MetadataType {
    ...
    static async preDeployTasks(metadata) {
        ...
        if (Array.isArray(metadata.relationships)) {
            ...
            for (const relationship of metadata.relationships) {
                for (const type of ['left', 'right']) {
                    relationship[type + 'Item'].connectingID = relationship[type + 'Item']
                        .connectingID || {
                    ...
                    switch (relationship[type + 'Item'].relationshipType) {
                        case 'AttributeGroup': {
                            ...
                            // resolve field values
                            for (const attr of relationship.relationshipAttributes) {
                                const relationshipObj = cache.getByKey(
                                    'attributeSet',
                                    attr['c__' + type + 'FullyQualifiedName'].split('.')[0]
                                );
                                attr[type + 'AttributeID'] = relationshipObj.valueDefinitions.find(
                                    (item) =>
                                        item.fullyQualifiedName ===
                                        attr['c__' + type + 'FullyQualifiedName']
                                ).valueDefinitionID;
                                attr[type + 'ConnectingID'] = attr[type + 'ConnectingID'] || {
                                    identifierType: 'FullyQualifiedName',
                                };

                                delete attr['c__' + type + 'FullyQualifiedName'];
                            }

                            // cleanup
                            delete relationship[type + 'Item']?.r__attributeGroup_definitionKey;
                            break;
                        }
                        case 'AttributeSet': {
                            try {
                                ...
                                // resolve field values
                                for (const attr of relationship.relationshipAttributes) {
                                    attr[type + 'AttributeID'] =
                                        relationshipObj.valueDefinitions.find(
                                            (item) =>
                                                item.fullyQualifiedName ===
                                                attr['c__' + type + 'FullyQualifiedName']
                                        ).valueDefinitionID;
                                ...
                                // cleanup
                                delete relationship[type + 'Item']?.r__attributeSet_definitionKey;
                            } catch (ex) {
                                Util.logger.warn(
                                    ` - ${this.definition.type} ${
                                        metadata[this.definition.keyField]
        ...
        // fullyQualifiedName and definitionName.value are equal to 'name' and need to be auto-populated for deploy
        metadata.fullyQualifiedName = metadata.name;
