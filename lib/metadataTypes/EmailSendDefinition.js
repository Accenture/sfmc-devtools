...
class EmailSendDefinition extends MetadataType {
    ...
    static async preDeployTasks(metadata) {
        ...
        // folder
        try {
            metadata.CategoryID = cache.search(
                this.buObject.mid,
                'folder',
        ...
        try {
            ...
            if (metadata.r__email_Name) {
                // classic
                metadata.Email.ID = cache.search(
                    this.buObject.mid,
                    'email',
        ...
        for (const sdl of metadata.SendDefinitionList) {
            ...
            if (sdl.r__dataExtension_Key) {
                ...
                sdl.CustomObjectID = cache.search(
                    this.buObject.mid,
                    'dataExtension',
            ...
            if (sdl.r__list_PathName) {
                sdl.List = {
                    ID: cache.getListObjectIdFromCache(
                        this.buObject.mid,
    ...
    static parseMetadata(metadata) {
        ...
        // folder
        try {
            metadata.r__folder_Path = cache.search(
                this.buObject.mid,
                'folder',
                metadata.CategoryID,
                'ID',
        ...
        try {
            // classic
            const classicEmail = cache.search(
                this.buObject.mid,
                'email',
                metadata.Email.ID,
                'ID',
            ...
            try {
                // content builder
                const contentBuilderEmailName = cache.search(
                    this.buObject.mid,
                    'asset',
                    metadata.Email.ID,
                    'legacyData.legacyId',
                ...
                const contentBuilderEmailKey = cache.search(
                    this.buObject.mid,
                    'asset',
                    metadata.Email.ID,
                    'legacyData.legacyId',
        ...
        for (const sdl of metadata.SendDefinitionList) {
            ...
            if (sdl.CustomObjectID) {
                try {
                    sdl.r__dataExtension_Key = cache.search(
                        this.buObject.mid,
                        'dataExtension',
                        sdl.CustomObjectID,
                        'ObjectID',
            ...
            // List
            try {
                sdl.r__list_PathName = cache.getListPathName(this.buObject.mid, sdl.List.ID, 'ID');
                delete sdl.List;
            } catch (ex) {
                Util.logger.warn(
                    `${this.definition.typeName} '${metadata.Name}'/'${metadata.CustomerKey}': ${ex.message}`
