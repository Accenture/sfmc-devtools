...
class MobilePush extends MetadataType {
    ...
    static async preDeployTasks(metadata) {
        if (metadata.r__mobileApplication_key) {
            // do a searchForField which throws if not found, so that the deployment fails instead of creating a mobilePush without a mobileApplication
            cache.searchForField(
                'mobileApplication',
                metadata.r__mobileApplication_key,
                'id',
                'id'
            );
            // actually assign the mobileApplication to the metadata object so that it can be deployed
            metadata.application = cache.getByKey(
                'mobileApplication',
                metadata.r__mobileApplication_key
            );
            delete metadata.r__mobileApplication_key;
        }
