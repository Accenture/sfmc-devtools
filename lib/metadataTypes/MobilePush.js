...
class MobilePush extends MetadataType {
    ...
    static postRetrieveTasks(metadata) {
        ...
        if (metadata.application?.id) {
            try {
                ...
                Util.logger.warn(
                    `Could not find mobilePushApp with id ${metadata.application.id} for mobilePush ${metadata.name} (${metadata.id}).`
                );
            }
        }
        // asset
    ...
    static async preDeployTasks(metadata) {
        if (metadata.r__mobilePushApp_key) {
            const mobilePushApp = cache.getByKey('mobilePushApp', metadata.r__mobilePushApp_key);
            if (!mobilePushApp) {
                throw new Error(
                    `mobilePushApp ${metadata.r__mobilePushApp_key} not found in cache`
                );
            }
