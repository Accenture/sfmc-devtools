...
class DataExtension extends MetadataType {
    ...
    static oldFields;
    static dataFilterMap = {};
    static deIdDataFilterMap = {};
    ...
    static async retrieve(retrieveDir, additionalFields, _, key) {
        ...
        if (cache.getCache()?.dataFilterHidden) {
            const fdhMap = cache.getCache()?.dataFilterHidden;
            for (const key in fdhMap) {
                const item = fdhMap[key];
                if (
                    item.description.startsWith('Filter generated automatically for filtered DE ')
                ) {
                    const deKey = item.description.split(
                        'Filter generated automatically for filtered DE '
                    )[1];
                    const deObjectId = item.name.split(deKey + '-')[1];
                    this.deIdDataFilterMap[deObjectId] = item;
                }
            }
        }
    ...
    static async postRetrieveTasks(metadata) {
        ...
        if (this.deIdDataFilterMap[metadata.ObjectID]) {
            metadata.r__dataFilterHidden_key = this.deIdDataFilterMap[metadata.ObjectID].key;
        }
        return metadata;
    }
    ...
    static async preDeployTasks(metadata) {
        ...
        delete metadata.r__dataFilterHidden_key;
