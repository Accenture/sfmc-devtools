...
class DataExtension extends MetadataType {
     ...
     * @returns {void}
     */
    static async postDeployTasks(upsertedMetadata, metadata) {
        if (DataExtension.oldFields) {
            // at least some dataExtensions were updated
            Util.logger.info(
        ...
        for (const key in upsertedMetadata) {
            ...
            if (cachedVersion) {
                ...
                if (item.Fields !== '' && existingFields) {
                    for (const tempValue of metadata[key].Fields.Field) {
                        newElementsArray.push(tempValue);
                    }

                    const keys = Object.keys(existingFields);
                    for (const key of keys) {
                        existingFieldsArray.push(existingFields[key]);
                    }

                    existingFieldsArray.sort((a, b) => a.Ordinal - b.Ordinal);

                    for (const valueExistingField of existingFieldsArray) {
                        for (const valueNewElements of newElementsArray) {
                            if (Object.is(valueNewElements.ObjectID, valueExistingField.ObjectID)) {
                                // replace existingField with the updated one

                                existingFieldsArray[
                                    existingFieldsArray.indexOf(valueExistingField)
                                ] = valueNewElements;
                                newElementsArray.pop(newElementsArray.indexOf(valueNewElements));
                            }
                        }
                    }

                    if (newElementsArray.length) {
                        existingFieldsArray.push(...newElementsArray);
                    }

                    delete item.Fields;
                    item.Fields = existingFieldsArray;
     ...
     * @returns {Promise.<{metadata: DataExtension, type: string}>} Promise of items
     */
