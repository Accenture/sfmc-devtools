...
class DataExtension extends MetadataType {
    ...
    static async retrieve(retrieveDir, additionalFields, buObject, _, isDeploy) {
        ...
        if (!retrieveDir && buObject.eid !== buObject.mid) {
            ...
            cache.mergeMetadata(this.buObject.eid, 'folder', result.metadata);
            ...
            for (const metadataEntry in metadataParentBu) {
                try {
                    ...
                    const folderContentType = cache.search(
                        this.buObject.eid,
                        'folder',
    ...
    static _parseMetadata(metadata) {
        ...
        try {
            metadata.r__folder_ContentType = cache.search(
                this.buObject.mid,
                'folder',
                metadata.CategoryID,
                'ID',
        ...
        if (metadata.Template?.CustomerKey) {
            try {
                metadata.r__dataExtensionTemplate_Name = cache.search(
                    this.buObject.mid,
                    'dataExtensionTemplate',
