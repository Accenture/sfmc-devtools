...
class DataExtension extends MetadataType {
     ...
     * @returns {void}
     */
    static async postDeployTasks(upsertedMetadata) {
        ...
        for (const key in upsertedMetadata) {
            ...
            if (cachedVersion) {
                ...
                if (item.Fields !== '' && existingFields) {
                    ...
                    // ! this is inefficient because it triggers a new download of the fields
                    item.Fields.length = 0;
                    await this._retrieveFieldsForSingleDe(upsertedMetadata, key);
                }
     ...
     */
    static async retrieve(retrieveDir, additionalFields, buObject, _, key, isDeploy) {
        ...
        console.log('DataExtension.retrieve:', metadata);
