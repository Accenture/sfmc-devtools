...
class DataExtension extends MetadataType {
    ...
    static async upsert(metadataMap) {
        ...
        for (const metadataKey in metadataMap) {
            if (metadataMap[metadataKey].Name.startsWith('_')) {
                Util.logger.warn(
                    ` ☇ skipping dataExtension ${metadataMap[metadataKey].Name}: Cannot Upsert Strongly Typed Data Extensions`
                );
                filteredByPreDeploy++;
                continue;
            }
            if (
                ...
                metadataMap[metadataKey].r__folder_Path.startsWith('Shared Items')
            ) {
                // this needs to be run before executing preDeployTasks()
                Util.logger.warn(
                    ` ☇ skipping dataExtension ${metadataMap[metadataKey].Name}: Cannot Create/Update a Shared Data Extension from the Child BU`
                );
                filteredByPreDeploy++;
                continue;
            }
            if (
                metadataMap[metadataKey].r__folder_Path.startsWith('Synchronized Data Extensions')
            ) {
                // this needs to be run before executing preDeployTasks()
                Util.logger.warn(
                    ` ☇ skipping dataExtension ${metadataMap[metadataKey].Name}:Cannot Create/Update a Synchronized Data Extension. Please use Contact Builder to maintain these`
                );
                filteredByPreDeploy++;
                continue;
            }
            try {
                ...
                Util.logger.error(
                    `- dataExtension ${metadataMap[metadataKey].Name}: ${ex.message}`
                );
        ...
        const createResults = (
            ...
            .filter((r) => r !== undefined && r !== null)
            .filter(this._filterUpsertResults);
    ...
    static async postDeleteTasks(customerKey) {
        ...
        await super.postDeleteTasks(customerKey, [`${this.definition.type}.md`]);
    }
