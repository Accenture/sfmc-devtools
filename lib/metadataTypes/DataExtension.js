    ...
    static async handleNotFoundDependencies(notFound, notFoundList) {
        ...
        for (const key of notFound) {
            ...
            if (contentType === 'shared_dataextension') {
                Util.logger.warn(
                    `dataExtension '${key}' is a shared dataExtension stored in _ParentBU_.`
                );
                Util.logger.warn(`It cannot be included in the child BU deployment package.`);
                Util.logger.warn(
                    `References to it may need to be changed during deployment via mcdev templating or manually.`
                );
                Util.logger.warn(
                    `To update the shared dataExtension itself, run build for the parent BU and deploy it separately.`
                );
                notFoundList[this.definition.type] ||= [];
                notFoundList[this.definition.type].push(key);
            } else if (contentType === 'synchronizeddataextension') {
                Util.logger.warn(
                    `dataExtension '${key}' is a synchronized dataExtension stored in _ParentBU_.`
                );
                Util.logger.warn(`It cannot be included in the child BU deployment package.`);
                Util.logger.warn(
