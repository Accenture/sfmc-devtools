...
class DataExtension extends MetadataType {
    ...
    /**
     * Returns paths in the _ParentBU_ retrieve folder for keys not found in the child BU folder.
     * Shared dataExtensions are always stored under _ParentBU_, never on the child BU.
     * Used by {@link MetadataType.getDependentFiles} to avoid false "not found" warnings.
     *
     * @param {string[]} keyArr keys that were not found in the child BU retrieve folder
     * @returns {Promise.<string[]>} file paths to check in the _ParentBU_ folder
     */
    static async getAlternativeFilesToCommit(keyArr) {
        // shared DEs are always in _ParentBU_; no alternative needed when already on parent BU
        if (this.buObject.eid === this.buObject.mid) {
            return [];
        }
    ...
    /**
     * Looks for a dataExtension file in the _ParentBU_ retrieve folder when not found in the
     * child BU folder. Shared dataExtensions are always stored under _ParentBU_.
     * Used by {@link MetadataType.buildTemplate} as a secondary file location.
     *
     * @param {string} templateDir primary retrieve directory (child BU)
     * @param {string[]} typeDirArr type sub-directory array, e.g. ['dataExtension']
     * @param {string} templateName key/name of the metadata item
     * @param {string} fileName full file name with extension suffix
     * @param {Error} ex the original error from the primary read attempt
     * @returns {Promise.<string>} file content string or undefined if not found
     */
    ...
    static async readSecondaryFolder(templateDir, typeDirArr, templateName, fileName, ex) {
        ...
        const parentBUDir = File.normalizePath([
            this.properties.directories.retrieve,
            this.buObject.credential,
            Util.parentBuName,
        ]);
        return File.readFilteredFilename([parentBUDir, ...typeDirArr], fileName, 'json');
    }
