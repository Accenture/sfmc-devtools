...
class DataExtension extends MetadataType {
    ...
    constructor(properties, buObject) {
        ...
        this.definition = require('../MetadataTypeDefinitions').dataExtension;
        const DataExtensionField = require(`./DataExtensionField`);
        this.dataExtensionField = new DataExtensionField(properties, buObject);
    }
    /**
     ...
     * @returns {void}
     */
    _cleanupRetentionPolicyFields(metadata) {
        if (
    ...
    async update(metadata) {
        ...
        this.dataExtensionField.oldFields = this.dataExtensionField.oldFields || {};
        this.dataExtensionField.oldFields[metadata.CustomerKey] =
            await this.dataExtensionField.prepareDeployColumnsOnUpdate(
                metadata.Fields,
     ...
     * @returns {void}
     */
    postDeployTasks(upsertedMetadata) {
        if (!DataExtension.oldFields) {
            // only run postDeploy if we are in update mode
            return;
        }
        Util.logger.info(
            'Re-retrieving updated dataExtension fields from server to ensure correct info in local JSONs and Docs.'
        );
     ...
     */
    async retrieve(retrieveDir, additionalFields, _, isDeploy) {
        ...
        if (isDeploy || (metadata && retrieveDir)) {
            ...
            for (const metadataEntry in metadata) {
                metadata[metadataEntry].Fields.sort(this.dataExtensionField.sortDeFields);
            }

            // remove attributes that we do not want to retrieve
            // * do this after sorting on the DE's field list
            fieldKeys.forEach((key) => {
                this.dataExtensionField.postRetrieveTasks(fieldsObj[key], true);
            });
        }
        const path = require('path');
        Util.logger.info(path.resolve(`./cache-${this.buObject.mid}.json`));
        await File.writeJSONToFile(`.`, `cache-${this.buObject.mid}`, cache.getCache());
        if (!retrieveDir && !this.buObject.isParent) {
            ...
            // init folder with parent SDK for retrieve
            const Folder = require(`./metadataTypes/DataExtensionField`);
            const parentfolder = new Folder(this.properties, {
                eid: this.buObject.eid,
                mid: this.buObject.eid,
                businessUnit: Util.parentBuName,
                credential: this.buObject.credential,
            });
            const result = await parentfolder.retrieveForCache();
     ...
     * @returns {Promise<{metadata:DataExtensionMap,type:string}>} Promise of item map
     */
    async retrieveChangelog(additionalFields) {
     ...
     */
    async postRetrieveTasks(metadata, _, isTemplating) {
     ...
     */
    async _retrieveFields(options, additionalFields) {
        if (!options) {
        ...
        const response = await this.dataExtensionField.retrieveForCache(options, additionalFields);
     ...
     */
    async _retrieveFieldsForSingleDe(metadata, customerKey) {
        const fieldOptions = {
            filter: {
        ...
        const fieldArr = this.dataExtensionField.convertToSortedArray(fieldsObj);

        // remove attributes that we do not want to retrieve
        // * do this after sorting on the DE's field list
        fieldArr.forEach((field) => {
            this.dataExtensionField.postRetrieveTasks(field, true);
        });
     ...
     */
    async _writeDoc(directory, filename, json, mode, fieldsToKeep) {
     ...
     */
    async document(metadata, isDeploy) {
        if (!metadata) {
     ...
     * @returns {Promise} Promise
     */
    async retrieveForCache(_, isDeploy) {
        return this.retrieve(null, ['ObjectID', 'CustomerKey', 'Name'], null, isDeploy);
    }
    /**
     ...
     */
    async _retrieveAll(additionalFields, options) {
        const metadata = (await super.retrieveSOAP(null, null, options, additionalFields)).metadata;
        for (const key in metadata) {
