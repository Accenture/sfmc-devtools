        ...
        if (Util.OPTIONS.matchName) {
            ...
            if (potentials.length > 1) {
                const potentialsWithMatchingFolder = potentials.filter(
                    (el) => el.r__folder_Path === metadataItem.r__folder_Path
                );
                throw new Error(
                    `found multiple name matches in cache. New folder: ${metadataItem.r__folder_Path}. Identified keys: ${potentials.map((p) => p[this.definition.keyField]).join(', ')}. ` +
                ...
                if (
                    ...
                    throw new Error(
                        `found ${this.definition.type} ${metadataItem[this.definition.keyField]} in cache by name but folder is different. New: "${deployFolderPath}". Old: "${cacheFolderPath}": Identified key: ${potentials[0][this.definition.keyField]}`
                    );
                }
            } else {
                Util.logger.debug(
