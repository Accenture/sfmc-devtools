...
class Filter extends MetadataType {
    ...
    static _preDeploy_dataTypeMapping(target, metadata, cachedVersion) {
        ...
        if (metadata[`r__${target}_dataExtension_key`]) {
            ...
            if (
                cachedVersion &&
                cachedVersion[`r__${target}_dataExtension_key`] !==
                    metadata[`r__${target}_dataExtension_key`]
            ) {
                throw new Error(`Updating r__${target}_dataExtension_key is not allowed.`);
    ...
    static async preDeployTasks(metadata) {
        ...
        if (metadata.r__dataFilter_key) {
            ...
            if (cachedVersion && cachedVersion.r__dataFilter_key !== metadata.r__dataFilter_key) {
                throw new Error(`Updating r__dataFilter_key is not allowed.`);
                // metadata.filterDefinitionId = cachedVersion.filterDefinitionId;
            }
