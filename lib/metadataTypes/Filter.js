...
class Filter extends MetadataType {
    ...
    static postRetrieveTasks(metadata) {
        ...
        try {
            // filterDefinition
            metadata.r__filterDefinition_key = cache.searchForField(
                'filterDefinition',
    ...
    static _preDeploy_dataTypeMapping(target, metadata) {
        ...
        if (metadata[`r__${target}_dataExtension_key`]) {
            ...
            if (target === 'source') {
                ...
                metadata.filterDefinitionSourceTypeId = 2;
    ...
    static async preDeployTasks(metadata) {
        ...
        // filterDefinition
        if (metadata.r__filterDefinition_key) {
            metadata.filterDefinitionId = cache.searchForField(
                'filterDefinition',
                metadata.r__filterDefinition_key,
                'key',
                'id'
            );
