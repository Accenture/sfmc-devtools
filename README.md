...
2. Acquire the URL to your Git repo and Clone it to your computer. It should end on `.git`. Also ask your team lead for `EID`, `Client ID`, `Client Secret` and the `Authentication Base URI`. You will need this later.
...
7. At this point the system will recognize the previously set up project and ask you for `EID`, `Client ID`, `Client Secret` and the `Authentication Base URI`.
8. Done.
...
The interactive setup will ask you for an `EID`, `Client ID` and `Client Secret` of an installed package. It also asks for the `Authentication Base Uri`. Each installed package on a given SFMC instance shares the same tenant sub-domain and always shows you 3 domains (Auth, REST and SOAP).
